#!/usr/bin/env python3
"""GC NY Continuation Longs — Round 1 Variable Sweeps (clean data, 1s magnifier).

Re-optimization from scratch on clean GC.v.0 data (2026-02-21).
Prior R6 anchor used as starting point (methodology was sound, numbers invalid).

Anchor:
  stop=4.0% ATR, min_gap=2.5%, rr=4.5, tp1=0.5, entry_end=11:00
...
# ── Anchor config (prior R6 winner — to be re-confirmed on clean data) ────────
...
GC_NY_ANCHOR = SessionConfig(
    ...
    entry_start="09:40",
    entry_end="11:00",        # Hard cliff from prior sweeps
    flat_start="15:50",
    flat_end="16:00",
    stop_atr_pct=4.0,
    min_gap_atr_pct=2.5,
...
def sweep_max_gap_points(anchor_calmar):
    ...
    for v in values:
        ...
        sess = SessionConfig(
            ...
            entry_start="09:40", entry_end="11:00",
            flat_start="15:50", flat_end="16:00",
            stop_atr_pct=4.0, min_gap_atr_pct=2.5,
...
def print_anchor_metrics(m):
    section("ANCHOR METRICS (Phase 1 Baseline)")
    print(f"  Config: stop=4.0% | rr=4.5 | min_gap=2.5% | tp1=0.5 | ATR 16")
...
if __name__ == "__main__":
    print()
    print("=" * 70)
    print("  GC NY CONT LONGS — R1 VARIABLE SWEEPS (clean data, 1s magnifier)")
    print("  Anchor: stop=4.0% | rr=4.5 | min_gap=2.5% | tp1=0.5 | ATR 16")
    ...
    print("  If anchor changes, re-run sweeps on new anchor (R2).")
    print("=" * 70)
    print()
