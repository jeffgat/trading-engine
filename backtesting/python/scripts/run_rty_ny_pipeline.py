#!/usr/bin/env python3
"""Robust pipeline: RTY NY Continuation Longs (1s magnifier).

Converged config from R7 fine-tune variable sweeps:
  stop=3.0% ATR, min_gap=1.0%, rr=5.5, tp1=0.45
  ATR 14, 15m ORB (09:30-09:45), entry≤15:30, flat_start=15:50
  long-only, no DOW exclusion, 1s bar magnifier

Five phases:
  Phase 1: Structural validation (full history, base config, 1s magnifier)
  Phase 2: Walk-forward + parameter stability (36m IS / 12m OOS / 12m step, 1m magnifier)
  Phase 3: Prop firm constraint filter (on combined WF OOS trades)
  Phase 4: Hold-out OOS test (2025-01-01 onward, mode params from WF, 1s magnifier)
...
# ── Base config (R7 converged) ───────────────────────────────────────────────

RTY_NY = SessionConfig(
    name="NY",
    orb_start="09:30",
...
BASE_CONFIG = StrategyConfig(
    rr=5.5,                  # Fine-tune grid winner
    tp1_ratio=0.45,          # Fine-tune grid winner
    risk_usd=5000.0,
    atr_length=14,
    min_qty=1.0,
    qty_step=1.0,
    ...
    name="RTY NY Pipeline",
)

DOW_EXCL = set()  # No DOW exclusion
...
# ── WF sweep params (centred on fine-tune grid winner) ───────────────────────

PARAM_RANGES = {
    "rr":                   [4.5, 5.0, 5.5, 6.0, 6.5],
    "tp1_ratio":            [0.35, 0.4, 0.45, 0.5, 0.55],
...
def phase1(df, df_1m, df_1s):
    ...
    print(f"  Config: stop=3.0% | rr=5.5 | min_gap=1.0% | tp1=0.45 | ATR 14 | 15m ORB")
    print(f"  Direction: long-only | entry≤15:30 | flat=15:50 | Magnifier: 1s")
...
def phase2(df, df_1m):
    ...
    print(f"  Magnifier: 1m (for WF speed)")
...
def main():
    print()
    print("=" * 70)
    print("  ROBUST PIPELINE: RTY NY Cont Longs")
    print("  stop=3.0% | rr=5.5 | min_gap=1.0% | tp1=0.45 | ATR 14 | 15m ORB")
    print("  entry≤15:30 | flat=15:50 | long-only | no DOW excl")
    print(f"  WF sweep: {GRID_SIZE} combos/fold (5×5×5×3)")
    print("  P1/P4: 1s magnifier | P2: 1m magnifier (WF speed)")
    print("=" * 70)
    print()

    print("Loading data (5m + 1m + 1s)...")
    t0 = time.time()
