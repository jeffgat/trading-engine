#!/usr/bin/env python3
"""GC NY Continuation Longs — Round 2 Variable Sweeps (clean data, 1s magnifier).

Grid R1 winner adopted as new anchor (changed rr 4.5→4.0, min_gap 2.5→3.5%).
Re-sweep all dimensions to check for interactions.

Anchor:
  stop=4.0% ATR, min_gap=3.5%, rr=4.0, tp1=0.5, entry_end=11:00
  10m ORB (09:30-09:40), long-only, ATR 16, flat_start=15:50
  FOMC dates excluded
...
Sweeps each dimension independently:
  ...
  8. max_gap_atr_pct    — off*, 15%, 20%, 25%, 30%
...
ANCHOR = StrategyConfig(
    rr=4.0,                    # changed from 4.5 (grid R1 winner)
    tp1_ratio=0.5,
    risk_usd=5000.0,
    atr_length=16,
    min_qty=1.0,
...
def sweep_max_gap_atr_pct(anchor_calmar):
    ...
    values = [0.0, 15.0, 20.0, 25.0, 30.0]
    results = []
    for v in values:
        label = f"max_gap_atr={v:.0f}%" if v > 0 else "off [anchor]"
        sess = SessionConfig(
            name="NY", orb_start="09:30", orb_end="09:40",
            entry_start="09:40", entry_end="11:00",
            flat_start="15:50", flat_end="16:00",
...
def print_anchor_metrics(m):
    section("ANCHOR METRICS (Grid R1 Winner)")
    print(f"  Config: stop=4.0% | rr=4.0 | min_gap=3.5% | tp1=0.5 | ATR 16")
    print(f"          10m ORB | entry→11:00 | long-only | FOMC excl | 1s magnifier")
    print(f"  Changed from R1: rr 4.5→4.0, min_gap 2.5%→3.5%")
    print()
...
if __name__ == "__main__":
    print()
    print("=" * 70)
    print("  GC NY CONT LONGS — R2 VARIABLE SWEEPS (clean data, 1s magnifier)")
    print("  Anchor: stop=4.0% | rr=4.0 | min_gap=3.5% | tp1=0.5 | ATR 16")
    print("          10m ORB | entry→11:00 | FOMC excl | long-only")
    print("  Changed from R1: rr 4.5→4.0, min_gap 2.5%→3.5%")
    print("=" * 70)
    ...
    print("  If anchor changes, re-run sweeps on new anchor (R3).")
    print("=" * 70)
    print()
