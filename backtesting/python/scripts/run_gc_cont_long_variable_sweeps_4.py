#!/usr/bin/env python3
"""GC NY Continuation Longs — Round 4 Variable Sweeps (clean data, 1s magnifier).

R3 adopted max_gap_atr=30% (Calmar 13.10 vs 11.80, Δ=+1.30).
...
Anchor:
  ...
  max_gap_atr=30%, FOMC dates excluded
...
# ── Anchor config (R3 winner: max_gap_atr=30% adopted) ───────────────────────
...
def print_anchor_metrics(m):
    section("ANCHOR METRICS (R3 Winner: max_gap_atr=30%)")
    ...
    print(f"  Changed from R3: added max_gap_atr=30%")
    print()
...
if __name__ == "__main__":
    ...
    print("  If anchor changes, re-run sweeps on new anchor (R5).")
    print("=" * 70)
    print()
