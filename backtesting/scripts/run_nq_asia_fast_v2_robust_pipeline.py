#!/usr/bin/env python3
"""NQ Asia FAST_V2 — 5-Phase Robust Pipeline (Bailey-aware).

FAST_V2 execution config NQ_Asia leg:
  ORB: 20:00-20:15, entry until 22:30, flat 04:00-04:10
  stop=150.0% ORB, min_gap=15.0% ORB, rr=5.0, tp1=0.25
  Both directions, Tuesday excluded
...
# ── Candidate config (FAST_V2 execution NQ_Asia) ───────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ Asia FAST_V2 Robust Pipeline",
)
...
def phase_4(df, df_1m, df_1s, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST_V2 NQ_Asia robust pipeline",
    )
    if entry.warning:
...
def main():
    ...
    print(f"\n  Candidate: stop={CANDIDATE_SESSION.stop_orb_pct}% ORB rr={CANDIDATE.rr} "
          f"gap={CANDIDATE_SESSION.min_gap_orb_pct}% ORB tp1={CANDIDATE.tp1_ratio}")
    ...
    print(f"  Candidate: stop={CANDIDATE_SESSION.stop_orb_pct}% ORB rr={CANDIDATE.rr} "
          f"gap={CANDIDATE_SESSION.min_gap_orb_pct}% ORB tp1={CANDIDATE.tp1_ratio}")
