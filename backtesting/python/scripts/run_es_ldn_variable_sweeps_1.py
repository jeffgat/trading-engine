#!/usr/bin/env python3
"""ES LDN Continuation Both — Variable Sweeps Round 1 (Full Optimization Step 2).

Sweep 12 dimensions one at a time, all others held at anchor.
Anchor from baseline: stop=10.0%, rr=2.5, gap=1.0%, tp1=0.5, ATR 14,
  ORB 15m (03:00-03:15), entry_end=08:25, flat=08:20, both dirs, ICF off.

Adoption rule: Calmar delta > +0.3 AND no new negative full years
  AND trade count >100 AND median stop >= 10 ticks.
ES minimum stop floor: 3.0% ATR.
"""
...
FULL_YEARS = [str(y) for y in range(2016, 2026)]  # exclude 2026 partial

# Anchor — starts as baseline defaults
...
def run_and_measure(df, config, df_1m, df_1s, dow_filter=None):
    """Run backtest, optionally apply DOW filter, return (trades, metrics)."""
    trades = run_backtest(df, config, start_date=START_DATE, df_1m=df_1m, df_1s=df_1s)
    if dow_filter:
        trades = apply_dow_filter(trades, dow_filter)
    m = compute_metrics(trades)
    return trades, m


def print_sweep_table(results, dim_name, anchor_value):
    """Print formatted table for one dimension sweep."""
    print(f"\n{'='*90}")
...
def best_by_calmar(results, anchor_value, anchor_neg_years):
    """Find the best value by Calmar, respecting adoption rule."""
    best_val, best_cal, best_m = None, -999, None
    for val, m in results:
        c = calmar(m)
        ny = neg_year_set(m)
        # Must not introduce new negative years beyond anchor
...
def sweep_stop(df, df_1m, df_1s):
    """Dim 1: Stop ATR %"""
    values = [3.0, 4.0, 5.0, 6.0, 7.5, 10.0, 12.0, 15.0]
    results = []
    for v in values:
...
def sweep_flat_start(df, df_1m, df_1s):
    ...
    for v in values:
        flat_end = "08:25"
        sess = replace(ANCHOR_SESSION, flat_start=v, flat_end=flat_end)
        cfg = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_measure(df, cfg, df_1m, df_1s)
        results.append((v, m))
...
def sweep_dow(df, df_1m, df_1s):
    """Dim 10: DOW exclusion (post-backtest filter)"""
    # Run anchor once, then filter
    ...
    for label, excluded in filters.items():
        ...
        results.append((label, m))
    print_sweep_table(results, "DOW Exclusion", "none")
    return results


def sweep_max_gap(df, df_1m, df_1s):
    """Dim 11: Max gap ATR % (ES uses max_gap_points historically, but test ATR-relative too)"""
    # Test max_gap_points first (historical ES approach)
    values = [("OFF", 0.0), ("20pt", 20.0), ("50pt", 50.0), ("75pt", 75.0), ("100pt", 100.0)]
    results = []
    for label, v in values:
        if v == 0.0:
            sess = replace(ANCHOR_SESSION, max_gap_points=0.0, max_gap_atr_pct=0.0)
        else:
            sess = replace(ANCHOR_SESSION, max_gap_points=v, max_gap_atr_pct=0.0)
        cfg = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_measure(df, cfg, df_1m, df_1s)
        results.append((label, m))

    # Also test ATR-relative
    atr_values = [("20%ATR", 20.0), ("50%ATR", 50.0), ("75%ATR", 75.0), ("100%ATR", 100.0), ("150%ATR", 150.0)]
    for label, v in atr_values:
        sess = replace(ANCHOR_SESSION, max_gap_points=0.0, max_gap_atr_pct=v)
        cfg = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_measure(df, cfg, df_1m, df_1s)
        results.append((label, m))
...
if __name__ == "__main__":
    print("=" * 90)
    print("  ES LDN CONTINUATION BOTH — VARIABLE SWEEPS ROUND 1")
    print("=" * 90)
    print(f"  Anchor: stop={ANCHOR_SESSION.stop_atr_pct}%, rr={ANCHOR.rr}, "
          ...
          f"dir={ANCHOR.direction_filter}, ICF={ANCHOR.impulse_close_filter}")
    print()
    ...
    for dim_name, anchor_val, sweep_fn in sweeps:
        ...
        # Find best
        best_val, best_cal, best_m = best_by_calmar(results, anchor_val, anchor_neg)
        all_sweeps[dim_name] = {
    ...
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY — Variable Sweeps Round 1")
    ...
    for dim_name, info in all_sweeps.items():
        delta = info["delta"]
        new_neg = info["best_neg_years"] - anchor_neg  # truly new negative years
        adopt = (delta > 0.3
                 and len(new_neg) == 0
    ...
    if adoptions:
        print(f"\n  >>> {len(adoptions)} adoption(s) — update anchor and re-sweep (round 2)")
    else:
