#!/usr/bin/env python3
"""NQ Asia v3 — Five-phase robust pipeline.

Confirmed anchor (R16 validated at flat_start=00:00):
  stop=3.7%, gap=0.90%, maxgap=5.0%, rr=1.75, tp1=0.35
  ATR 5, ORB 10m, entry_end=23:00, flat_start=00:00, no-Thursday
  → Calmar 20.14, 18.3 R/yr, DD -10.1R, Sharpe 2.123, 0 neg years (2015-2026)

Phases:
  1. Structural validation (2015-2026 full history)
  2. Walk-forward + parameter stability (36m IS / 12m OOS / 12m step, 2015-2024)
  3. Prop firm constraint filter on WF OOS trades
  4. Hold-out OOS (2025-01-01 onwards)
  5. Monte Carlo survival (2000 sims, -10R ruin threshold)
"""
...
def main():
    section("NQ ASIA v3 — ROBUST PIPELINE")
    print("Anchor: stop=3.7%, gap=0.90%, maxgap=5%, rr=1.75, tp1=0.35")
    print("        ATR 5, ORB 10m, entry≤23:00, flat_start=00:00, no-Thursday")
    print("        Full-history ref: Calmar 20.14, 18.3 R/yr, DD -10.1R, Sharpe 2.123")
    ...
    param_ranges = {
        "asia_stop_atr_pct":    [3.0, 3.5, 3.7, 4.0, 4.5],
        "asia_min_gap_atr_pct": [0.75, 0.90, 1.00, 1.10, 1.20],
        "rr":                   [1.25, 1.50, 1.75, 2.00, 2.25],
    ...
    print(f"    Trades={cm['total_trades']}, R={cm['total_r']:.1f}, "
          ...
          f"Calmar={cm.get('calmar_ratio',0):.2f}, Sharpe={cm['sharpe_ratio']:.3f}, "
          f"DD={cm['max_drawdown_r']:.1f}R")
    ...
    with open("/tmp/wf_nq_asia_v3.pkl", "wb") as f:
    ...
    db_name = f"NQ ASIA 2015-2026 v3 flat00 Pipeline {verdict}"
    db_notes = (
        f"v3 robust pipeline. stop=3.7%, gap=0.90%, maxgap=5%, rr=1.75, tp1=0.35, "
        f"ATR 5, ORB 10m, entry<=23:00, flat_start=00:00, no-Thu. "
        f"WF objective: Calmar. 36m IS / 12m OOS / 12m step. "
