#!/usr/bin/env python3
"""Robust pipeline: 6B LDN Inversion Longs (1s magnifier).

Structural exploration anchor (324 configs tested):
  strategy=inversion, long-only, rr=4.0, tp1=0.50, ATR 50
  30m ORB (03:00-03:30), entry→08:25, flat 08:20-08:25
  stop_atr_pct=15.0, min_gap=1.0 (default)

Structural winner: Calmar 4.07, +87.4R, PF 1.20, DD -21.5R, 828 trades.
Longs are the profitable direction — shorts only reached Calmar 2.37.
Wide stops (15%) critical; stop=5% catastrophic, stop=10% mostly negative.
...
from orb_backtest.config import LDN_SESSION, StrategyConfig, SessionConfig, with_overrides
...
# ── WF sweep params (centred on validated anchor) ─────────────────────────────

PARAM_RANGES = {
    "rr":               [3.5, 4.0, 4.5],
    "tp1_ratio":        [0.40, 0.50, 0.60],
    "ldn_stop_atr_pct": [12.0, 14.0, 15.0, 16.0, 18.0],
}
...
def phase1(df, df_1m, df_1s):
    ...
    print(f"  Config: stop=15% | rr=4.0 | min_gap=1.0% | tp1=0.50 | ATR 50")
    print(f"          30m ORB (03:00-03:30) | entry→08:25 | long-only | inversion")
...
def phase3(wf_result):
    ...
    print(f"  {'Annual R (avg full yrs)':<30s} {avg_annual_r:>10.1f} {'>= 18':>12s} {pf(cr.annual_r_passed):>10s}")
    ...
    band = "PASS" if passed else ("CAUTION" if avg_annual_r >= 12.0 else "FAIL")
...
def verdict(results, mc_stats=None, p3_annual_r=None):
    ...
    for phase, passed in results.items():
        extra = ""
        if phase == "phase3" and not passed and p3_annual_r is not None and p3_annual_r >= 12.0:
            extra = f"  (CAUTION: {p3_annual_r:.1f} R/yr — marginal, not NO-GO)"
        print(f"  {phase.upper()}: {'PASS' if passed else 'FAIL'}{extra}")
    print()

    all_pass = all(results.values())
    p3_marginal = not results.get("phase3", True) and p3_annual_r is not None and p3_annual_r >= 12.0
    ...
    if all_pass:
        ...
        print(f"  >>> Tradeable with reduced position size ({p3_annual_r:.1f} vs 18.0 threshold). <<<")
    else:
...
def main():
    print()
    print("=" * 70)
    print("  ROBUST PIPELINE: 6B LDN Inversion Longs (1s magnifier)")
    print("  stop=15% | rr=4.0 | min_gap=1.0% | tp1=0.50 | ATR 50")
    print("  30m ORB (03:00-03:30) | entry→08:25 | long-only | inversion")
