#!/usr/bin/env python3
"""No-ORB GC — sweep BE offset ticks.

Fixes QM=100%, stop=12%, rr=5.0, entry_end=20:00, longs only.
Tests BE offset values: 0, 5, 10, 15, 20, 30
"""
...
def main():
    ...
    for be in BE_VALS:
        ...
        if len(filled) < 10:
            results.append({"be": be, "trades": len(filled),
        ...
        print(f"  be={be} ticks: {m['total_trades']} trades, "
              f"{nr}R, {dd}R DD, Sharpe {m['sharpe_ratio']:.3f}")

    print(f"\nDone in {time.time()-t0:.0f}s\n")

    print("=" * 100)
    print("NO-ORB GC — BE OFFSET SWEEP (QM=100%, stop=12%, rr=5.0, entry_end=20:00, longs)")
    print("=" * 100)
    hdr = (f"{'BE(tks)':>8} | {'Trades':>6} | {'WR':>6} | {'Net R':>7} | "
    ...
    for r in results:
        ...
        print(f"{r['be']:>8} | {r['trades']:>6} | {r.get('wr', 0):>5.1%} | "
    ...
    print(f"\nBest BE: {best['be']} ticks — {best['trades']} trades, "
          f"{best['net_r']}R, {best['max_dd']}R DD, Sharpe {best['sharpe']:.3f}")

    print(f"\n{'='*60}")
    print("YEARLY — best BE config")
    print(f"{'='*60}")
    if "yearly" in best:
