#!/usr/bin/env python3
"""NQ NY Continuation Short — Variable Sweeps Round 10 (ORB-based).

R10 anchor (from R9 adoptions: orbgap=7.0%, DOW=excl M+F):
  ORB: 09:30-09:50 (20m), entry until 15:00, flat 15:50-16:00
  stop_orb_pct=15.0%, min_gap_orb_pct=7.0% (ORB-based)
...
ANCHOR_DOW_EXCL = {0, 4}  # excl Mon+Fri
...
def main():
    ...
    print("=" * 90)
    print(f"Anchor: rr={ANCHOR.rr}, tp1={ANCHOR.tp1_ratio}, orbstop={ANCHOR_SESSION.stop_orb_pct}%, "
          f"orbgap={ANCHOR_SESSION.min_gap_orb_pct}%")
    ...
    # -- 1. STOP ORB % --------------------------------------------------------
    stop_vals = [7.5, 10.0, 12.5, 15.0, 17.5, 20.0, 25.0, 30.0]
    ...
    # -- 3. ATR LENGTH — INFO ONLY (insensitive with ORB-based params) --------
    atr_vals = [5, 14, 30]
    print_header(f"3. ATR LENGTH — INFO ONLY (anchor={ANCHOR.atr_length}, expect insensitive)")
    for i, atr in enumerate(atr_vals, 1):
        cfg = replace(ANCHOR, atr_length=atr)
        _, m = run_and_metric(df_5m, df_1m, df_1s, cfg)
        print_row(i, f"atr={atr}", m, is_base=(atr == ANCHOR.atr_length))
    print(f"      (skipping adoption — ATR insensitive with ORB-based stop/gap)")
    ...
    # -- 9. MIN GAP ORB % -----------------------------------------------------
    gap_vals = [2.0, 3.0, 5.0, 7.0, 10.0, 12.0, 15.0, 20.0]
