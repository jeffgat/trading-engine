#!/usr/bin/env python3
"""NQ Asia ORB — Variable sweeps round 3: re-sweep after R2 anchor change.

R2 adopted (5 changes):
  ORB window:       15m → 10m  (Δ+3.67, oscillated back from R1→R2)
  flat_start:       00:00 → 23:00  (Δ+2.40)
  tp1_ratio:        0.35 → 0.30  (Δ+1.75)
  ICF:              OFF → ON  (Δ+1.65)
  min_gap_atr_pct:  0.90% → 1.25%  (Δ+0.54)

R3 anchor:
  ORB: 20:00-20:10 (10m), entry until 01:00, flat 23:00
  stop=3.7%, min_gap_atr=1.25%, max_gap_atr=5.0%
  rr=1.75, tp1=0.30, ATR=5, direction=both, ICF=ON, continuation, 1s magnifier
...
ANCHOR_SESSION = SessionConfig(
    name="Asia",
    orb_start="20:00",
    orb_end="20:10",         # 10m ORB (oscillated back)
    entry_start="20:10",
    entry_end="01:00",       # R1 adopted
    flat_start="23:00",      # R2 adopted
    flat_end="07:00",
    stop_atr_pct=3.7,
    min_gap_atr_pct=1.25,    # R2 adopted (was 0.90)
    max_gap_points=0.0,
    max_gap_atr_pct=5.0,
)
...
ANCHOR = StrategyConfig(
    ...
    rr=1.75,
    tp1_ratio=0.30,          # R2 adopted (was 0.35)
    atr_length=5,
    impulse_close_filter=True,  # R2 adopted
    name="NQ Asia R3 Anchor",
)
...
def main():
    print("NQ Asia ORB — Round 3: re-sweep after R2 changes")
    print("=" * 90)
    print(f"Anchor: rr={ANCHOR.rr}, tp1={ANCHOR.tp1_ratio}, "
          f"stop={ANCHOR_SESSION.stop_atr_pct}%, gap={ANCHOR_SESSION.min_gap_atr_pct}%, "
          f"maxgap_atr={ANCHOR_SESSION.max_gap_atr_pct}%, ICF=ON")
    print(f"ORB=10m (20:00-20:10), entry≤01:00, flat=23:00, ATR={ANCHOR.atr_length}, "
    ...
    # ── 4. ENTRY END TIME ────────────────────────────────────────────
    entry_ends = ["22:00", "23:00", "00:00", "01:00", "02:00", "03:00"]
    print_header("4. ENTRY END TIME (anchor=01:00)")
    for i, ee in enumerate(entry_ends, 1):
        sess = replace(ANCHOR_SESSION, entry_end=ee)
        config = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"end={ee}", m, is_base=(ee == "01:00"))
        all_results.append(("entry_end", ee, m))

    # ── 5. FLAT START ────────────────────────────────────────────────
    flat_starts = ["22:00", "22:30", "23:00", "23:30", "00:00", "01:00"]
    print_header("5. FLAT START (anchor=23:00)")
    for i, fs in enumerate(flat_starts, 1):
        sess = replace(ANCHOR_SESSION, flat_start=fs)
        config = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"flat={fs}", m, is_base=(fs == "23:00"))
        all_results.append(("flat_start", fs, m))

    # ── 6. MIN GAP ATR % ────────────────────────────────────────────
    gap_values = [0.50, 0.75, 0.90, 1.10, 1.25, 1.50, 1.75, 2.0]
    print_header("6. MIN GAP ATR % (anchor=1.25%)")
    for i, g in enumerate(gap_values, 1):
        sess = replace(ANCHOR_SESSION, min_gap_atr_pct=g)
        config = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"gap={g}%", m, is_base=(abs(g - 1.25) < 0.01))
        all_results.append(("min_gap_atr_pct", g, m))

    # ── 7. MAX GAP ATR % ────────────────────────────────────────────
    maxgap_values = [3.0, 4.0, 5.0, 7.0, 11.0, 0.0]
    ...
    print_header("10. TP1 RATIO (anchor=0.30)")
    for i, tp1 in enumerate(tp1_values, 1):
        config = replace(ANCHOR, tp1_ratio=tp1)
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"tp1={tp1}", m, is_base=(abs(tp1 - 0.30) < 0.01))
    ...
    if not any_adopted:
        ...
        print(f"  Ready for grid sweep.")
    else:
        print(f"\n  ** NOT CONVERGED — Update anchor and re-sweep as R4 **")
