"""Walk-forward optimization for Gap Fill strategies.

Mirrors walkforward.py but uses:
- GapFillStrategyConfig / with_gapfill_overrides instead of StrategyConfig / with_overrides
- run_gapfill_backtest / build_gapfill_signal_cache instead of run_backtest / build_signal_cache
- run_gapfill_sweep / _load_or_build_gapfill_signal_cache instead of run_sweep / _load_or_build_signal_cache
"""
...
from ..gapfill_config import GapFillStrategyConfig, with_gapfill_overrides
from ..engine.gapfill_simulator import run_gapfill_backtest, build_gapfill_signal_cache
...
from .parallel_gapfill import run_gapfill_sweep, _load_or_build_gapfill_signal_cache
...
def _generate_gapfill_param_grid(
    base_config: GapFillStrategyConfig,
    param_ranges: dict[str, list],
) -> list[GapFillStrategyConfig]:
    """Generate all combinations of parameter overrides for Gap Fill configs.

    Mirrors generate_param_grid() from grid.py but uses with_gapfill_overrides
    instead of with_overrides.

    Args:
        base_config: Base Gap Fill strategy configuration.
        param_ranges: Dict mapping param names to lists of values.
            Supports session-prefixed params (e.g., 'ny_rth_open').

    Returns:
        List of GapFillStrategyConfig instances, one per combination.
    """
    if not param_ranges:
        return [base_config]
    ...
    for values in product(*value_lists):
        ...
        configs.append(with_gapfill_overrides(base_config, **overrides))

    return configs


def run_walkforward_gapfill(
    df: pd.DataFrame,
    ...
    """Run walk-forward optimization for a Gap Fill strategy.

    For each fold:
    1. Slice df to IS window (with warmup before IS start)
    2. Run Gap Fill grid sweep on IS data
    ...
    Args:
        ...
        df_1m: Optional 1-minute OHLCV DataFrame for bar magnifier.
            Gap Fill strategies benefit from 1m resolution for fill/exit simulation.
        base_config: Base Gap Fill strategy configuration.
    ...
    configs = _generate_gapfill_param_grid(base_config, param_ranges)
    ...
    print("[wf-gapfill] Pre-building full-range Gap Fill signal cache and maps...")
    full_maps = build_maps(df, df_1m)
    full_signal_cache = _load_or_build_gapfill_signal_cache(df, configs)
    ...
    for fold_idx, window in enumerate(windows):
        ...
        if gate_factory is not None:
            ...
            # signal_bar position. If we pass the full df to run_gapfill_sweep,
        ...
        if gate_factory is None:
            ...
            oos_trades_all = run_gapfill_backtest(
                df, best_config,
            ...
            oos_sig = build_gapfill_signal_cache(oos_df, [best_config])
            oos_trades_all = run_gapfill_backtest(
                oos_df, best_config,
                start_date=window.oos_start, end_date=window.oos_end,
                df_1m=oos_df_1m,
        ...
        # Extract best params
        best_params = _extract_swept_params_gapfill(best_config, param_ranges.keys())
...
def _extract_swept_params_gapfill(
    config: GapFillStrategyConfig,
    param_names,
) -> dict[str, float]:
    """Extract swept parameter values from a Gap Fill config.

    Handles both top-level GapFillStrategyConfig fields (stop_multiplier,
    tp1_ratio, min_gap_atr_pct, max_gap_atr_pct, min_gap_points,
    atr_length, risk_usd, etc.) and session-prefixed params
    (ny_rth_open, asia_flat_start, ldn_flat_end, etc.).

    Session prefixes supported: ny_, asia_, ldn_.
    """
    params = {}
    for name in param_names:
        # Check session-prefixed params (e.g., ny_rth_open)
        for sess_prefix in ("ny_", "asia_", "ldn_"):
            ...
            # Top-level GapFillStrategyConfig attribute
