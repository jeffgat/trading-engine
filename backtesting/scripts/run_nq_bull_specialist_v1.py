#!/usr/bin/env python3
"""Bull-market specialist V1 research runner.

Goal:
- strong performance in 2024-latest
- weak or inactive performance in 2022-2023
...
OUTPUT_DIR = ROOT / "data" / "results" / "nq_bull_specialist_v1"
GATE_NAMES = (
    "none",
    "hh_hl_2_vwap",
    "pullback_holds_vwap",
...
def make_base_config() -> StrategyConfig:
    ...
    return StrategyConfig(
        ...
        excluded_days=(4,),
        name="NQ Bull Specialist V1 Base",
        notes="Bull-market specialist V1 base config.",
    )
...
def build_candidate_specs() -> list[dict[str, object]]:
    ...
    # Current fast-payout base (ungated).
    specs.append(
        {
            "rr": 3.5,
            "tp1_ratio": 0.4,
    ...
    # Current HH/HL+VWAP specialist winner.
    specs.append(
        {
            "rr": 3.0,
            "tp1_ratio": 0.6,
            "stop_atr_pct": 6.0,
            "entry_end": "12:00",
            "gate_name": "hh_hl_2_vwap",
        }
    )

    for rr in (2.5, 3.0):
        for tp1_ratio in (0.5, 0.6):
            for stop_atr_pct in (5.0, 6.0, 7.0):
                for entry_end in ("11:30", "12:00", "12:30"):
...
def build_candidate_configs(base: StrategyConfig) -> list[tuple[StrategyConfig, str]]:
    ...
    for spec in build_candidate_specs():
        cfg = with_overrides(
            ...
            ny_stop_atr_pct=float(spec["stop_atr_pct"]),
        ...
        cfg = replace(
            cfg,
            name=(
                f"NQ Bull V1 rr{cfg.rr:.2f}_tp1{cfg.tp1_ratio:.2f}_"
                f"stop{cfg.sessions[0].stop_atr_pct:.1f}_"
                f"end{cfg.sessions[0].entry_end.replace(':', '')}_{gate_label}"
            ),
            notes=f"Bull V1 candidate with {gate_label} context filter.",
        )
...
def record_to_row(record: dict) -> dict:
    ...
    return {
        ...
        "stop_atr_pct": record["config"].sessions[0].stop_atr_pct,
...
def write_summary(
    ...
    holdout = year_windows["holdout_2025_latest"]
    status = (
        "true bull specialist"
        if year_windows["survives_bull_v1"] and specialist["survives_round1"]
        else "still too generalist"
    )
    lines = [
        "# NQ Bull Specialist V1",
        "",
        "## Outcome",
        "",
        ...
        "- `2022-2023`: rejection window",
        "- `2024+`: acceptance window",
        "- `2025+`: holdout-quality funded-account check",
        "",
        "## Selected Candidate",
        "",
        f"- Acceptance net R: `{year_windows['acceptance_net_r']}`",
        f"- Acceptance trades: `{year_windows['acceptance_2024_latest']['total_trades']}`",
        f"- Rejection net R: `{year_windows['rejection_net_r']}`",
        f"- Rejection share of acceptance: `{year_windows['rejection_share_of_acceptance']}`",
        f"- Holdout payout rate: `{holdout['payout_rate']}`",
        f"- Holdout breach rate: `{holdout['breach_rate']}`",
        f"- Holdout average days to payout: `{holdout['average_days_to_payout']}`",
        f"- Round-1 specialization ratio: `{specialist['specialization_ratio']}`",
        "",
        "## Interpretation",
        "",
    ]
    if status == "true bull specialist":
        lines.extend(
            [
                "- The selected route cleared both the bull-market window test and the original specialist readout.",
                "- It is strong in the 2024+ acceptance era and meaningfully weaker in the 2022-2023 rejection window.",
            ]
        )
    else:
        lines.extend(
            [
                "- The selected route improved the bull-window profile, but it still does not fully satisfy the stricter specialist intent.",
                "- Treat it as an interim bull-biased route rather than a finished regime-specialist deployment.",
            ]
        )
...
def main() -> None:
    ...
    ranked = sorted(records, key=lambda record: bull_market_rank_key(record["year_windows"]), reverse=True)
    ...
    for record in ranked:
        ...
        holdout = score["holdout_2025_latest"]
        year_window_rows.append(
            {
                "config_name": record["config"].name,
                "gate_name": record["gate_name"],
                "diagnostic_2021_trades": score["diagnostic_2021"]["total_trades"],
                "rejection_2022_2023_trades": score["rejection_2022_2023"]["total_trades"],
                "rejection_2022_2023_net_r": score["rejection_net_r"],
                "acceptance_2024_latest_trades": score["acceptance_2024_latest"]["total_trades"],
                "acceptance_2024_latest_net_r": score["acceptance_net_r"],
                "holdout_payout_rate": holdout["payout_rate"],
                "holdout_breach_rate": holdout["breach_rate"],
                "holdout_average_days_to_payout": holdout["average_days_to_payout"],
                "acceptance_rejection_separation": score["acceptance_rejection_separation"],
                "survives_bull_v1": score["survives_bull_v1"],
            }
        )
