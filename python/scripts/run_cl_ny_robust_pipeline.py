#!/usr/bin/env python3
"""CL NY Continuation Both — 5-Phase Robust Pipeline.

Final anchor (from R4 convergence region):
  stop=1.0%, rr=6.0, gap=1.0%, tp1=0.60
  ORB 10m (09:30-09:40), flat 15:50-16:00, ATR 10
  Both directions, 1s bar magnifier
  In-sample: Calmar 38.01, Sharpe 2.983, DD -23.8R, 0 neg years, 905.7 Net R

Note: R4 grid winner was even more extreme (stop=0.50%, rr=7.0, Calmar 64).
Using the conservative center of the convergence region for robustness.
Walk-forward will sweep wider ranges to find the best OOS params.
...
# -- Walk-forward param ranges (wider to test robustness) ----------------------

PARAM_RANGES = {
    "ny_stop_atr_pct": [0.75, 1.0, 1.25, 1.5],
    "rr": [5.0, 5.5, 6.0, 6.5, 7.0],
    "ny_min_gap_atr_pct": [0.75, 1.0, 1.25],
    "tp1_ratio": [0.5, 0.6, 0.7],
}
# 4 x 5 x 3 x 3 = 180 combos per fold
...
def phase_1(df, df_1m, df_1s):
    ...
    checks = {
        ...
        "Win rate > 25%": m["win_rate"] > 0.25,  # CL at high RR has lower WR
...
def phase_2(df, df_1m, df_1s):
    ...
    # NOTE: WF uses 1m magnifier only (not 1s). The 1s maps are ~74M entries
...
if __name__ == "__main__":
    print(flush=True)
    print("=" * 70, flush=True)
    print("  CL NY CONTINUATION BOTH — 5-PHASE ROBUST PIPELINE", flush=True)
    print("  Anchor: stop=1.0% | rr=6.0 | gap=1.0% | tp1=0.6", flush=True)
    print("  Structural: ORB 10m | entry 14:00 | flat 15:50 | ATR 10 | 1s", flush=True)
    print("=" * 70, flush=True)
    ...
    print(f"  Anchor: stop=1.0% | rr=6.0 | gap=1.0% | tp1=0.6", flush=True)
    print(f"  Structural: ORB 10m | entry 14:00 | flat 15:50 | ATR 10 | 1s", flush=True)
    print(flush=True)
