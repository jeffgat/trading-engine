...
export function useStarredHistory(): UseStarredReturn {
  ...
  const refreshHistory = useCallback(async () => {
    ...
    try {
      const res = await fetch("/bt-api/starred");
      if (res.ok) {
  ...
  const unstarBacktest = useCallback(async (id: string) => {
    try {
      await fetch(`/bt-api/backtests/${id}/star`, { method: "POST" });
  ...
  const bulkUnstarBacktests = useCallback(async (ids: string[]) => {
    ...
    if (toUnstar.length > 0) {
      await Promise.all(toUnstar.map((id) => fetch(`/bt-api/backtests/${id}/star`, { method: "POST" })));
      await refreshHistory();
    }
