#!/usr/bin/env python3
"""NQ Asia LSI FAST — 5-Phase Robust Pipeline (Bailey-aware).

FAST execution config NQ_Asia_LSI leg:
  ORB: 20:00-20:05, entry 20:40-23:30, flat 04:00-07:00
...
# ── Candidate config (FAST execution NQ_Asia_LSI) ──────────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ Asia LSI FAST Robust Pipeline",
)
...
def phase_4(df, df_1m, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST NQ_Asia_LSI robust pipeline",
    )
    if entry.warning:
...
def main():
    ...
    print(f"  Structural: LSI strategy, entry 20:40-23:30, flat 04:00, ATR={CANDIDATE.atr_length}, "
