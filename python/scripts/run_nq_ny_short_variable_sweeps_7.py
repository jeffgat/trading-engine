#!/usr/bin/env python3
"""NQ NY Continuation Short — Variable Sweeps Round 7.

R7 anchor (from R6 adoptions: orb=25m, atr=14, end=15:00, rr=1.5, gap=2.0%, excl Tue, maxgap=25pts):
  ORB: 09:30-09:55 (25m), entry until 15:00, flat 15:50-16:00
  stop=5.0%, min_gap_atr=2.0%, max_gap_pts=25, max_gap_atr=0
  rr=1.5, tp1=0.2, ATR=14, direction=short, ICF=OFF, continuation, 1s magnifier
  DOW gate: excl Tue
...
def main():
    ...
    print(f"maxgap={ANCHOR_SESSION.max_gap_points}pts, MIN_TP1_RATIO={MIN_TP1_RATIO}")
    ...
    for i, s in enumerate(stop_vals, 1):
        ...
        if mt < 10:
            print(f"    {i:>3} {'stop=' + str(s) + '%':>24}  SKIP (median stop {mt:.1f} ticks < 10)")
            continue
        print_row(i, f"stop={s}%", m, is_base=(abs(s - ANCHOR_SESSION.stop_atr_pct) < 0.01))
        if m["calmar_ratio"] > best_cal:
            best_cal, best_lbl, best_m, best_t = m["calmar_ratio"], f"stop={s}%", m, trades_s
    ...
    # -- 9. MIN GAP ATR % -----------------------------------------------------
    gap_vals = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0]
    ...
    # -- 11. MAX GAP POINTS ---------------------------------------------------
    maxgap_vals = [0, 15, 25, 50, 75, 100, 150]
