#!/usr/bin/env python3
"""NQ Asia Config C — Five-phase robust pipeline (Calmar WF objective).

Candidate: stop=3.9%, gap=0.75%, maxgap=5.0%, rr=2.0, tp1=0.35
           ORB 10m, ATR 14, be=0, entry≤22:30, both dirs, no-Thursday

WF objective: Calmar (was Sharpe).
Rationale: Sharpe rewarded high-RR params in trending IS periods (2019-2022),
which then collapsed in 2022 OOS (-40.7R). Calmar penalises drawdown directly
and should select more conservative params that survive regime shifts.
...
def main():
    ...
    with open("/tmp/wf_nq_asia_c_calmar.pkl", "wb") as f:
    ...
    db_name = f"NQ ASIA 2015-2026 Config C BE0 Pipeline Calmar {verdict}"
    db_notes = (
        f"Config C robust pipeline, Calmar WF objective. be=0 (user preference). "
