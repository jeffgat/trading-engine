#!/usr/bin/env python3
"""Step 2 — Variable Sweeps Round 4: GC NY Continuation Longs.

Anchor (from R3 adoptions):
  stop=3.0%, rr=4.0, tp1=0.7, ATR 30, min_gap=5.0%, max_gap_atr=25%
  ICF=True, 8m ORB (09:30-09:38), entry→12:00, flat 15:50, long-only, FOMC excluded

Stop sweep floor: 3.0% (never test <10 tick stops).
Note: 3.0% is borderline with ATR 30 in low-vol periods — will check median ticks.
"""
...
GC_NY = SessionConfig(
    name="NY",
    orb_start="09:30",
    orb_end="09:38",       # 8m ORB (adopted R3)
    entry_start="09:38",
    entry_end="12:00",
    flat_start="15:50",
    flat_end="16:00",
    stop_atr_pct=3.0,       # adopted R3 (borderline 10-tick)
    min_gap_atr_pct=5.0,    # adopted R3
)

ANCHOR = StrategyConfig(
    rr=4.0,                  # adopted R3
    tp1_ratio=0.7,           # adopted R3
    risk_usd=5000.0,
    atr_length=30,           # adopted R3
    min_qty=1.0,
    qty_step=1.0,
    sessions=(GC_NY,),
    instrument=GC,
...
def sweep_stop_atr():
    values = [3.0, 4.0, 5.0, 6.0, 7.5, 8.0, 9.0, 10.0, 12.0, 15.0]
    results = []
    for v in values:
...
def sweep_atr_length():
    values = [5, 7, 10, 14, 16, 18, 20, 25, 30, 35, 40, 50]
    results = []
    for v in values:
...
def sweep_rr():
    values = [2.0, 2.5, 3.0, 3.5, 4.0, 4.5, 5.0, 6.0, 7.0]
    results = []
    for v in values:
...
def sweep_min_gap_atr():
    values = [1.0, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0, 6.0, 7.0, 8.0]
    results = []
    for v in values:
...
if __name__ == "__main__":
    print("\n" + "=" * 90)
    print("  GC NY CONTINUATION LONGS — VARIABLE SWEEPS ROUND 4")
    print("  Anchor: stop=3.0%, rr=4.0, tp1=0.7, ATR 30, gap=5.0%, max_gap_atr=25%")
    ...
    print("\n" + "=" * 90)
    print("  ROUND 4 SUMMARY — Adoption Decisions")
    ...
    if adoptions:
        ...
        print(f"\n  → Anchor changed. Must re-sweep all 12 dimensions (Round 5).")
    else:
