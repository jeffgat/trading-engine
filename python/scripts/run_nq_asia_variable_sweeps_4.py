#!/usr/bin/env python3
"""NQ Asia ORB — Variable sweeps round 4: reset to best-proven config.

R3 showed that the 5 R2 adoptions interacted destructively (Calmar 20.18 → 11.97).
The best config found across R1-R3 was ORB=10m + entry_end=01:00 + flat=00:00
with original v3 continuous params → Calmar 23.85.

R4 anchor (proven best from R1 §4 and R2 §2):
...
ANCHOR_SESSION = SessionConfig(
    name="Asia",
    orb_start="20:00",
    orb_end="20:10",         # 10m ORB (confirmed best at entry_end=01:00)
    entry_start="20:10",
    entry_end="01:00",       # R1 adopted
    flat_start="00:00",      # original v3
    flat_end="07:00",
    stop_atr_pct=3.7,
    min_gap_atr_pct=0.90,    # original v3
    max_gap_points=0.0,
    max_gap_atr_pct=5.0,
)
...
ANCHOR = StrategyConfig(
    ...
    rr=1.75,
    tp1_ratio=0.35,          # original v3
    atr_length=5,
    impulse_close_filter=False,  # original v3
    name="NQ Asia R4 Anchor",
)
...
def main():
    print("NQ Asia ORB — Round 4: reset to best-proven config (Calmar 23.85)")
    print("=" * 90)
