#!/usr/bin/env python3
"""NQ Asia ORB — Variable sweeps round 9 (R5): post bug fix, updated anchor.

R4 (sweeps_8) adopted 3 changes:
  - entry_end: 00:00 -> 01:00 (Δ+4.14)
  - max_gap_atr_pct: 5.0 -> 7.0 (Δ+2.91)
  - dow_exclusion: excl Thu -> none (Δ+13.12)

WARNING: 3 simultaneous changes. Watch for destructive interaction.

R5 anchor:
  ORB: 20:00-20:10 (10m), entry until 01:00, flat 01:00
  stop=3.7%, min_gap_atr=0.90%, max_gap_atr=7.0%
  rr=1.75, tp1=0.10, ATR=5, direction=both, ICF=OFF, continuation, 1s magnifier
  DOW gate: none
...
ANCHOR_SESSION = SessionConfig(
    ...
    orb_end="20:10",
    entry_start="20:10",
    entry_end="01:00",       # R4 adopted (was 00:00)
    flat_start="01:00",
    flat_end="07:00",
    stop_atr_pct=3.7,
    min_gap_atr_pct=0.90,
    max_gap_points=0.0,
    max_gap_atr_pct=7.0,    # R4 adopted (was 5.0)
)
...
ANCHOR = StrategyConfig(
    ...
    name="NQ Asia R5-R5 Anchor",
)

ANCHOR_DOW_EXCL = set()  # R4 adopted: no DOW exclusion
...
def main():
    print("NQ Asia ORB — Round 9 (R5): entry_end=01:00, maxgap=7.0, no DOW gate")
    print("=" * 90)
    print(f"Anchor: rr={ANCHOR.rr}, tp1={ANCHOR.tp1_ratio}, "
          f"stop={ANCHOR_SESSION.stop_atr_pct}%, gap={ANCHOR_SESSION.min_gap_atr_pct}%")
    print(f"ORB=10m (20:00-20:10), entry<=01:00, flat=01:00, ATR={ANCHOR.atr_length}, "
          f"dir={ANCHOR.direction_filter}, no DOW gate, ICF=OFF")
    ...
    print_row(0, "ANCHOR (no gate)", m_anchor, is_base=True)
    ...
    # ── 4. ENTRY END TIME ────────────────────────────────────────────
    entry_ends = ["23:00", "00:00", "00:30", "01:00", "01:30", "02:00", "03:00"]
    ...
    # ── 5. FLAT START ────────────────────────────────────────────────
    flat_starts = ["00:00", "00:30", "01:00", "01:30", "02:00", "03:00"]
    print_header("5. FLAT START (anchor=01:00)")
    for i, fs in enumerate(flat_starts, 1):
        sess = replace(ANCHOR_SESSION, flat_start=fs)
        config = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"flat={fs}", m, is_base=(fs == "01:00"))
    ...
    # ── 7. MAX GAP ATR % ────────────────────────────────────────────
    maxgap_values = [3.0, 5.0, 7.0, 9.0, 11.0, 0.0]
    print_header("7. MAX GAP ATR % (anchor=7.0%, 0=OFF)")
    for i, mg in enumerate(maxgap_values, 1):
        ...
        print_row(i, label, m, is_base=(abs(mg - 7.0) < 0.01))
    ...
    # ── 10. TP1 RATIO ────────────────────────────────────────────────
    tp1_values = [0.05, 0.08, 0.10, 0.12, 0.15, 0.20, 0.25]
    print_header("10. TP1 RATIO (anchor=0.10)")
    for i, tp1 in enumerate(tp1_values, 1):
        config = replace(ANCHOR, tp1_ratio=tp1)
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"tp1={tp1}", m, is_base=(abs(tp1 - 0.10) < 0.01))
    ...
    print_header("11. DOW EXCLUSION (anchor=none)")
