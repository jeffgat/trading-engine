#!/usr/bin/env python3
"""GC continuation shorts — variable sweeps R1.

Starting anchor from diagnostic compound:
  stop=3.0%, rr=7.0, gap=5.0%, tp1=0.5, ATR 10, 15m ORB, entry→15:00
...
10-tick minimum stop: WAIVED for GC shorts (user decision).
"""
...
def make_config(
    ...
    atr_length=ANCHOR_ATR,
    orb_end=ANCHOR_ORB_END,
    entry_end=ANCHOR_ENTRY_END,
    flat_start=ANCHOR_FLAT,
    impulse_close_filter=ANCHOR_ICF,
):
...
def neg_year_set(trades):
    """Return set of full calendar years with negative R (exclude current partial year)."""
...
def stats(trades):
    ...
    return {
        ...
        "neg_years": len(neg_yrs),
        "neg_year_set": neg_yrs,
        "med_stop": med_stop,
...
print("=" * 130)
print("VARIABLE SWEEPS R1: GC NY Continuation Shorts")
print(f"Anchor: stop={ANCHOR_STOP}%, rr={ANCHOR_RR}, gap={ANCHOR_GAP}%, tp1={ANCHOR_TP1}, "
      f"ATR {ANCHOR_ATR}, 15m ORB, entry→{ANCHOR_ENTRY_END}")
print("=" * 130)
...
for stop in [1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0, 7.5, 10.0, 12.0, 15.0]:
...
for d in ["both", "long", "short"]:
    cfg = make_config()
    # Override direction — need to rebuild
    session = SessionConfig(
...
for gap in [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0, 5.5, 6.0, 7.0, 8.0]:
...
for mga in [0.0, 15.0, 20.0, 25.0, 30.0, 50.0, 75.0, 100.0]:
...
all_bests = [
    ...
    ("7. R:R", best_7),
    ("8. TP1 Ratio", best_8),
    ("9. Min Gap ATR %", best_9),
    ("10. DOW Exclusion", best_10),
    ("11. Max Gap ATR %", best_11),
    ("12. ICF", best_12),
]
...
for dim_name, (val, s) in all_bests:
    ...
    adopt = delta > 0.3 and not has_new_neg and s["trades"] > 100
...
if adoptions:
    ...
    print("\n  >>> Anchor changed. Re-sweep all 12 dimensions in R2.")
else:
    print("\n  >>> No adoptions. Anchor converged. Ready for grid sweep.")
