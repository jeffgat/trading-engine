#!/usr/bin/env python3
"""ES LDN Continuation Both — Variable Re-Sweep #2 (1s magnifier).

Anchor changed from fine-tune grid. All variable sweeps must be rerun.

New anchor (from 1,960-combo fine-tune, best clean config >= 3% stop):
  stop=5.0%, rr=2.0, gap=1.25%, tp1=0.40
  Calmar 10.39, Sharpe 1.223, Net R 149.7, DD -14.4R, 0 neg years

Structural config locked from sweep_1:
...
# -- Anchor config (from fine-tune grid, best clean config >= 3% stop) ---------
...
ES_LDN_ANCHOR = SessionConfig(
    ...
    entry_start="03:10",
    entry_end="08:25",
    flat_start="07:30",     # sweep_1 winner
    flat_end="08:25",
...
def sweep_flat_start():
    ...
    header()
    times = ["07:00", "07:15", "07:30", "07:45", "08:00", "08:10", "08:20"]
    results = []
    for t in times:
        label = f"flat_start={t}" + (" [anchor]" if t == "07:30" else "")
...
def print_summary(m):
    section("ANCHOR METRICS (fine-tune winner, 1s magnifier)")
    print(f"  rr=2.0 | tp1=0.4 | stop=5.0% | gap=1.25% | ORB 10m | ATR 50 | both dir | 1s", flush=True)
    print(flush=True)
...
if __name__ == "__main__":
    print(flush=True)
    print("=" * 70, flush=True)
    print("  ES LDN CONTINUATION BOTH — VARIABLE RE-SWEEP #2 (1s magnifier)", flush=True)
    print("  New anchor: rr=2.0 | tp1=0.4 | stop=5.0% | gap=1.25% | 10m ORB", flush=True)
    print("  Structural: ORB 10m | flat 07:30 | ATR 50 | both dir | 1s", flush=True)
    print("  From fine-tune: Calmar 10.39, 0 neg years, DD -14.4R", flush=True)
    print("=" * 70, flush=True)
    ...
    print("  Note: rr, tp1, stop, gap are NOT swept here — they were just", flush=True)
    print("  fine-tuned in the 1,960-combo grid. Only structural variables", flush=True)
    print("  are re-swept on the new anchor.", flush=True)
