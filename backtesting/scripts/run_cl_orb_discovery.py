#!/usr/bin/env python3
"""CL ORB Discovery — 3-session sweep with 1m magnifier.

Starting thesis: ORB continuation on CL (Crude Oil) across NY, Asia, LDN sessions.
Baseline: RR=2.0, TP1=0.5, stop=10% ATR, min_gap=1% ATR.
Sweeps ATR stops, ORB-range stops, both directions, multiple ORB windows.

Pre-holdout only (<2025-01). Uses 1m magnifier (no 1s data for CL).

CL learnings note: stop_atr_pct < 7% produces sub-tick stops on CL (ATR ~$1.50,
min_tick $0.01). The engine clamps to 10-tick floor, but configs at 5% ATR will
hit this floor. ATR stops at 7.5%+ are clean. ORB-range stops are unaffected.

Prior CL testing: NY Continuation (INVALIDATED by sub-tick artifact), VWAP (NO-GO),
LSI (DEFINITIVE NO-GO), Reversal (NO-GO). Asia + LDN never tested.
"""
...
OUTPUT_DIR = ROOT / "data" / "results" / "cl_orb_discovery_3session"
...
def _ny_session(orb_minutes: int, stop_mode: str) -> SessionConfig:
    if orb_minutes <= 30:
        orb_end = f"09:{30 + orb_minutes:02d}"
    else:
        orb_end = f"10:{orb_minutes - 30:02d}"
    return SessionConfig(
        name="NY", orb_start="09:30", orb_end=orb_end,
        entry_start=orb_end, entry_end="13:00",
    ...
    for orb_min in ORB_MINUTES:
        for stop_mode in ["atr", "orb"]:
            ...
            base = StrategyConfig(
                ...
                tp1_ratio=0.5,
                atr_length=14,
                name=f"CL {session_name} ORB{orb_min}m {stop_mode}-stop",
            )
...
def compute_period_metrics(trades):
    ...
    l2 = compute_metrics([t for t in trades if "2022-06-01" <= t.date < HOLDOUT_START])
    l1 = compute_metrics([t for t in trades if "2023-06-01" <= t.date < HOLDOUT_START])
...
def run_session(name, df_5m, df_1m):
    print(f"\n{'=' * 70}")
    ...
    print(f"  Sweep done: {len(results)} results [{elapsed:.0f}s, {elapsed / max(len(results), 1):.1f}s/config]")
    ...
    for config, trades in results:
        ...
        m = compute_period_metrics(trades)
        s = rank_score(m)
        ...
        rows.append({
            **desc,
            ...
            "l2_calmar": round(float(m["last_2yr"].get("calmar_ratio", 0) or 0), 4),
            "l1_calmar": round(float(m["last_1yr"].get("calmar_ratio", 0) or 0), 4),
            "l2_net_r": round(float(m["last_2yr"].get("total_r", 0)), 2),
            "l1_net_r": round(float(m["last_1yr"].get("total_r", 0)), 2),
            "score": round(s, 4),
    ...
    print(f"  {'#':>3s} {'ORB':>4s} {'SMode':>5s} {'SVal':>5s} {'RR':>4s} {'TP1':>4s} {'Dir':>5s} | {'Tr':>5s} {'NetR':>7s} {'Cal':>6s} {'Shp':>5s} {'DD':>6s} {'WR':>5s} {'L2C':>5s} {'L1C':>5s} {'Neg':>3s} {'Scr':>5s}")
    print(f"  {'-' * 100}")
    for i, r in enumerate(rows[:25]):
        print(
            f"  {i+1:3d} {r['orb_min']:3d}m {r['stop_mode']:>5s} {r['stop_val']:5.0f} "
            f"{r['rr']:4.1f} {r['tp1']:4.1f} {r['dir']:>5s} | "
            f"{r['trades']:5d} {r['net_r']:+7.1f} {r['calmar']:6.2f} "
...
def main():
    print("CL ORB Discovery — 3-Session Sweep (1m magnifier)")
    print("=" * 70)
    print(f"Workers: {N_WORKERS} | Pre-holdout only (<{HOLDOUT_START})")
    print(f"ORB windows: {ORB_MINUTES}m | Stop modes: ATR + ORB")
    print(f"RR sweep: 2.0/2.5/3.0/3.5 | TP1 sweep: 0.3/0.4/0.5/0.6")
    print(f"Directions: both/long/short | Gap: 1.0% ATR")
    print(f"Bar magnifier: ON (1m) — no 1s data for CL")
    print(f"NOTE: ATR stops at 5% will hit 10-tick floor on CL. 7.5%+ are clean.")
    ...
    for name in ["NY", "Asia", "LDN"]:
        rows = run_session(name, df_5m, df_1m)
