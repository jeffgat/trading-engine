#!/usr/bin/env python3
"""ES LDN Continuation Both — Variable Sweeps Round 3 (Full Optimization Step 2).

Updated anchor from R2 adoptions:
  ATR=20 (was 3), gap=1.0% (was 1.5)
  Stop=10.0% (unchanged), rr=2.5, tp1=0.5, ORB 15m, flat 08:20, both, ICF off.
...
ANCHOR_SESSION = SessionConfig(
    ...
    flat_end="08:25",
    stop_atr_pct=10.0,
    min_gap_atr_pct=1.0,    # R2: 1.5 → 1.0
    max_gap_points=50.0,
)
...
if __name__ == "__main__":
    print("=" * 90)
    print("  ES LDN CONTINUATION BOTH — VARIABLE SWEEPS ROUND 3")
    print("=" * 90)
    ...
    print(f"  R2 adoptions: ATR 3→20, gap 1.5→1.0%")
    print()
    ...
    # Summary
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY — Variable Sweeps Round 3")
    ...
    if adoptions:
        print(f"\n  >>> {len(adoptions)} adoption(s) — update anchor and re-sweep (round 4)")
    else:
