#!/usr/bin/env python3
"""Robust pipeline: GC NY Continuation Longs — R1 config (clean data, 1s magnifier).

Re-derived on clean GC.v.0 data (2026-02-21).
Config confirmed stable after R1 variable sweeps + grid sweep:
...
# ── Base config (R1 anchor — confirmed on clean data) ────────────────────────
...
# ── WF sweep params (centred on grid results) ────────────────────────────────
...
def main():
    print()
    print("=" * 70)
    print("  ROBUST PIPELINE: GC NY Cont Longs R1 (clean data, 1s magnifier)")
