#!/usr/bin/env python3
"""NQ LDN Continuation — Variable Sweeps Round 2.

R1 adoptions (2 dimensions):
  max_gap_points: 50 → 20   (Calmar 0.09 → 0.41, Δ=+0.32, no new neg)
  ICF: OFF → ON             (Calmar 0.09 → 0.73, Δ=+0.64, no new neg)
...
ANCHOR_SESSION = SessionConfig(
    ...
    stop_atr_pct=10.0,
    min_gap_atr_pct=1.0,
    max_gap_points=20.0,     # R1 adopted (was 50)
    max_gap_atr_pct=0.0,
)
...
ANCHOR = StrategyConfig(
    ...
    tp1_ratio=0.5,
    atr_length=14,
    impulse_close_filter=True,   # R1 adopted (was False)
    name="NQ LDN R2 Anchor",
)
...
def main():
    print("NQ LDN ORB — Round 2: max_gap=20 + ICF=ON adopted from R1")
    print("=" * 90)
