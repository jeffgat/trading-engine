#!/usr/bin/env python3
"""NQ NY Continuation — Variable Sweeps Round 3 (Longs-Only Fresh Start).

R3 anchor (R2 + 5 adoptions):
  ORB: 09:30-09:50 (20m), entry until 14:00, flat 15:00-16:00
  stop=7.0%, min_gap_atr=2.5%, max_gap_pts=100.0, max_gap_atr=0.0
  rr=3.0, tp1=0.5, ATR=20, direction=long, ICF=ON, continuation, 1s magnifier
  DOW gate: excl Fri

R2 adoptions applied (5 — watch for destructive interaction):
  atr_length: 12 -> 20 (+2.51)
  entry_end: 12:00 -> 14:00 (+1.39)
  flat_start: 15:50 -> 15:00 (+0.45)
  icf: OFF -> ON (+1.76)
  stop_atr_pct: 9.0 -> 7.0 (+5.14)
...
def main():
    ...
    print(f"\nWATCH: 5 adoptions applied from R2 — monitoring for destructive interaction")
    ...
    # Check for destructive interaction from R2's 5 adoptions
    # R2 anchor was Calmar 9.35. If R3 anchor is significantly lower, flag it.
    r2_calmar = 9.35
    if anc_cal < r2_calmar * 0.7:
        print(f"\n  *** WARNING: DESTRUCTIVE INTERACTION DETECTED ***")
        print(f"  R2 anchor Calmar was {r2_calmar:.2f}, R3 anchor is {anc_cal:.2f} ({(anc_cal/r2_calmar - 1)*100:+.0f}%)")
        print(f"  Consider reverting to R2 anchor and applying adoptions one at a time.")
    ...
    # -- 3. ENTRY END TIME -----------------------------------------------------
    entry_ends = ["11:00", "11:30", "12:00", "12:30", "13:00", "13:30", "14:00", "14:30", "15:00", "15:30"]
    ...
    # -- 5. MAX GAP POINTS ----------------------------------------------------
    maxgap_vals = [50, 75, 100, 150, 9999]
    print_header(f"5. MAX GAP POINTS (anchor={ANCHOR_SESSION.max_gap_points}, 9999=disabled)")
    best_cal, best_lbl, best_m = anc_cal, "anchor", m_anc
    ...
    # -- 6. ICF (Impulse Close Filter) ----------------------------------------
    print_header(f"6. ICF (anchor={'ON' if ANCHOR.impulse_close_filter else 'OFF'})")
    best_cal, best_lbl, best_m = anc_cal, "anchor", m_anc
    ...
    # -- 7. DOW EXCLUSION (post-filter on raw trades) -------------------------
    dow_sets = [
        ("none",      set()),
        ("excl Mon",  {0}),
        ("excl Tue",  {1}),
    ...
    print_header(f"7. DOW EXCLUSION (anchor={ANCHOR_DOW_EXCL or 'none'})")
    best_cal, best_lbl, best_m = anc_cal, "anchor", m_anc
    ...
    # -- 10. STOP ATR % -------------------------------------------------------
    stop_vals = [4, 5, 6, 7, 8, 9, 10, 12]
    print_header(f"10. STOP ATR % (anchor={ANCHOR_SESSION.stop_atr_pct}%)")
    best_cal, best_lbl, best_m = anc_cal, "anchor", m_anc
    ...
    print_header(f"11. MIN GAP ATR % (anchor={ANCHOR_SESSION.min_gap_atr_pct}%)")
    best_cal, best_lbl, best_m = anc_cal, "anchor", m_anc
    ...
    # -- 12. DIRECTION ---------------------------------------------------------
    print_header(f"12. DIRECTION (anchor={ANCHOR.direction_filter})")
    best_cal, best_lbl, best_m = anc_cal, "anchor", m_anc
