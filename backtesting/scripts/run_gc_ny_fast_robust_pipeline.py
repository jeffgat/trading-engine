#!/usr/bin/env python3
"""GC NY FAST — 5-Phase Robust Pipeline (Bailey-aware).

FAST execution config GC_NY leg:
  ORB: 09:30-09:40, entry until 12:00, flat 13:30-16:00
  stop=4.5% ATR-7, min_gap=3.0%, rr=9.0, tp1=0.35
  Long only, ICF on, FOMC excluded, Friday excluded
...
# ── Candidate config (FAST execution GC_NY) ──────────────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="GC NY FAST Robust Pipeline",
)

# ── Walk-forward param ranges (centered on candidate) ────────────────────
PARAM_RANGES = {
    "ny_stop_atr_pct": [3.0, 3.5, 4.0, 4.5, 5.0],
    "rr": [7.0, 8.0, 9.0, 10.0, 11.0],
    "ny_min_gap_atr_pct": [2.0, 2.5, 3.0, 3.5, 4.0],
    "tp1_ratio": [0.25, 0.3, 0.35, 0.4],
}
...
def phase_4(df, df_1m, df_1s, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST GC_NY robust pipeline",
    )
    if entry.warning:
...
def main():
    ...
    print(f"  Structural: ORB 10m, entry<12:00, flat 13:30, ATR={CANDIDATE.atr_length}, "
          f"dir={CANDIDATE.direction_filter}, ICF=on, FOMC excl, DOW excl Fri")
