...
def main():
    ...
    for i, (stop, rr, gap, tp1) in enumerate(GRID):
        ...
        # NOTE: If anchor uses stop_orb_pct ({STOP_ORB} > 0), change to stop_orb_pct=stop
        # See "ORB%-Based Stop Grid" in Optional Extensions.
...
| `{ANCHOR_STOP}` | 3.0 | Anchor stop value (stop_atr_pct if ATR-based, stop_orb_pct if ORB%-based) |
| `{STOP_ORB}` | 0.0 | Anchor stop_orb_pct value (0=off) |
...
| `{STOP_GRID}` | [2.0, 2.5, 3.0, 3.5, 4.0] | Stop values to sweep (ATR% or ORB% depending on anchor method) |
...
| `stop_orb_pct` | 4-6 values, +/-25 around anchor | e.g., anchor=100 -> [50, 75, 100, 125, 150] (use when anchor has ORB%-based stops) |
...
1. **Stop method awareness**: Check `{STOP_ORB}`. If `> 0`, the anchor uses ORB%-based stops — replace
   `stop_atr_pct=stop` with `stop_orb_pct=stop` in the grid loop, fill `{STOP_GRID}` with ORB% values
   (e.g., `[50, 75, 100, 125, 150]`), and set `{ANCHOR_STOP}` to the `stop_orb_pct` value. See the
   "ORB%-Based Stop Grid" optional extension for the code pattern.

2. **Docstring and print labels**: When the anchor uses ORB%-based stops, update the docstring anchor
   line and `print(f"Anchor: ...")` to show `stop_orb=X%` instead of `stop=X%`.

3. **Grid sizing**: Use the `stop_orb_pct` row in Grid Sizing Guidelines when the anchor uses ORB%.
...
When the anchor uses `stop_orb_pct` (ORB-range-based stops), replace the `stop_atr_pct` axis with `stop_orb_pct`:

```python
# Replace STOPS and the grid loop session replacement
STOPS = [75, 100, 125, 150, 175]  # ORB% values centered on anchor

# Inside the grid loop:
for i, (stop, rr, gap, tp1) in enumerate(GRID):
    sess = replace(ANCHOR_SESSION, stop_orb_pct=stop, min_gap_atr_pct=gap)
    # ... rest unchanged
```

Update the header, printout columns, and anchor-matching logic to use `stop_orb_pct` instead of `stop_atr_pct`. The `{STOP_GRID}` placeholder should contain ORB% values (e.g., `[75, 100, 125, 150, 175]`) when the anchor uses ORB%-based stops.
