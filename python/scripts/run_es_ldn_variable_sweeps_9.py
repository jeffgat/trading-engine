#!/usr/bin/env python3
"""ES LDN Continuation Both — Variable Sweeps Round 9.

R8 adoption: ATR 7→10 (Calmar 6.85, 0 neg years).
Anchor: stop=5.0%, rr=3.0, gap=1.0%, tp1=0.6, ATR=10
...
def sweep_atr(df, df_1m, df_1s):
    ...
    for v in [3, 5, 7, 10, 14, 20, 30, 50]:
        ...
        results.append((v, m))
    print_sweep_table(results, "ATR Length", 10); return results
...
if __name__ == "__main__":
    print("=" * 90)
    print("  ES LDN CONTINUATION BOTH — VARIABLE SWEEPS ROUND 9")
    print("=" * 90)
    print(f"  Anchor: stop=5%, rr=3.0, gap=1.0%, tp1=0.6, ATR=10, ORB 10m")
    print(f"  Flat 08:20, entry 08:25, both, DOW excl Mon, ICF off, 1s mag")
    print(f"  R8 adoption: ATR 7→10\n")
    ...
    sweeps = [
        ...
        ("ATR Length", 10, sweep_atr), ("Entry End", "08:25", sweep_entry_end),
    ...
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY — Variable Sweeps Round 9 ({total_time:.0f}s)")
    print(f"{'='*90}")
    ...
    if not adoptions:
        ...
        print(f"\n  Final anchor: stop=5%, rr=3.0, gap=1.0%, tp1=0.6, ATR=10")
