#!/usr/bin/env python3
"""Walk-forward ranking for shortlisted ORB indicator-confluence overlays.

Purpose:
- keep the overlay shortlist tiny and frozen
- select among overlays on each IS fold
- evaluate transfer to OOS across the combined anchor basket

This remains a post-trade overlay study, but it is a stronger Bailey-style
check than the earlier structural/validation snapshots because it explicitly
tests transfer across rolling unseen windows.
"""
...
OUTPUT_DIR = ROOT / "data" / "results" / "orb_indicator_confluence_walkforward"
REPORT_PATH = ROOT / "learnings" / "reports" / "ORB_INDICATOR_CONFLUENCE_WALKFORWARD.md"
...
CANDIDATES = [
    Candidate("sma20__aligned_0_10", "SMA20 aligned [0,10%)", ("sma20",), 0.0, 0.10),
    Candidate("ema20__aligned_0_10", "EMA20 aligned [0,10%)", ("ema20",), 0.0, 0.10),
    Candidate("sma20__aligned_0_20", "SMA20 aligned [0,20%)", ("sma20",), 0.0, 0.20),
    Candidate("vwap_ema20__aligned_0_10", "VWAP+EMA20 aligned [0,10%)", ("vwap", "ema20"), 0.0, 0.10),
]
...
def metrics_for_period(trades: list[TradeResult], start: str, end: str) -> dict:
...
def main() -> None:
    ...
    print("ORB indicator confluence walk-forward")
    print("=" * 70)
    ...
    anchor_base_trades: dict[str, list[TradeResult]] = {}
    ...
    for anchor in anchors:
        ...
        anchor_base_trades[anchor.key] = filled
    ...
    for fold_idx, (is_start, is_end, oos_start, oos_end) in enumerate(folds, start=1):
        ...
        viable = [
            ...
            if int(row[2].get("total_trades", 0)) >= MIN_COMBINED_OOS_TRADES
        ]
        ...
        fold_rows.append(
            {
                ...
                "selected_is_trades": int(best_is["total_trades"]),
                "selected_is_avg_r": round(float(best_is["avg_r"]), 6),
                "selected_is_pf": round(float(best_is["profit_factor"]), 6),
                "selected_oos_trades": int(best_oos["total_trades"]),
                "selected_oos_avg_r": round(float(best_oos["avg_r"]), 6),
                "selected_oos_pf": round(float(best_oos["profit_factor"]), 6),
                "selected_oos_total_r": round(float(best_oos["total_r"]), 6),
            }
        )

        for candidate, is_metrics, oos_metrics in candidate_scores:
            fold_rows.append(
                {
                    "fold": fold_idx,
                    "is_start": is_start,
                    ...
                    "oos_end": oos_end,
                    "selected_overlay": "",
                    "candidate_overlay": candidate.overlay_key,
                    "candidate_label": candidate.label,
                    "candidate_is_trades": int(is_metrics["total_trades"]),
                    "candidate_is_avg_r": round(float(is_metrics["avg_r"]), 6),
                    "candidate_is_pf": round(float(is_metrics["profit_factor"]), 6),
                    "candidate_oos_trades": int(oos_metrics["total_trades"]),
                    "candidate_oos_avg_r": round(float(oos_metrics["avg_r"]), 6),
                    "candidate_oos_pf": round(float(oos_metrics["profit_factor"]), 6),
                    "candidate_oos_total_r": round(float(oos_metrics["total_r"]), 6),
                }
            )
    ...
    summary = {
        "meta": {
            ...
            "note": "Post-trade overlay walk-forward study; not engine-integrated yet.",
        },
        "selection_counts": selection_counts,
        "selected_oos_metrics": selected_oos_metrics,
        "fixed_oos_summary": fixed_rows,
    }
    ...
    lines = [
        ...
        "## Scope",
        "",
        f"- Candidate overlays: {', '.join(c.overlay_key for c in CANDIDATES)}",
        f"- Walk-forward: {IS_MONTHS}m IS / {OOS_MONTHS}m OOS / {STEP_MONTHS}m step",
    ...
    for _, row in fixed_df.iterrows():
        lines.append(
            f"- `{row['overlay_key']}`: OOS avgR {row['oos_avg_r']:+.3f}, "
            f"PF {row['oos_pf']:.2f}, trades {int(row['oos_trades'])}, "
            f"totalR {row['oos_total_r']:+.1f}, DD {row['oos_max_dd_r']:.1f}R"
        )
    lines.extend(["", "## Overlay Selection Frequency", ""])
    if selection_counts:
        for key, count in selection_counts.items():
            lines.append(f"- `{key}` selected in {count} folds")
    else:
        lines.append("- No selections recorded.")
    lines.extend(
        [
            "",
            "## Selected-By-IS Combined OOS",
            "",
            f"- Trades: {int(selected_oos_metrics['total_trades'])}",
            f"- Avg R: {selected_oos_metrics['avg_r']:+.3f}",
            f"- PF: {selected_oos_metrics['profit_factor']:.2f}",
            f"- Total R: {selected_oos_metrics['total_r']:+.1f}",
            f"- Sharpe: {selected_oos_metrics['sharpe_ratio']:.2f}",
            f"- Max DD: {selected_oos_metrics['max_drawdown_r']:.1f}R",
            "",
            "## Notes",
            "",
    ...
    print("\nFixed overlay OOS ranking:")
    for _, row in fixed_df.iterrows():
        print(
            f"  {row['overlay_key']}: OOS avgR={row['oos_avg_r']:+.3f} | "
            f"PF={row['oos_pf']:.2f} | trades={int(row['oos_trades'])} | "
            f"totalR={row['oos_total_r']:+.1f}"
        )
    print("\nSelection counts:")
    for key, count in selection_counts.items():
        print(f"  {key}: {count}")
    print(
        f"\nSelected-by-IS combined OOS: avgR={selected_oos_metrics['avg_r']:+.3f} | "
        f"PF={selected_oos_metrics['profit_factor']:.2f} | trades={int(selected_oos_metrics['total_trades'])}"
    )
    print(f"\nReport: {REPORT_PATH}")
