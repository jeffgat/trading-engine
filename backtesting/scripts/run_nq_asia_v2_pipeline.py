#!/usr/bin/env python3
"""NQ Asia improved config — robust pipeline Phase 2-5."""
...
def main():
    ...
    for v in param_ranges.values():
        total *= len(v)
    print(f"Grid: {total} combos per fold", flush=True)
    ...
    # Save for reference
    with open("/tmp/wf_result_nq_asia_v2.pkl", "wb") as f:
        pickle.dump(wf_result, f)
    with open("/tmp/stability_nq_asia_v2.pkl", "wb") as f:
    ...
    for yr, r in hm.get("r_by_year", {}).items():
        print(f"    {yr}: {r:>8.1f}R")
    ...
    print("=" * 60)
    print(f"  Phase 1 (Structural):    PASS — {1757} trades, {78.5:.1f}% WR, PF {1.31:.2f}")
    results = [
    ...
    all_pass = p2_pass and cr.passed and p4_pass and p5_pass
    if all_pass:
        verdict = "GO"
