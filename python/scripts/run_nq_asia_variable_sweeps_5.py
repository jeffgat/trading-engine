#!/usr/bin/env python3
"""NQ Asia ORB — Variable sweeps round 5: post same-candle SL/TP bug fix.

The simulator bug fix changed trade outcomes — stops and take-profits on the
entry candle are now correctly counted. The entire parameter surface has shifted,
so we re-sweep all 12 dimensions from the R4 Final anchor.

R5 anchor (= R4 Final config, unchanged):
  ORB: 20:00-20:10 (10m), entry until 01:00, flat 00:00
  stop=3.7%, min_gap_atr=0.90%, max_gap_atr=5.0%
  rr=1.75, tp1=0.35, ATR=5, direction=both, ICF=OFF, continuation, 1s magnifier
  DOW gate: no-Thursday

Adoption rule: Calmar delta > +0.3 AND no NEW negative full years.
If converged, move to grid sweep.
"""
...
def main():
    print("NQ Asia ORB — Round 5 (R1): post same-candle SL/TP bug fix")
    print("=" * 90)
    print(f"Anchor: rr={ANCHOR.rr}, tp1={ANCHOR.tp1_ratio}, "
          f"stop={ANCHOR_SESSION.stop_atr_pct}%, gap={ANCHOR_SESSION.min_gap_atr_pct}%")
    print(f"ORB=10m (20:00-20:10), entry<=01:00, flat=00:00, ATR={ANCHOR.atr_length}, "
          f"dir={ANCHOR.direction_filter}, no-Thu, ICF=OFF")
    ...
    # ── 5. FLAT START ────────────────────────────────────────────────
    flat_starts = ["22:30", "23:00", "23:30", "00:00", "00:30", "01:00", "02:00"]
    ...
    for var in dimension_order:
        ...
        if adopt == "YES":
            any_adopted = True
            adopted_changes.append((var, best_val, delta))
    ...
    if not any_adopted:
        ...
        print(f"\n  ** NOT CONVERGED — {len(adopted_changes)} adoption(s): **")
        for var, val, delta in adopted_changes:
            print(f"    {var} -> {val} (Δ={delta:+.2f})")
        print(f"  Update anchor and re-sweep as R2 (run_nq_asia_variable_sweeps_6.py)")
