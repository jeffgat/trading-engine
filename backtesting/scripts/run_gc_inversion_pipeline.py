#!/usr/bin/env python3
"""Robust pipeline: GC NY Inversion Longs with magnifier.

Validates the v7 parameter set through 5 phases:
...
# Phase 3: Prop constraints (GC-adjusted: lower annual R target)
PROP_CONSTRAINTS = PropFirmConstraints(
    max_drawdown_r=10.0,
    min_annual_r=-999.0,     # Disabled: low-frequency strategy, sizing compensates
...
HOLDOUT_MIN_PF = 1.0
HOLDOUT_MIN_TRADES = 15     # ~14 months of holdout, longs-only

# Phase 5: Monte Carlo
MC_SIMS = 2000
MC_MIN_SURVIVAL = 0.70

# ── Walk-forward sweep ranges (narrow bands around v7 values) ────────────────
...
def phase1(df, df_1m):
    ...
    print(f"  Config: GC NY Inversion Longs rr=3.5/tp1=0.2/atr50/entry15:00 + magnifier")
...
def phase3(wf_result):
    ...
    # Compute average annual R from full years
    full_year_vals = {y: r for y, r in cr.annual_r_values.items()
                      if len(cr.monthly_r_values) > 0}  # show all years
...
def main():
    print()
    print("=" * 70)
    print("  ROBUST PIPELINE: GC NY Inversion Longs v8 (rr3.5/tp0.2/atr50/entry15:00)")
    print("=" * 70)
    print()
