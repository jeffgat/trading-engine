#!/usr/bin/env python3
"""RTY NY Continuation — Variable Sweeps Round 5.

R5 anchor (grid winner from R1 sweep):
  stop=2.0%, rr=5.5, gap=0.75%, tp1=0.5
  direction=long, ATR=14, 15m ORB, entry≤12:00, flat=15:50
  No DOW exclusion

Anchor changed significantly from R4 (stop 2.5→2.0, rr 4.0→5.5, gap 0.5→0.75).
Must re-sweep all dimensions per optimization discipline.
...
ANCHOR_SESSION = SessionConfig(
    ...
    flat_start="15:50",
    flat_end="16:00",
    stop_atr_pct=2.0,         # GRID WINNER
    min_gap_atr_pct=0.75,     # GRID WINNER
    max_gap_points=50.0,
    max_gap_atr_pct=0.0,
)
...
ANCHOR = StrategyConfig(
    ...
    rr=5.5,                    # GRID WINNER
    tp1_ratio=0.5,             # GRID WINNER (unchanged)
    atr_length=14,
    impulse_close_filter=False,
    name="RTY NY R5 Anchor",
)
...
def main():
    ...
    # ── Anchor ──
    print("=" * 100)
    print("  ANCHOR (R5: grid winner — stop=2.0%, rr=5.5, gap=0.75%, tp1=0.5)")
    print("=" * 100)
    ...
    # ── 4. Stop fine-tune ──
    print("=" * 100)
    print("  SWEEP 4: Stop ATR % (fine-tune around 2.0%)")
    print("=" * 100)
    print(HDR)
    for i, s in enumerate([1.0, 1.5, 2.0, 2.5, 3.0, 3.5], 1):
    ...
    # ── 5. R:R fine-tune ──
    print("=" * 100)
    print("  SWEEP 5: Risk-Reward Ratio (fine-tune around 5.5)")
    print("=" * 100)
    print(HDR)
    for i, rr in enumerate([4.0, 4.5, 5.0, 5.5, 6.0, 6.5, 7.0], 1):
    ...
    # ── 7. Gap fine-tune ──
    print("=" * 100)
    print("  SWEEP 7: Min Gap ATR % (fine-tune around 0.75%)")
    print("=" * 100)
    print(HDR)
    for i, g in enumerate([0.25, 0.5, 0.75, 1.0, 1.25, 1.5], 1):
    ...
    print("=" * 100)
    print("  R5 COMPLETE — If converged, proceed to grid sweep R2 with updated anchor.")
    print("=" * 100)
