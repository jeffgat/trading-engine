...
import type { LogResponse, MainLogEntry } from "@/execution/lib/types";
...
export function useMainLogs(
  ...
  useEffect(() => {
    fetch("/exec-api/logs/main?limit=100&offset=0")
      .then((r) => r.json())
  ...
  const loadMore = useCallback(() => {
    ...
    fetch(`/exec-api/logs/main?limit=100&offset=${offset}`)
      .then((r) => r.json())
      .then((data: LogResponse<MainLogEntry>) => {
