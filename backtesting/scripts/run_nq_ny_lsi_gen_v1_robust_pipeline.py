#!/usr/bin/env python3
"""NQ NY LSI general_v1 — 5-Phase Robust Pipeline (Bailey-aware).

general_v1 execution config NQ_NY_LSI leg:
  ORB: 09:30-09:45, entry 09:35-15:30, flat 15:50-16:00
  min_gap=3.75%, rr=3.0, tp1=0.3
...
# ── Candidate config (general_v1 execution NQ_NY_LSI) ────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ NY LSI general_v1 Robust Pipeline",
)
...
def phase_4(df, df_1m, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="general_v1 NQ_NY_LSI robust pipeline",
    )
    if entry.warning:
