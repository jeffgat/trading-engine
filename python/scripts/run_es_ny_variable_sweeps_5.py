#!/usr/bin/env python3
"""ES NY ORB Continuation — Variable Sweeps Round 5.

R4 adopted only 3 dimensions (rr, gap, ICF). Anchor is stabilizing.
R4 anchor had Calmar 10.97, 0 negative years, 450 trades, 64.4% WR, DD -7.6R.

R5 anchor:
  ORB: 09:30-09:55 (25m), entry until 13:00, flat 15:50-16:00
  stop=3.0%, min_gap_atr=1.5%, max_gap_pts=100, max_gap_atr=0 (off)
  rr=5.0, tp1=0.2, ATR=3, direction=long, ICF=ON, continuation, 1s magnifier
...
ANCHOR_SESSION = SessionConfig(
    ...
    flat_end="16:00",
    stop_atr_pct=3.0,        # ES minimum floor (stable since R1)
    min_gap_atr_pct=1.5,     # adopted R4 (from 3.0%)
    max_gap_points=100.0,    # insensitive
...
ANCHOR = StrategyConfig(
    ...
    rr=5.0,                   # adopted R4 (from 3.5)
    tp1_ratio=0.2,            # stable since R3
    atr_length=3,             # stable since R3
    impulse_close_filter=True,  # adopted R4 (from OFF)
    name="ES NY R5 Anchor",
)
...
def main():
    print("ES NY ORB Continuation — Round 5: stabilization check")
    print("=" * 90)
    ...
    # ── 1. STOP ATR % ────────────────────────────────────────────────
    stop_values = [3.0, 3.5, 4.0, 5.0, 6.0, 7.0, 8.0, 10.0]
    print_header("1. STOP ATR % (anchor=3.0%)")
    for i, s in enumerate(stop_values, 1):
        ...
        print_row(i, f"stop={s}%", m, is_base=(abs(s - 3.0) < 0.01))
    ...
    print_header("2. ORB WINDOW (anchor=25m)")
    ...
    print_header("3. ATR LENGTH (anchor=3)")
    ...
    # ── 6. DIRECTION ─────────────────────────────────────────────────
    print_header("6. DIRECTION (anchor=long)")
    ...
    # ── 7. REWARD:RISK ───────────────────────────────────────────────
    rr_values = [2.5, 3.0, 3.5, 4.0, 4.5, 5.0, 6.0, 7.0]
    print_header("7. REWARD:RISK (anchor=5.0)")
    for i, rr in enumerate(rr_values, 1):
        config = replace(ANCHOR, rr=rr)
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"rr={rr}", m, is_base=(abs(rr - 5.0) < 0.01))
        all_results.append(("rr", rr, m))

    # ── 8. TP1 RATIO ─────────────────────────────────────────────────
    tp1_values = [0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.5]
    print_header("8. TP1 RATIO (anchor=0.2)")
    for i, tp1 in enumerate(tp1_values, 1):
        config = replace(ANCHOR, tp1_ratio=tp1)
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"tp1={tp1}", m, is_base=(abs(tp1 - 0.2) < 0.01))
        all_results.append(("tp1_ratio", tp1, m))

    # ── 9. MIN GAP ATR % ─────────────────────────────────────────────
    gap_values = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0]
    ...
    # ── 11. MAX GAP POINTS ────────────────────────────────────────────
    maxgap_values = [0, 10, 20, 50, 100]
    print_header("11. MAX GAP POINTS (anchor=100, 0=no limit)")
    ...
    dimension_order = [
        ...
        "direction", "rr", "tp1_ratio", "min_gap_atr_pct", "dow_exclusion",
        "max_gap_points", "icf",
    ]
    ...
    if not any_adopted:
        ...
        print(f"\n  ** NOT CONVERGED — Update anchor and re-sweep as R6 **")
