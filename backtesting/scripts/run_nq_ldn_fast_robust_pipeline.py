#!/usr/bin/env python3
"""NQ LDN FAST — 5-Phase Robust Pipeline (Bailey-aware).

FAST execution config NQ_LDN leg:
  ORB: 03:00-03:30, entry until 08:25, flat 08:20-08:25
  stop=1.5% ATR-10, min_gap=1.0%, rr=6.0, tp1=0.7
  Long only

G5 gate (skip if NQ Asia hit TP1) is IGNORED — this pipeline validates
...
# ── Candidate config (FAST execution NQ_LDN) ──────────────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ LDN FAST Robust Pipeline",
)

# ── Walk-forward param ranges (centered on candidate) ────────────────────
PARAM_RANGES = {
    "ldn_stop_atr_pct": [1.0, 1.25, 1.5, 1.75, 2.0],
    "rr": [4.0, 5.0, 6.0, 7.0, 8.0],
    "ldn_min_gap_atr_pct": [0.5, 0.75, 1.0, 1.25, 1.5],
...
def phase_4(df, df_1m, df_1s, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST NQ_LDN robust pipeline",
    )
    if entry.warning:
...
def main():
    ...
    print(f"\n  Candidate: stop_atr={CANDIDATE_SESSION.stop_atr_pct}% rr={CANDIDATE.rr} "
          f"gap={CANDIDATE_SESSION.min_gap_atr_pct}% tp1={CANDIDATE.tp1_ratio}")
    print(f"  Structural: ORB 30m, entry<08:25, flat 08:20-08:25, ATR={CANDIDATE.atr_length}, "
    ...
    print(f"  Candidate: stop_atr={CANDIDATE_SESSION.stop_atr_pct}% rr={CANDIDATE.rr} "
          f"gap={CANDIDATE_SESSION.min_gap_atr_pct}% tp1={CANDIDATE.tp1_ratio}")
