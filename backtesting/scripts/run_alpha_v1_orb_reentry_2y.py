"""Explore sequential same-day ORB re-entries for the ALPHA_V1 ORB sleeve.

Study design:
- Legs: the three ORB legs currently active in ALPHA_V1
- Window: 2024-04-17 through 2026-04-17
- Variants: baseline cap=1, capped re-entry cap=2, and uncapped cap=0
- Regime work: attribute uncapped re-entry trades by combined trend x vol bucket,
  then test a small set of re-entry-only regime gates that match the
  "bullish + higher-vol" hypothesis.
"""
...
RESULT_DIR = ROOT / "data" / "results" / "alpha_v1_orb_reentry_2y"
REPORT_PATH = ROOT / "learnings" / "reports" / "ALPHA_V1_ORB_REENTRY_2Y.md"
...
def main() -> None:
    ...
    variant_streams: dict[str, dict[str, list[TradeResult]]] = {variant: {} for variant in VARIANT_CAPS}
    leg_variant_tables: dict[str, list[dict[str, Any]]] = {}
    uncapped_reentry_summaries: dict[str, dict[str, Any]] = {}
    combined_reentry_attr_frames: list[pd.DataFrame] = []

    for leg_key, leg in orb_legs.items():
        configs = []
        config_names: dict[str, str] = {}
        for variant, trade_cap in VARIANT_CAPS.items():
            name = (
            ...
            configs.append(
                with_overrides(
                    ...
                    name=name,
                    notes=(
                        "ALPHA_V1 ORB same-day re-entry exploration over the last two years; "
                        f"orb_trade_max_per_session={trade_cap}."
                    ),
        ...
        for variant, trade_cap in VARIANT_CAPS.items():
            ...
            if variant == "cap0_uncapped":
                ...
                if not reentry_attr.empty:
                    ...
                    combined_reentry_attr_frames.append(reentry_attr)
                uncapped_reentry_summaries[leg_key] = {
                    ...
                    "top_reentry_regimes": _top_regime_rows(reentry_attr),
    ...
    for gate_name, regimes in REENTRY_GATE_SETS.items():
        ...
        for leg_key in ORB_LEG_KEYS:
            leg_summary = uncapped_reentry_summaries[leg_key]
        ...
        if baseline_total_r is not None:
            delta_r = sleeve["total_r"] - baseline_total_r
        gate_rows.append(
            {
                "gate": gate_name,
                ...
                "delta_vs_cap1_r": _round(delta_r, 2) if delta_r is not None else None,
    ...
    combined_reentry_table = _top_regime_rows(
        pd.concat(combined_reentry_attr_frames, ignore_index=True) if combined_reentry_attr_frames else pd.DataFrame(),
        min_trades=5,
        limit=8,
    )
            ...
            [
                ...
                _markdown_table(
                    [
                        {
                            "bucket": "first_trades",
                            **uncapped_reentry_summaries[leg_key]["first_metrics"],
                        },
                        {
                            "bucket": "reentries_only",
                            **uncapped_reentry_summaries[leg_key]["reentry_metrics"],
                        },
                    ],
                    [
                        "bucket",
                ...
                "",
                f"- Re-entry days: `{uncapped_reentry_summaries[leg_key]['reentry_days']}`",
                f"- Max trades in one session-day: `{uncapped_reentry_summaries[leg_key]['max_trades_per_day']}`",
                f"- Trades-per-day distribution: `{uncapped_reentry_summaries[leg_key]['trade_count_distribution']}`",
                "",
                "### Top Re-Entry Regimes",
                "",
                _markdown_table(
                    uncapped_reentry_summaries[leg_key]["top_reentry_regimes"],
                    [
                        "bucket",
                        "trades",
                        "avg_r",
    ...
    payload = {
        ...
        "uncapped_reentry": {
            leg_key: {
                "first_metrics": uncapped_reentry_summaries[leg_key]["first_metrics"],
                "reentry_metrics": uncapped_reentry_summaries[leg_key]["reentry_metrics"],
                "reentry_days": uncapped_reentry_summaries[leg_key]["reentry_days"],
                "max_trades_per_day": uncapped_reentry_summaries[leg_key]["max_trades_per_day"],
                "trade_count_distribution": uncapped_reentry_summaries[leg_key]["trade_count_distribution"],
                "top_reentry_regimes": uncapped_reentry_summaries[leg_key]["top_reentry_regimes"],
            }
            for leg_key in ORB_LEG_KEYS
        },
        "combined_reentry_regimes": combined_reentry_table,
    ...
    print("ALPHA_V1 ORB RE-ENTRY EXPLORATION")
