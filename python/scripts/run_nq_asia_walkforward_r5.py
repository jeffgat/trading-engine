#!/usr/bin/env python3
"""NQ Asia ORB — Fixed-param walk-forward + hold-out test on R5 anchor.

Post same-candle SL/TP bug fix. Converged from R1-R5 variable sweeps + grid sweep.

Config: stop=3.7%, rr=1.75, gap=0.90%, tp1=0.10, ORB=10m, entry<=00:00,
        flat=01:00, ATR=5, direction=both, no-Thursday, ICF=OFF, 1s magnifier

Full-history: 1,510 trades, 94.5% WR, PF 2.98, Sharpe 4.70, 165.1R (16.5 R/yr),
              DD -2.7R, Calmar 60.19, 0 negative years
"""
...
ANCHOR = StrategyConfig(
    ...
    rr=1.75, tp1_ratio=0.10, atr_length=5, name="NQ Asia R5 Final",
)
...
def main():
    print("NQ Asia ORB — Fixed-param Walk-Forward + Hold-out (R5)")
    print("Config: stop=3.7% rr=1.75 gap=0.90% tp1=0.10")
    print("ORB=10m, entry<=00:00, flat=01:00, ATR=5, both, no-Thu, ICF=OFF")
    print("=" * 90)
    ...
    print(f"\n  ** VERDICT: {verdict} **")
    print(f"  Config: stop=3.7% rr=1.75 gap=0.90% tp1=0.10")
    print(f"  ORB=10m, entry<=00:00, flat=01:00, ATR=5, both, no-Thu, ICF=OFF")
    print(f"  Full-history: Calmar 60.19, 16.5 R/yr, DD -2.7R, 0 neg years")
