#!/usr/bin/env python3
"""ES NY FAST — 5-Phase Robust Pipeline (Bailey-aware).

FAST execution config ES_NY leg:
  ORB: 09:30-09:45, entry until 13:00, flat 15:50-16:00
  stop=5.0% ATR-7, min_gap=0.25%, rr=5.0, tp1=0.2
  Long only, min_stop_points=3.0, min_tp1_points=3.0, Thursday excluded
...
from orb_backtest.analysis.gates import THU, apply_dow_filter
...
# ── Candidate config (FAST execution ES_NY) ──────────────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="ES NY FAST Robust Pipeline",
)

# ── Walk-forward param ranges (centered on candidate) ────────────────────
PARAM_RANGES = {
    "ny_stop_atr_pct": [3.0, 4.0, 5.0, 6.0, 7.0],
    "rr": [3.5, 4.0, 4.5, 5.0, 5.5],
    "ny_min_gap_atr_pct": [0.15, 0.2, 0.25, 0.3, 0.5],
...
def phase_1(df, df_1m, df_1s):
    ...
    print(f"  DOW filter: exclude Thursday")
...
def phase_4(df, df_1m, df_1s, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST ES_NY robust pipeline",
    )
    if entry.warning:
...
def main():
    ...
    print(f"  Structural: ORB 15m, entry<13:00, flat 15:50, ATR={CANDIDATE.atr_length}, "
          f"dir={CANDIDATE.direction_filter}, min_stop=3pt, min_tp1=3pt, DOW excl Thu")
