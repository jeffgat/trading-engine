#!/usr/bin/env python3
"""Sweep direction_filter for YM NY 5m ORB + ATR10.
...
def make_config(entry_end, direction_filter="both"):
...
def main():
    ...
    for entry_end, cutoff_label in [("11:30", "11:30"), ("13:00", "13:00")]:
        # --- Direction Filter Sweep ---
        dir_results = []
        print(f"\nSweeping direction_filter for {cutoff_label} cutoff...")
        for direction in ["both", "long", "short"]:
            cfg = make_config(entry_end, direction_filter=direction)
            r = run_and_report(f"direction={direction}", cfg, df)
            dir_results.append(r)
            print(f"  {direction}: Sharpe={r['sharpe']:.3f} Trades={r['trades']} AvgR={r['avg_r']:.3f}")

        print_table(f"DIRECTION FILTER — {cutoff_label} CUTOFF", dir_results)
