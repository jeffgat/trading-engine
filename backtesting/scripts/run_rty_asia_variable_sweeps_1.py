#!/usr/bin/env python3
"""RTY Asia Continuation — Variable Sweeps Round 1 (fresh start).

No prior RTY Asia optimization exists. Starting from default ASIA_SESSION
with conservative continuation params.

R1 anchor:
  ORB: 20:00-20:15 (15m), entry until 23:15, flat 06:45-07:00
  stop=5.25%, min_gap_atr=0.9%, max_gap_pts=50, max_gap_atr=0 (off)
...
ANCHOR_SESSION = SessionConfig(
    name="Asia",
    orb_start="20:00",
    orb_end="20:15",         # 15m ORB (Asia default)
    entry_start="20:15",
    entry_end="23:15",       # Asia default
    flat_start="06:45",      # Asia default
    flat_end="07:00",
    stop_atr_pct=5.25,       # Asia default
    min_gap_atr_pct=0.9,     # Asia default
    max_gap_points=50.0,     # Asia default
...
def main():
    print("RTY Asia ORB — Round 1: fresh start with default Asia config")
    print("=" * 90)
    ...
    print_header("1. STOP ATR % (anchor=5.25%)")
    for i, s in enumerate(stop_values, 1):
        ...
        print_row(i, f"stop={s}%", m, is_base=(abs(s - 5.25) < 0.01))
        print_years(m)
    ...
    print_header("9. MIN GAP ATR % (anchor=0.9%)")
    ...
    # ── 11. MAX GAP POINTS ────────────────────────────────────────────
    maxgap_values = [0, 10, 20, 30, 50, 75, 100]
