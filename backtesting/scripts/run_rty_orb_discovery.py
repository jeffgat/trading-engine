#!/usr/bin/env python3
"""RTY ORB Discovery — 3-session sweep with 1m magnifier.

Starting thesis: ORB continuation on RTY across NY, Asia, LDN sessions.
Baseline: RR=2.0, TP1=0.5, stop=10% ATR, min_gap=1% ATR.
Sweeps ATR stops, ORB-range stops, both directions, multiple ORB windows.

Pre-holdout only (<2025-01). Uses 1m magnifier (no 1s data for RTY).

Previous RTY learnings:
- Asia Cont longs: NO-GO (WFE 0.28, avg annual R 7.6)
- NY Cont longs: NO-GO (WFE 0.17, overfitting confirmed)
- NY LSI: NO-GO (PF < 1.0)
- Shorts destructive across all sessions
- LDN never tested — this is the primary gap to fill
"""
...
OUTPUT_DIR = ROOT / "data" / "results" / "rty_orb_discovery_3session"
    ...
    for orb_min in ORB_MINUTES:
        for stop_mode in ["atr", "orb"]:
            ...
            base = StrategyConfig(
                ...
                tp1_ratio=0.5,
                atr_length=14,
                name=f"RTY {session_name} ORB{orb_min}m {stop_mode}-stop",
            )
...
def main():
    print("RTY ORB Discovery — 3-Session Sweep (1m magnifier)")
    print("=" * 70)
    ...
    print(f"Bar magnifier: ON (1m) — no 1s data for RTY")
