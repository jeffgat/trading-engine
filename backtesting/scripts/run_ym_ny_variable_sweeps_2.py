#!/usr/bin/env python3
"""YM NY Long Continuation — Variable Sweeps Round 2.

R1 adopted: excl Tuesday (Calmar 0.35 → 0.65, 0 neg years).

R2 anchor:
  ORB: 09:30-09:45 (15m), entry until 12:00, flat 15:50-16:00
  stop=7.5%, min_gap_atr=1.0%, max_gap_pts=100, max_gap_atr=0 (off)
  rr=3.0, tp1=0.4, ATR=14, direction=long, continuation, 1m magnifier
...
ANCHOR = StrategyConfig(
    ...
    name="YM NY Long R2 Anchor",
)

# R1 adoption: exclude Tuesday
ANCHOR_DOW_EXCL = {1}
...
def main():
    ...
    # ── Anchor ──
    print("=" * 100)
    print("  ANCHOR R2 (R1 + excl Tue: rr=3.0, stop=7.5%, gap=1.0%, tp1=0.4, long-only)")
    print("=" * 100)
    ...
    # ── 3. Entry end ──
    print("=" * 100)
    print("  SWEEP 3: Entry End Time")
    print("=" * 100)
    print(HDR)
    for i, ee in enumerate(["11:00", "11:30", "12:00", "12:30", "13:00", "14:00", "15:00"], 1):
    ...
    # ── 4. ATR length ──
    print("=" * 100)
    print("  SWEEP 4: ATR Length")
    print("=" * 100)
    print(HDR)
    for i, atr in enumerate([10, 12, 14, 16, 20, 30, 50], 1):
    ...
    # ── 5. Stop ATR % ──
    print("=" * 100)
    print("  SWEEP 5: Stop ATR %")
    print("=" * 100)
    print(HDR)
    for i, s in enumerate([3.0, 5.0, 6.0, 7.5, 9.0, 10.0, 12.0], 1):
    ...
    # ── 6. R:R ──
    print("=" * 100)
    print("  SWEEP 6: Risk-Reward Ratio")
    print("=" * 100)
    print(HDR)
    for i, rr in enumerate([2.0, 2.5, 3.0, 3.5, 4.0, 5.0], 1):
    ...
    # ── 8. Min gap ATR % ──
    print("=" * 100)
    print("  SWEEP 8: Min Gap ATR %")
    print("=" * 100)
    print(HDR)
    for i, g in enumerate([0.5, 0.75, 1.0, 1.5, 2.0, 2.5, 3.0], 1):
        cfg = with_overrides(ANCHOR, ny_min_gap_atr_pct=g)
        _, m = run_and_metric(df, df_1m, cfg)
        print(fmt(i, f"gap={g}%", m))
    print()

    # ── 9. DOW exclusion (additional) ──
    print("=" * 100)
    print("  SWEEP 9: DOW Exclusion (additional, Tue already excluded)")
    print("=" * 100)
    trades_raw = run_backtest(df, ANCHOR, start_date=START_DATE, df_1m=df_1m)
    # Already excluding Tue from anchor; now test adding a second exclusion
    print(HDR)
    days = {0: "Mon", 2: "Wed", 3: "Thu", 4: "Fri"}
    for i, (d, name) in enumerate(days.items(), 1):
        m = compute_metrics(apply_dow_filter(trades_raw, ANCHOR_DOW_EXCL | {d}))
        print(fmt(i, f"excl Tue+{name}", m))
    print()

    # ── 10. Flat start ──
    print("=" * 100)
    print("  SWEEP 10: Flat Start Time")
    print("=" * 100)
    print(HDR)
    ...
    print("=" * 100)
    print("  R2 COMPLETE — Review sweeps above and adopt changes with Calmar Δ > +0.3")
    print("=" * 100)
