...
The absolute DD in R is NOT a hard filter during optimization. Position sizing can always be scaled down to bring dollar drawdown within any prop firm limit. A strategy with -15R DD and 15 R/yr (Calmar 1.0) is identical in practice to -10R DD and 10 R/yr — just trade at 2/3 the position size. What cannot be fixed by sizing is a low Calmar.

**The DD threshold in `PropFirmConstraints` is a position-sizing input**, not a go/no-go gate. Set `max_drawdown_r` to reflect account sizing, not to filter out strategies with "too much" drawdown.
...
Do NOT use a fixed DD threshold as a hard filter during optimization. Report DD alongside Calmar so the user can set position size accordingly.
