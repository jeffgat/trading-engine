#!/usr/bin/env python3
"""Step 2 — Variable Sweeps Round 12: GC NY Continuation Longs.

Anchor (from R11 adoption):
...
def rm(config):
    trades = dow_filter(run_backtest(df_5m, config, start_date=START_DATE, df_1m=df_1m, df_1s=df_1s))
...
def sw_atr():
    ...
    for v in [3,5,7,10,14,16,18,20,25,30,50]:
        m=rm(replace(ANCHOR,atr_length=v)); m["value"]=v; m["is_anchor"]=(v==7); R.append(m)
    pt(R,"ATR Length"); return R

def sw_ee():
    R=[]
    for v in ["10:30","11:00","11:30","12:00","13:00"]:
        m=rm(replace(ANCHOR,sessions=(replace(GC_NY,entry_end=v),))); m["value"]=v; m["is_anchor"]=(v=="12:00"); R.append(m)
    pt(R,"Entry End Time"); return R

def sw_fs():
    R=[]
    for v in ["12:00","12:30","13:00","13:30","14:00","14:30","15:00","15:50"]:
        m=rm(replace(ANCHOR,sessions=(replace(GC_NY,flat_start=v),))); m["value"]=v; m["is_anchor"]=(v=="13:30"); R.append(m)
...
def sw_tp1():
    ...
    for v in [0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.6,0.7]:
        m=rm(replace(ANCHOR,tp1_ratio=v)); m["value"]=v; m["is_anchor"]=(v==0.35); R.append(m)
    pt(R,"TP1 Ratio"); return R

def sw_gap():
    R=[]
    for v in [1.0,1.5,2.0,2.5,3.0,3.5,4.0,5.0,6.0,8.0]:
        m=rm(replace(ANCHOR,sessions=(replace(GC_NY,min_gap_atr_pct=v),))); m["value"]=v; m["is_anchor"]=(v==3.0); R.append(m)
    pt(R,"Min Gap ATR %"); return R

def sw_dow():
    ta = run_backtest(df_5m, ANCHOR, start_date=START_DATE, df_1m=df_1m, df_1s=df_1s)
    R=[]
    for l,es in [("Fri only",{4}),("none",set()),("Mon+Fri",{0,4}),("Wed+Fri",{2,4}),("Thu+Fri",{3,4})]:
        f = df_custom(ta,es) if es else ta
        m=compute_metrics(f); m["neg_full_years"]=sum(1 for y,r in m.get("r_by_year",{}).items() if r<0 and y!=CPY)
        m["r_per_yr"]=m["total_r"]/DATA_YEARS; m["value"]=l; m["is_anchor"]=(l=="Fri only"); R.append(m)
    pt(R,"DOW Exclusion"); return R

def sw_mga():
    R=[]
    for l,v in [("OFF",0.0),("15%",15.0),("20%",20.0),("25%",25.0),("30%",30.0),("35%",35.0),("40%",40.0),("50%",50.0),("75%",75.0)]:
        m=rm(replace(ANCHOR,sessions=(replace(GC_NY,max_gap_atr_pct=v),))); m["value"]=l; m["is_anchor"]=(l=="30%"); R.append(m)
...
if __name__ == "__main__":
    print(f"\n{'='*90}")
    print("  GC NY CONT LONGS — VARIABLE SWEEPS ROUND 12")
    print("  Anchor: stop=4.5%, rr=9.0, tp1=0.35, ATR 7, gap=3.0%, max_gap_atr=30%")
    print("          ICF=True, 8m ORB, entry→12:00, flat 13:30, long-only, FOMC excl, Fri excl")
    print(f"{'='*90}")
    ts=time.time()
    print("\nRunning anchor...")
    at, am = rmt(ANCHOR)
    ...
    print(f"\n{'='*90}\n  ROUND 12 SUMMARY — Anchor Calmar: {ac:.2f} | Neg years: {an}\n{'='*90}")
    adoptions=[]
    ...
    for dn,res in sweeps.items():
        ...
        ad=d>0.3 and best["neg_full_years"]<=an and best["total_trades"]>100
        dec="→ ADOPT" if ad else "  keep"
