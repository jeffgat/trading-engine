#!/usr/bin/env python3
"""ES LDN Continuation Both — Grid Sweep R1 (Full Optimization Step 3).

5D grid around converged anchor from 7 rounds of variable sweeps:
  stop=6%, rr=2.5, gap=1.0%, tp1=0.5, ATR=3
  ORB 10m, entry_end 08:25, flat 08:20, both, DOW excl Mon, ICF off, 1s mag.

Grid dimensions:
  Stop: [5.0, 6.0, 7.5, 10.0]          = 4 values
  RR:   [2.0, 2.5, 3.0, 3.5]           = 4 values
  Gap:  [0.5, 1.0, 1.5]                 = 3 values
  TP1:  [0.4, 0.5, 0.6]                 = 3 values
  ATR:  [3, 5, 7]                       = 3 values (unstable during sweeps)
...
if __name__ == "__main__":
    print("=" * 100)
    print("  ES LDN CONTINUATION BOTH — GRID SWEEP R1")
    print("=" * 100)
    ...
    anchor_rank = next((i+1 for i, r in enumerate(results)
                        if r["stop"]==6.0 and r["rr"]==2.5 and r["gap"]==1.0
                        and r["tp1"]==0.5 and r["atr"]==3), None)
    ...
    if clean:
        winner = clean[0]
        anchor = next((r for r in results if r["stop"]==6.0 and r["rr"]==2.5
                        and r["gap"]==1.0 and r["tp1"]==0.5 and r["atr"]==3), None)
        if anchor:
