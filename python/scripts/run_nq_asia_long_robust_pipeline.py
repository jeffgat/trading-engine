#!/usr/bin/env python3
"""NQ Asia Continuation Long — 5-Phase Robust Pipeline (R9 converged anchor).

Anchor from variable sweeps R1-R9 convergence + Grid Sweep R1 confirmation:
  stop_orb=100%, rr=6.0, min_gap_orb=10%, tp1=0.3
  ORB 15m (20:00-20:15), entry until 22:30, flat 04:00-07:00
  Long only, DOW excl Tue, ICF OFF, 1s bar magnifier, continuation

Grid sweep R1: Anchor ranked #1/400 (Calmar 22.61), delta=0 — confirmed.
In-sample: Calmar 22.61, DD -8.9R, 0 neg years, 746 trades.
...
PARAM_RANGES = {
    "asia_stop_orb_pct": [75.0, 100.0, 125.0],
    "rr": [5.0, 6.0, 7.0],
    "asia_min_gap_orb_pct": [7.0, 10.0, 12.0],
    "tp1_ratio": [0.2, 0.3, 0.4],
}
...
if __name__ == "__main__":
    print(flush=True)
    print("=" * 70, flush=True)
    print("  NQ ASIA CONTINUATION LONG — 5-PHASE ROBUST PIPELINE (R9)", flush=True)
    print("  Anchor: stop_orb=100% | rr=6.0 | gap_orb=10% | tp1=0.3", flush=True)
    print("  Structural: ORB 15m | entry<22:30 | flat 04:00", flush=True)
    print("  Long only | DOW excl Tue | ICF OFF | 1s mag", flush=True)
    print("=" * 70, flush=True)
    ...
    print(f"  Anchor: stop_orb=100% | rr=6.0 | gap_orb=10% | tp1=0.3", flush=True)
    print(f"  Structural: ORB 15m | entry<22:30 | flat 04:00", flush=True)
    print(f"  Long only | DOW excl Tue | ICF OFF | 1s mag", flush=True)
    print(flush=True)
