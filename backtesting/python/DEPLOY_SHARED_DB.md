...
```
backtesting/python/data/results/experiments.db
```
...
#### File 1: `backtesting/python/deploy/experiments_api.py`
...
#### File 2: `backtesting/python/deploy/experiments-db.service`
...
#### File 3: `backtesting/python/deploy/deploy_db.sh`
...
#   bash backtesting/python/deploy/deploy_db.sh
#
# First time? Run with --setup flag:
#   bash backtesting/python/deploy/deploy_db.sh --setup
set -euo pipefail
...
#### File 4: `backtesting/python/src/orb_backtest/experiments_remote.py`
...
Add a conditional import at the top of `backtesting/python/src/orb_backtest/experiments.py`. Right after the existing imports (after line 12), add:
...
chmod +x backtesting/python/deploy/deploy_db.sh

# 3. First-time setup (pass the path to Chris's DB as second arg)
bash backtesting/python/deploy/deploy_db.sh --setup ~/Downloads/experiments.db
...
| File | Purpose |
|------|---------|
| `backtesting/python/deploy/experiments_api.py` | Standalone FastAPI wrapping experiments module |
| `backtesting/python/deploy/experiments-db.service` | systemd unit file |
| `backtesting/python/deploy/deploy_db.sh` | Deploy script (with `--setup` for first time) |
| `backtesting/python/src/orb_backtest/experiments_remote.py` | HTTP client matching experiments.py interface |

Files to modify:

| File | Change |
|------|--------|
| `backtesting/python/src/orb_backtest/experiments.py` | Add conditional import for remote mode |
