#!/usr/bin/env python3
"""NQ NY+LDN ORB Robust Pipeline — 5-phase prop firm validation.

Instrument: NQ (Nasdaq-100), continuation strategy, bar magnifier enabled.
Sessions: NY (09:30-09:45 ET ORB) + LDN (03:00-03:15 ET ORB).
Data range: ~2015-01-01 to 2026-02 (~11 years).
BE offset: 0 ticks (no breakeven cushion).
...
PARAM_RANGES = {
    ...
    "ny_stop_atr_pct": [5.0, 6.5, 7.5, 9.0, 10.0],
    "ny_min_gap_atr_pct": [1.5, 2.0, 2.5, 3.0],
    "ldn_stop_atr_pct": [5.0, 7.0, 10.0, 12.0],
    "ldn_min_gap_atr_pct": [0.75, 1.0, 1.5, 2.0],
}
...
def main():
    ...
    t0 = time.time()
    df_5m = load_5m_data("NQ_5m.csv", start=None, end=None)
    df_1m = load_1m_for_5m("NQ_5m.csv", start=None, end=None)
    print(f"5m bars: {len(df_5m):,}  |  1m bars: {len(df_1m):,}")
    print(f"Date range: {df_5m.index[0]} -> {df_5m.index[-1]}")
    print(f"Loaded in {time.time() - t0:.1f}s")

    # Base config: NQ NY+LDN continuation with magnifier
    base_config = StrategyConfig(
        sessions=(NY_SESSION, LDN_SESSION),
        instrument=NQ,
        ...
        rr=2.5,
        tp1_ratio=0.5,
        name="NQ NY+LDN Robust Pipeline",
    )
    ...
    print(f"Params: rr={base_config.rr}, tp1_ratio={base_config.tp1_ratio}")
