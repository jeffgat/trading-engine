#!/usr/bin/env python3
"""NQ NY LSI — 3-minute timeframe study.

Tests whether the proven NQ NY LSI RR2/TP0.5 edge survives on 3m candles.
Uses the exact winning config from the 5m study as the anchor, with swing
params scaled to preserve the same real-time window (5h = 100 bars at 3m
vs 60 bars at 5m).

Phases:
  A) Baseline: 5m-equivalent params on 3m bars (sanity check)
...
from orb_backtest.data.loader import load_1m_for_5m, load_1s_for_5m
...
# ── Anchor config: 5m winner with params scaled for 3m bars ─────────────
# Time-equivalent scaling:
#   5m n_left=8  → 3m n_left=13  (8 * 5/3 = 13.3 → 13)
#   5m n_right=60 → 3m n_right=100 (60 * 5/3 = 100)
#   5m fvg_window_left=20 → 3m fvg_window_left=33 (20 * 5/3 = 33.3 → 33)
#   5m fvg_window_right=5 → 3m fvg_window_right=8 (5 * 5/3 = 8.3 → 8)
ANCHOR_3M = StrategyConfig(
    sessions=(NY_SESSION,),
    instrument=NQ,
    strategy="lsi",
    ...
    tp1_ratio=0.5,
    atr_length=14,
    lsi_n_left=13,          # time-scaled from 8
    lsi_n_right=100,         # time-scaled from 60
    lsi_fvg_window_left=33,  # time-scaled from 20
    lsi_fvg_window_right=8,  # time-scaled from 5
    ...
    name="NQ NY LSI 3m Baseline",
)


# ── Helpers ──────────────────────────────────────────────────────────────

def resample_1m_to_3m(df_1m: pd.DataFrame) -> pd.DataFrame:
    """Resample 1-minute bars to 3-minute bars.

    Groups every 3 consecutive 1m bars starting from the first bar.
...
def run_one(df_3m, config, gate_fn, maps, df_1s=None):
    """Run a single backtest, apply regime gate, return metrics dict."""
    trades_raw = run_backtest(
        df_3m, config,
        end_date=END_DATE,
        df_1m=df_1s,  # 1s bars passed as the drill-down tier
        _maps=maps,
    )
...
def main():
    ...
    print("=" * 80)
    print("NQ NY LSI — 3-MINUTE TIMEFRAME STUDY")
    print("=" * 80)
    ...
    print("Resampling 1m → 3m...", flush=True)
    df_3m = resample_1m_to_3m(df_1m)
    print(f"  3m bars: {len(df_3m):,}")

    print("Loading NQ 1s data for magnifier...", flush=True)
    df_1s = load_1s_for_5m("NQ_5m.parquet")
    if df_1s is not None:
        print(f"  1s bars: {len(df_1s):,}")
    else:
        print("  1s: not available (proceeding without magnifier)")

    print("Building bar maps (3m → 1s)...", flush=True)
    # Use the generic timestamp-based mapping: 3m bars map to 1s sub-bars
    maps = build_maps(df_3m, df_1m=df_1s) if df_1s is not None else build_maps(df_3m)
    print(f"  Maps built in {time.time() - t_start:.1f}s")

    print("Building regime calendar from 1m data...", flush=True)
    # Regime calendar needs daily data — 1m resamples to daily fine
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # PHASE A: Baseline — 5m-equivalent params on 3m
    # ═══════════════════════════════════════════════════════════════════════
    print(f"\n{'─' * 80}")
    print("PHASE A: Baseline (5m-equivalent params on 3m)")
    print(f"{'─' * 80}")

    t1 = time.time()
    r = run_one(df_3m, ANCHOR_3M, gate_fn, maps, df_1s)
    print_row(1, 1, "3m baseline (time-equiv)", r, time.time() - t1)
    all_results["baseline"] = [("3m baseline (time-equiv)", r)]

    # Also run with exact 5m bar counts (no scaling) for comparison
    exact_5m_cfg = dataclasses.replace(
        ANCHOR_3M,
        lsi_n_left=8, lsi_n_right=60,
        lsi_fvg_window_left=20, lsi_fvg_window_right=5,
        name="NQ NY LSI 3m Exact5mParams",
    )
    t1 = time.time()
    r2 = run_one(df_3m, exact_5m_cfg, gate_fn, maps, df_1s)
    print_row(2, 2, "3m exact-5m-params (no scaling)", r2, time.time() - t1)
    all_results["baseline"].append(("3m exact-5m-params (no scaling)", r2))
    ...
    # n_left range: 3m equivalent of 5m [4, 6, 8, 10, 12, 15, 20, 25]
    # At 3m, each bar = 3 min. Scale up for time-equivalence but also test raw counts.
    N_LEFT_VALUES = [5, 8, 10, 13, 15, 20, 25, 30, 40]
    # n_right: 3m equivalent of 5m [30, 45, 60, 75, 90, 100, 120]
    N_RIGHT_VALUES = [30, 50, 75, 100, 120, 150, 180]
    ...
    for nl in N_LEFT_VALUES:
        for nr in N_RIGHT_VALUES:
            idx += 1
            label = f"nL={nl} nR={nr}"
            cfg = dataclasses.replace(ANCHOR_3M, lsi_n_left=nl, lsi_n_right=nr,
                                       name=f"NQ NY LSI 3m nL{nl} nR{nr}")
            t1 = time.time()
            r = run_one(df_3m, cfg, gate_fn, maps, df_1s)
    ...
    # Extract best swing params for subsequent phases
    best_swing_label, best_swing_r = swing_sorted[0]
    # Parse n_left/n_right from label
    ...
    FVG_LEFT_VALUES = [10, 15, 20, 25, 33, 40, 50]
    FVG_RIGHT_VALUES = [3, 5, 8, 10, 15, 20]
    ...
    for fl in FVG_LEFT_VALUES:
        for fr in FVG_RIGHT_VALUES:
            ...
            cfg = dataclasses.replace(
                ...
                name=f"NQ NY LSI 3m fvgL{fl} fvgR{fr}",
            )
            t1 = time.time()
            r = run_one(df_3m, cfg, gate_fn, maps, df_1s)
    ...
    # ═══════════════════════════════════════════════════════════════════════
    print(f"\n{'=' * 80}")
    print("FINAL SUMMARY — 3m TIMEFRAME STUDY")
    print(f"{'=' * 80}")
    ...
    # ── Save results ─────────────────────────────────────────────────────
    output_path = ROOT / "data" / "results" / "nq_ny_lsi_3m_tf_study.json"
    ...
    save_data = {
        "info": {
            "description": "NQ NY LSI 3m timeframe study",
