#!/usr/bin/env python3
"""Phase-One Robust Pipeline — SI ORB Continuation promoted candidates.

3 promoted + 2 challengers from discovery pipeline (all Asia shorts):
  Asia-3: 30m ORB, ORB 75%, RR=3.0, TP1=0.6, short (WF Cal 9.50, WFE 0.570) — PROMOTE
  Asia-1: 30m ORB, ORB 75%, RR=2.5, TP1=0.6, short (WF Cal 8.08, WFE 0.469) — PROMOTE
  Asia-2:  5m ORB, ORB 50%, RR=3.5, TP1=0.6, short (WF Cal 5.74, WFE 0.429) — PROMOTE
  Asia-4: 30m ORB, ORB 75%, RR=3.0, TP1=0.5, short (WF Cal 5.13, WFE 0.417) — CHALLENGER
  NY-1:   10m ORB, ORB 75%, RR=2.5, TP1=0.5, short (WF Cal 2.38, WFE 0.337) — CHALLENGER
"""
...
OUTPUT_DIR = ROOT / "data" / "results" / "si_orb_phase_one"
...
def main():
    print("Phase-One Robust Pipeline — SI ORB Continuation (5 candidates)")
    print("=" * 70)
    t0 = time.time()
    ...
    for cand_name, config in CANDIDATES.items():
        ...
        viable = (metrics.get("total_trades", 0) >= 50
        ...
        all_results[cand_name] = {
            "verdict": verdict,
            "pre_holdout_metrics": {k: round(float(v), 4) if isinstance(v, (int, float)) else v
                for k, v in metrics.items()
                if k in {"total_trades", "total_r", "calmar_ratio", "sharpe_ratio",
                         "max_drawdown_r", "win_rate", "profit_factor", "avg_r", "r_by_year"}},
            "holdout_metrics": {k: round(float(v), 4) if isinstance(v, (int, float)) else v
    ...
    print(f"\n{'=' * 70}")
    print("PHASE-ONE SUMMARY — SI ORB Continuation")
    print(f"{'=' * 70}")
