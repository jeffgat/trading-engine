#!/usr/bin/env python3
"""Robust pipeline: GC NY Continuation Longs — R2 config (clean data, 1s magnifier).

Converged anchor after R2-R4 variable sweeps + R2 grid sweep:
  stop=4.0% ATR, min_gap=3.5%, rr=4.0, tp1=0.5
  ATR 10, 10m ORB (09:30-09:40), entry→11:00, flat_start=15:50
  max_gap_atr=30%, long-only, FOMC dates excluded

Anchor evolution: R1 anchor (R6 prior) → R1 grid winner (rr 4.0, gap 3.5) →
  R2 sweep (ATR 10) → R3 sweep (max_gap_atr=30%) → R4 sweep (confirmed) →
  R2 grid (#2/450, Δ=+0.07, stays)
...
# ── Base config (R2 converged anchor) ────────────────────────────────────────
...
# ── WF sweep params (centred on R2 grid results) ────────────────────────────
...
def phase1(df, df_1m, df_1s):
    ...
    print(f"  Config: stop=4.0% | rr=4.0 | min_gap=3.5% | tp1=0.5 | ATR 10 | max_gap_atr=30%")
    print(f"          10m ORB | entry→11:00 | FOMC excl | long-only")
...
def main():
    print()
    print("=" * 70)
    print("  ROBUST PIPELINE: GC NY Cont Longs R2 (clean data, 1s magnifier)")
    print("  stop=4.0% | rr=4.0 | min_gap=3.5% | tp1=0.5 | ATR 10")
    print("  max_gap_atr=30% | 10m ORB | entry→11:00 | FOMC excluded")
