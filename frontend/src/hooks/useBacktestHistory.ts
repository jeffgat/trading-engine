...
export function useBacktestHistory(): UseHistoryReturn {
  ...
  const starBacktest = useCallback(async (id: string): Promise<boolean> => {
    try {
      const res = await fetch(`/api/backtests/${id}/star`, { method: "POST" });
  ...
  return { history, loading, activeId, refreshHistory, loadBacktest, refilterBacktest, deleteBacktest, starBacktest, hideBacktest, setActiveId };
}
