#!/usr/bin/env python3
"""ES LDN Continuation Both — Variable Sweeps Round 6 (Full Optimization Step 2).

R5 adoption: ORB 15m → 10m.
Anchor: stop=6%, ORB 10m (03:00-03:10), rr=2.5, tp1=0.5, ATR=3, gap=1.5%,
  flat 08:20, entry_end 08:25, both, DOW excl Mon, ICF off.

ATR unstable (14→3→20→3→5 wanted) — include in grid if still wants to move.
"""
...
ANCHOR_SESSION = SessionConfig(
    name="LDN",
    orb_start="03:00",
    orb_end="03:10",        # R5: 15m → 10m
    entry_start="03:10",    # matches orb_end
    entry_end="08:25",
    flat_start="08:20",
    flat_end="08:25",
    stop_atr_pct=6.0,
...
def sweep_orb_window(df, df_1m, df_1s):
    windows = [("5m","03:00","03:05","03:05"),("10m","03:00","03:10","03:10"),
               ("15m","03:00","03:15","03:15"),("20m","03:00","03:20","03:20"),
               ("25m","03:00","03:25","03:25"),("30m","03:00","03:30","03:30"),
    ...
    print_sweep_table(results, "ORB Window", "10m")
    return results
...
def sweep_dow(df, df_1m, df_1s):
    ...
    filters = {"none": set(), "Mon": {MON}, "Tue": {TUE}, "Wed": {WED},
               "Thu": {THU}, "Fri": {FRI}, "M+F": {MON, FRI}, "Th+F": {THU, FRI}}
    results = []
...
def sweep_max_gap(df, df_1m, df_1s):
    values = [("OFF",0,0),("20pt",20,0),("50pt",50,0),("75pt",75,0),("100pt",100,0),
              ("20%ATR",0,20),("50%ATR",0,50),("75%ATR",0,75),("100%ATR",0,100),("150%ATR",0,150)]
    results = []
...
if __name__ == "__main__":
    ...
    sweeps = [
        ...
        ("ORB Window",   "10m",                        sweep_orb_window),
    ...
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY — Variable Sweeps Round 6 ({total_time:.0f}s)")
    print(f"{'='*90}")
    ...
    for dim_name, info in all_sweeps.items():
        ...
        # Block ATR from adoption (unstable history)
    ...
    if adoptions:
        print(f"\n  >>> {len(adoptions)} adoption(s) — would need round 7")
    else:
        print(f"\n  >>> CONVERGED. Ready for grid sweep.")
        print(f"\n  Converged anchor:")
        print(f"    stop=6%, rr=2.5, gap=1.5%, tp1=0.5, ATR=3")
        print(f"    ORB 10m, entry_end 08:25, flat 08:20")
        print(f"    Both dirs, DOW excl Mon, ICF off, 1s magnifier")
        print(f"\n  Grid dimensions (5D): stop × rr × gap × tp1 × ATR")
