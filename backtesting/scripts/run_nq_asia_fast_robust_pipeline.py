#!/usr/bin/env python3
"""NQ ASIA FAST — 5-Phase Robust Pipeline (Bailey-aware).

FAST execution config NQ_Asia leg:
...
# ── Candidate config (FAST execution NQ_Asia) ──────────────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ Asia FAST Robust Pipeline",
)
...
def phase_4(df, df_1m, df_1s, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST NQ_Asia robust pipeline",
    )
    if entry.warning:
