#!/usr/bin/env python3
"""GC continuation shorts — variable sweeps R2.

Anchor after R1 adoption of max_gap_atr=30%:
  stop=3.0%, rr=7.0, gap=5.0%, tp1=0.5, ATR 10, 15m ORB, entry→15:00, max_gap_atr=30%
  Calmar 0.45, 1 neg year (2022)

Sweep 12 dimensions one at a time, all others held at anchor.
Adoption rule: Calmar delta > +0.15 AND no new negative full years AND trades >100.
  (Relaxed from +0.3 for low-Calmar strategy — user approved in R1)
...
# Adoption threshold (relaxed for low-Calmar strategy)
ADOPT_DELTA = 0.15
...
print("=" * 130)
print("VARIABLE SWEEPS R2: GC NY Continuation Shorts")
...
for mga in [0.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 50.0, 75.0, 100.0]:
