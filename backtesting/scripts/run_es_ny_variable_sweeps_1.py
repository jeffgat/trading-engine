#!/usr/bin/env python3
"""ES NY ORB Continuation — Variable Sweeps Round 1 (fresh start).

No prior ES NY optimization exists. Starting from default NY_SESSION config.
NQ NY R20 GO config (orb=20m, rr=2.625, stop=8.75%) serves as reference,
but we start from defaults and let the data speak.

R1 anchor:
  ORB: 09:30-09:45 (15m default), entry until 13:00, flat 15:50-16:00
  stop=7.5%, min_gap_atr=2.25%, max_gap_pts=100, max_gap_atr=0 (off)
...
ANCHOR_SESSION = SessionConfig(
    name="NY",
    orb_start="09:30",
    orb_end="09:45",         # 15m ORB (NY default)
    entry_start="09:45",
    entry_end="13:00",       # NY default
    flat_start="15:50",      # NY default
    flat_end="16:00",
    stop_atr_pct=7.5,        # NY default
    min_gap_atr_pct=2.25,    # NY default
    max_gap_points=100.0,    # NY default
...
ANCHOR = StrategyConfig(
    ...
    name="ES NY R1 Anchor",
)

ANCHOR_DOW_EXCL = set()  # no exclusion for fresh start
...
def main():
    print("ES NY ORB Continuation — Round 1: fresh start with default NY config")
    print("=" * 90)
    print(f"Anchor: rr={ANCHOR.rr}, tp1={ANCHOR.tp1_ratio}, "
          f"stop={ANCHOR_SESSION.stop_atr_pct}%, gap={ANCHOR_SESSION.min_gap_atr_pct}%")
    print(f"ORB=15m (09:30-09:45), entry≤13:00, flat=15:50, ATR={ANCHOR.atr_length}, "
          f"dir={ANCHOR.direction_filter}, no DOW excl, ICF=OFF")
    ...
    # ── 1. STOP ATR % ────────────────────────────────────────────────
    # ES minimum stop floor: 3.0% ATR (~7 ticks). Below this, perfect-fill sim inflates results.
    stop_values = [3.0, 4.0, 5.0, 6.0, 7.0, 7.5, 9.0, 10.0, 12.0, 15.0]
    print_header("1. STOP ATR % (anchor=7.5%)")
    for i, s in enumerate(stop_values, 1):
        ...
        print_row(i, f"stop={s}%", m, is_base=(abs(s - 7.5) < 0.01))
        print_years(m)
    ...
    # ── 3. ATR LENGTH ────────────────────────────────────────────────
    atr_values = [5, 7, 10, 12, 14, 20, 30, 50]
    ...
    # ── 11. MAX GAP POINTS ────────────────────────────────────────────
    maxgap_values = [0, 20, 50, 75, 100, 150]
