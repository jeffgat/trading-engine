#!/usr/bin/env python3
"""NQ NY Continuation — Variable Sweeps Round 1 (Longs-Only Fresh Start).

Fresh optimization on the fixed engine (TP1+BE same-bar exit bug fix, commit 6079ad4).
All previous NY results are CORRUPT — starting from sensible defaults.

R1 anchor (fresh start):
  ORB: 09:30-09:50 (20m), entry until 15:30, flat 15:50-16:00
  stop=9.0%, min_gap_atr=2.5%, max_gap_pts=100.0, max_gap_atr=0.0
  rr=2.5, tp1=0.5, ATR=12, direction=long, ICF=OFF, continuation, 1s magnifier
  DOW gate: none
...
ANCHOR = StrategyConfig(
    ...
    name="NQ NY Long R1 Anchor (fresh start)",
)

ANCHOR_DOW_EXCL = set()  # no DOW exclusion (fresh start)
...
def main():
    print(f"{INSTRUMENT_NAME} {SESSION_NAME} ORB — Variable Sweeps Round {SWEEP_ROUND} (Longs-Only Fresh Start)")
    print("=" * 90)
    ...
    print(f"\nContext: Fresh start — all previous NY results CORRUPT (TP1+BE same-bar exit bug fix)")
    ...
    print(f"\n{'='*90}")
    print(f"  SUMMARY — Round {SWEEP_ROUND} (Longs-Only Fresh Start, tp1 >= {MIN_TP1_RATIO})")
