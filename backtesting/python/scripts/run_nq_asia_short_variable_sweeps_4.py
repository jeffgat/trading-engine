#!/usr/bin/env python3
"""NQ Asia Continuation SHORT — Variable Sweeps Round 4.

R4 anchor (from Grid Sweep R1 winner):
  ORB: 20:00-20:10, entry until 23:00, flat 23:00-07:00
  stop=3.5%, min_gap_atr=1.0%, max_gap_pts=0, max_gap_atr=0
  rr=3.0, tp1=0.6, ATR=30, direction=short, ICF=ON, continuation, 1s magnifier
  DOW gate: excl Thu

Grid R1 winner: stop=3.5/rr=3.0/gap=1.0/tp1=0.6 (Calmar 4.69)
  Previous anchor: stop=5.0/rr=2.5 (Calmar 4.05), delta +0.64
  Both stop=3.5 and rr=3.0 were at grid boundary — sweep ranges extended.
...
ANCHOR = StrategyConfig(
    ...
    name="NQ Asia Short R4 Anchor",
)

ANCHOR_DOW_EXCL = {3}  # excl Thursday
...
def main():
    print(f"{INSTRUMENT_NAME} {SESSION_NAME} SHORT — Variable Sweeps Round {SWEEP_ROUND}")
    print("=" * 90)
    ...
    # -- 1. STOP ATR % ---------------------------------------------------------
    stop_vals = [1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0, 6.0, 7.5, 10.0]
    ...
    # -- 4. ENTRY END TIME -----------------------------------------------------
    entry_ends = ["21:30", "22:00", "22:30", "23:00", "23:30", "00:00", "01:00", "02:00", "03:00"]
    ...
    # -- 6. DIRECTION (skip — fixed at short for this optimization) ------------
    print(f"\n{'='*90}")
    print(f"  6. DIRECTION — SKIPPED (fixed at short)")
    print(f"{'='*90}")

    # -- 7. REWARD:RISK --------------------------------------------------------
    rr_vals = [2.0, 2.5, 2.75, 3.0, 3.25, 3.5, 4.0, 4.5, 5.0]
    ...
    # -- 9. MIN GAP ATR % ------------------------------------------------------
    gap_vals = [0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2.0, 2.5, 3.0]
    ...
    # Fixed: check best value that passes adoption criteria, not just absolute best
    best_adoptable_cal, best_adoptable_lbl, best_adoptable_m = anc_cal, "anchor", m_anc
    for i, (label, excluded) in enumerate(dow_sets, 1):
        ...
        print_years(m)
        # Check if this value passes adoption criteria
        cal = m["calmar_ratio"]
        delta = cal - anc_cal
        new_neg = neg_year_set(m) - anc_neg
        if delta > 0.3 and len(new_neg) == 0 and m["total_trades"] > 100:
            if cal > best_adoptable_cal:
                best_adoptable_cal = cal
                best_adoptable_lbl = label
                best_adoptable_m = m
    ok, delta = check_adopt(best_adoptable_lbl, best_adoptable_m, anc_cal, anc_neg)
    if ok:
        adoptions.append(("dow_exclusion", best_adoptable_lbl, delta))

    # -- 11. MAX GAP POINTS ----------------------------------------------------
    maxgap_vals = [0, 25, 50, 75, 100, 150, 200]
    print_header(f"11. MAX GAP POINTS (anchor={ANCHOR_SESSION.max_gap_points}, 0=off)")
    best_cal, best_lbl, best_m = anc_cal, "anchor", m_anc
