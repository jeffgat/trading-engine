...
from orb_continuation.config import default_config, NY_SESSION, ASIA_SESSION, LDN_SESSION
...
from core.optimize.grid import generate_param_grid, linspace_range, describe_grid
from core.optimize.parallel import run_sweep
from core.results.metrics import compute_metrics
from core.results.export import grid_results_to_dict, save_optimization_result
...
def main():
    ...
    parser.add_argument("--name", default=None, help="Label for this optimization run")
    ...
    grid_dict["date_start"] = str(df.index.min().date()) if len(df) > 0 else ""
    grid_dict["date_end"] = str(df.index.max().date()) if len(df) > 0 else ""
    if args.name:
...
def _print_best(title: str, config: StrategyConfig, metrics: dict, sweep_keys) -> None:
    ...
    print(f"    Sharpe: {metrics['sharpe_ratio']:.3f} | Calmar: {metrics['calmar_ratio']:.3f} | Max DD: ${metrics['max_drawdown_usd']:,.2f}")
