...
```bash
# From repo root:
bash execution/python/deploy/deploy.sh
```
...
rsync -avz --delete --exclude '.venv/' --exclude '__pycache__/' --exclude '.env' --exclude 'logs/' \
    execution/python/ root@143.110.148.234:/opt/orb-trader/
...
The React dashboard lives at `execution/frontend/`. In development, Vite proxies `/api/*` to the droplet at `143.110.148.234:8000`.
...
The API serves the built frontend from `execution/frontend/dist/` when present (static file mount in `api.py`).
