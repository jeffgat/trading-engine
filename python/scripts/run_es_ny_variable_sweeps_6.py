#!/usr/bin/env python3
"""ES NY ORB Continuation — Variable Sweeps Round 6 (convergence check).

R5 adopted only tp1 (0.2→0.4) and maxgap (100→10). Checking for convergence.

R6 anchor:
  ORB: 09:30-09:55 (25m), entry until 13:00, flat 15:50-16:00
  stop=3.0%, min_gap_atr=1.5%, max_gap_pts=10, max_gap_atr=0 (off)
  rr=5.0, tp1=0.4, ATR=3, direction=long, ICF=ON, continuation, 1s magnifier
...
ANCHOR_SESSION = SessionConfig(
    ...
    max_gap_points=10.0,     # adopted R5 (from 100)
...
ANCHOR = StrategyConfig(
    ...
    rr=5.0,                   # stable since R4
    tp1_ratio=0.4,            # adopted R5 (from 0.2)
    atr_length=3,             # stable since R3
    impulse_close_filter=True,  # stable since R4
    name="ES NY R6 Anchor",
)
...
def main():
    print("ES NY ORB Continuation — Round 6: convergence check")
    print("=" * 90)
    ...
    # ── 1. STOP ATR % ────────────────────────────────────────────────
    stop_values = [3.0, 3.5, 4.0, 5.0, 6.0, 8.0]
    ...
    # ── 5. DIRECTION ─────────────────────────────────────────────────
    print_header("5. DIRECTION (anchor=long)")
    ...
    # ── 6. REWARD:RISK ───────────────────────────────────────────────
    rr_values = [3.0, 3.5, 4.0, 4.5, 5.0, 6.0, 7.0]
    print_header("6. REWARD:RISK (anchor=5.0)")
    ...
    # ── 7. TP1 RATIO ─────────────────────────────────────────────────
    tp1_values = [0.2, 0.25, 0.3, 0.35, 0.4, 0.5, 0.6]
    ...
    print_header("8. MIN GAP ATR % (anchor=1.5%)")
    ...
    print_header("9. DOW EXCLUSION (anchor=excl Th+F)")
    ...
    print_header("10. MAX GAP POINTS (anchor=10, 0=no limit)")
    ...
    # ── 11. ICF ───────────────────────────────────────────────────────
    print_header("11. IMPULSE CLOSE FILTER (anchor=ON)")
    ...
    dimension_order = [
        "stop_atr_pct", "orb_window", "atr_length", "entry_end",
    ...
    if not any_adopted:
        ...
        print(f"\n  ** NOT CONVERGED — Update anchor and re-sweep as R7 **")
