...
from core.data.instruments import get_instrument, NQ
from core.engine.simulator import run_backtest, EXIT_NAMES, EXIT_NO_FILL
from core.results.metrics import compute_metrics
from core.results.export import results_to_json, results_to_dict, save_results, save_backtest_result
...
def main():
    ...
    parser.add_argument("--strategy", default=None, choices=["continuation", "reversal"],
                        help="Strategy type: continuation (default) or reversal (flip trade direction)")
    ...
    if args.plot:
        from core.viz.equity import plot_equity_curve, plot_monthly_returns
...
def _print_summary(m: dict) -> None:
    ...
    print(f"  Calmar (R):       {m['calmar_ratio']:.3f}")
