#!/usr/bin/env python3
"""ES Asia Continuation — Variable Sweeps Round 2.

R2 anchor (adopted from R1):
  ORB: 20:00-20:10 (10m), entry until 01:00, flat 06:45-07:00
  stop=3.0%, min_gap_atr=0.5%, max_gap_pts=50, max_gap_atr=0 (off)
  rr=2.0, tp1=0.3, ATR=14, direction=both, ICF=OFF, continuation, 1s magnifier
  DOW gate: none

R1 adoptions (Calmar Δ > +0.3, no new neg years):
  stop:  5.25% → 3.0% (+2.74)
  ORB:   15m → 10m (+1.05)
  entry: 23:15 → 01:00 (+2.01)
  tp1:   0.5 → 0.3 (+0.76)
  gap:   0.9% → 0.5% (+2.88)
...
def main():
    print("ES Asia ORB — Round 2: re-sweep on R1 adopted anchor")
    print("=" * 90)
    print(f"Anchor: rr={ANCHOR.rr}, tp1={ANCHOR.tp1_ratio}, "
          f"stop={ANCHOR_SESSION.stop_atr_pct}%, gap={ANCHOR_SESSION.min_gap_atr_pct}%")
    print(f"ORB=10m (20:00-20:10), entry≤01:00, flat=06:45, ATR={ANCHOR.atr_length}, "
