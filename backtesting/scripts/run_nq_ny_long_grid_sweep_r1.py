#!/usr/bin/env python3
"""NQ NY Continuation — Grid Sweep R1 (Longs-Only Fresh Start).

4D grid around converged R7 anchor:
  stop=[7,8,8.5,9,9.5,10], rr=[2.5,2.75,3.0,3.25,3.5], gap=[2.0,2.5,3.0], tp1=[0.45,0.5,0.55,0.6,0.65,0.7]
  Total: 6 × 5 × 3 × 6 = 540 combos

Structural params held fixed:
  ORB: 09:30-09:50 (20m), entry until 12:00, flat 15:30-16:00
  ATR=20, ICF=ON, direction=long, continuation, 1s magnifier
  DOW gate: excl Fri
"""
...
# Grid dimensions
STOP_VALUES = [7.0, 8.0, 8.5, 9.0, 9.5, 10.0]
RR_VALUES = [2.5, 2.75, 3.0, 3.25, 3.5]
GAP_VALUES = [2.0, 2.5, 3.0]
TP1_VALUES = [0.45, 0.5, 0.55, 0.6, 0.65, 0.7]

DOW_EXCL = {4}  # excl Friday

# Base session config (stop and gap will be varied)
BASE_SESSION = SessionConfig(
    ...
    flat_start="15:30",
    flat_end="16:00",
    stop_atr_pct=9.0,       # will be overridden
    min_gap_atr_pct=2.5,    # will be overridden
)

# Base strategy config (rr and tp1 will be varied)
BASE_STRATEGY = StrategyConfig(
    ...
    rr=3.0,                  # will be overridden
    tp1_ratio=0.6,           # will be overridden
    atr_length=20,
    impulse_close_filter=True,
    name="NQ NY Long Grid R1",
)

# Anchor values (for ranking reference)
ANCHOR_STOP = 9.0
ANCHOR_RR = 3.0
ANCHOR_GAP = 2.5
ANCHOR_TP1 = 0.6
...
def main():
    ...
    print(f"NQ NY ORB — Grid Sweep R1 (Longs-Only)")
    print(f"=" * 100)
    print(f"Grid: stop={STOP_VALUES} × rr={RR_VALUES} × gap={GAP_VALUES} × tp1={TP1_VALUES}")
    print(f"Total combos: {total_combos}")
    print(f"Structural: 20m ORB, entry<=12:00, flat=15:30, ATR=20, ICF=ON, excl Fri, long-only")
    ...
    for stop, rr, gap, tp1 in combos:
        ...
        # Hard constraint: 10-tick minimum stop
    ...
    # -- Decision: grid winner vs anchor Calmar delta --------------------------
    if zero_neg:
