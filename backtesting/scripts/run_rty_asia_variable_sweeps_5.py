#!/usr/bin/env python3
"""RTY Asia Continuation — Variable Sweeps Round 5.

R1 adopted: direction=long
R2 adopted: excl Tue
R3 adopted: stop=4.0%, tp1=0.4
R4 adopted: entry_end=21:00, tp1=0.6 (overrides R3 tp1), min_gap_atr=2.5%

R5 anchor:
  ORB: 20:00-20:15 (15m), entry until 21:00, flat 06:45-07:00
  stop=4.0%, min_gap_atr=2.5%, max_gap_pts=50, max_gap_atr=0 (off)
  rr=2.0, tp1=0.6, ATR=14, direction=LONG, ICF=OFF, continuation, 1s magnifier
  DOW gate: excl Tue
...
ANCHOR_SESSION = SessionConfig(
    ...
    orb_end="20:15",
    entry_start="20:15",
    entry_end="21:00",         # ← R4 adoption
    flat_start="06:45",
    flat_end="07:00",
    stop_atr_pct=4.0,           # R3
    min_gap_atr_pct=2.5,        # ← R4 adoption
)
...
ANCHOR = StrategyConfig(
    ...
    rr=2.0,
    tp1_ratio=0.6,              # ← R4 adoption (overrides R3 tp1=0.4)
    atr_length=14,
    impulse_close_filter=False,
    name="RTY Asia R5 Anchor",
)
...
def main():
    print("RTY Asia ORB — Round 5: entry_end=21:00 + tp1=0.6 + gap=2.5% from R4")
    print("=" * 90)
    print(f"Anchor: rr={ANCHOR.rr}, tp1={ANCHOR.tp1_ratio}, "
          f"stop={ANCHOR_SESSION.stop_atr_pct}%, gap={ANCHOR_SESSION.min_gap_atr_pct}%")
    print(f"ORB=15m (20:00-20:15), entry≤21:00, flat=06:45, ATR={ANCHOR.atr_length}, "
    ...
    print(f"      *** TRADE COUNT CHECK: {m_anchor['total_trades']} trades ***")

    # ── 1. STOP ATR % ────────────────────────────────────────────────
    stop_values = [2.0, 3.0, 3.5, 4.0, 4.5, 5.0, 6.0, 7.0]
    ...
    # ── 4. ENTRY END TIME ────────────────────────────────────────────
    entry_ends = ["20:30", "20:45", "21:00", "21:30", "22:00", "23:00", "23:15"]
    print_header("4. ENTRY END TIME (anchor=21:00)")
    for i, ee in enumerate(entry_ends, 1):
        sess = replace(ANCHOR_SESSION, entry_end=ee)
        config = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"end={ee}", m, is_base=(ee == "21:00"))
        print_years(m)
        all_results.append(("entry_end", ee, m))

    # ── 5. FLAT START ────────────────────────────────────────────────
    flat_starts = ["02:00", "04:00", "06:00", "06:45"]
    ...
    print_header("7. TP1 RATIO (anchor=0.6)")
    ...
    print_header("8. MIN GAP ATR % (anchor=2.5%)")
    for i, g in enumerate(gap_values, 1):
        sess = replace(ANCHOR_SESSION, min_gap_atr_pct=g)
        config = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"gap={g}%", m, is_base=(abs(g - 2.5) < 0.01))
    ...
    # ── 10. ICF ───────────────────────────────────────────────────────
    print_header("10. IMPULSE CLOSE FILTER (anchor=OFF)")
    ...
    dimension_order = [
        ...
        "rr", "tp1_ratio", "min_gap_atr_pct", "dow_exclusion", "icf",
    ]
