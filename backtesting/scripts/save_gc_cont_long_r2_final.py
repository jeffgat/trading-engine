...
WF mode params (use for live trading):
  ...
  ATR 10, 10m ORB, entry→11:00, max_gap_atr=30%, FOMC excluded
  Long-only, 1s magnifier
"""
...
NY_SESSION = SessionConfig(
    ...
    stop_atr_pct=3.0,       # WF mode (tighter than structural 4.0%)
    min_gap_atr_pct=3.5,
)

CONFIG = StrategyConfig(
    rr=4.5,                  # WF mode (structural 4.0, WF selects 4.5)
    tp1_ratio=0.5,
    risk_usd=5000.0,
    atr_length=10,
    min_qty=1.0,
    ...
    name="GC NY Cont Longs R2 Final (WF Mode)",
    notes="GO config — R2 pipeline all 5 phases PASS. WF mode params for live trading.",
)
