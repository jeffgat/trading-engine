#!/usr/bin/env python3
"""ES LDN Continuation Both — Variable Sweeps Round 5 (Full Optimization Step 2).

Updated anchor from R4: stop=6.0% (was 10.0)
Structural: rr=2.5, tp1=0.5, ATR=3, gap=1.5%, ORB 15m, flat 08:20, both, DOW excl Mon, ICF off.
Stop sweep floor: 5.0% (10 ticks).

ATR and gap are oscillating. If they flip again this round → CONVERGE and grid sweep.
Grid will include ATR (3/20/50) and gap (1.0/1.5) as dimensions to resolve the interaction.
"""
...
if __name__ == "__main__":
    print("=" * 90)
    print("  ES LDN CONTINUATION BOTH — VARIABLE SWEEPS ROUND 5")
    print("=" * 90)
    ...
    print(f"  R4 adoption: stop 10→6%")
    print(f"  ATR history: 14→3→20→3→? | Gap history: 1.0→1.5→1.0→1.5→?")
    print(f"  If ATR/gap flip again → CONVERGE → grid sweep with ATR+gap as dims")
    print()
    ...
    # Summary
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY — Variable Sweeps Round 5")
    ...
    for dim_name, info in all_sweeps.items():
        ...
        # Block oscillating dimensions
        if dim_name == "ATR Length" and str(info["best_val"]) != str(info["anchor_val"]) and delta > 0.3:
            oscillating.append(f"ATR wants {info['best_val']} (was {info['anchor_val']})")
            adopt = False
        if dim_name == "Min Gap ATR%" and str(info["best_val"]) != str(info["anchor_val"]) and delta > 0.3:
            oscillating.append(f"Gap wants {info['best_val']}% (was {info['anchor_val']}%)")
            adopt = False
    ...
    if oscillating:
        print(f"\n  OSCILLATION DETECTED (blocked from adoption):")
        for osc in oscillating:
            print(f"    {osc}")

    print(f"\n  Non-oscillating adoptions: {len(adoptions)}")
    for dim, val, cal in adoptions:
        print(f"    {dim}: {val} (Calmar {cal:.2f})")

    if adoptions:
        print(f"\n  >>> {len(adoptions)} adoption(s) — update anchor and re-sweep (round 6)")
    else:
        print(f"\n  >>> CONVERGED. Ready for grid sweep.")
        print(f"\n  Grid sweep config:")
        print(f"    Structural: ORB 15m, flat 08:20, entry_end 08:25, both dirs, DOW excl Mon, ICF off")
        print(f"    Grid dims: stop × rr × gap × tp1 × ATR")
        print(f"    Stop: [5.0, 6.0, 7.5, 10.0]")
        print(f"    RR: [2.0, 2.5, 3.0]")
        print(f"    Gap: [1.0, 1.5, 2.0]")
        print(f"    TP1: [0.4, 0.5, 0.6]")
        print(f"    ATR: [3, 14, 20, 50]")
