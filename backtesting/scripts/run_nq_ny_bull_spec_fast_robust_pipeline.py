#!/usr/bin/env python3
"""NQ NY BULL SPECIALIST FAST — 5-Phase Robust Pipeline (Bailey-aware).

FAST execution config NQ_NY_BULL_SPECIALIST leg:
...
# ── Candidate config (FAST execution NQ_NY_BULL_SPECIALIST) ─────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ NY Bull Specialist FAST Robust Pipeline",
)
...
def phase_4(df, df_1m, df_1s, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST NQ_NY_BULL_SPECIALIST robust pipeline",
    )
    if entry.warning:
