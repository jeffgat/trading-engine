#!/usr/bin/env python3
"""NQ NY Continuation — Grid Sweep R3 (Longs-Only) — Batched version.

Same grid as r3, but saves intermediate results to JSON to survive crashes.
...
DATA_YEARS = 10
MIN_TP1_RATIO = 0.2
CHECKPOINT_FILE = Path("/tmp/nq_ny_grid_r3_checkpoint.json")

STOP_VALUES = [6.0, 6.5, 7.0, 7.5, 8.0]
RR_VALUES = [3.0, 3.25, 3.5, 3.75, 4.0]
GAP_VALUES = [2.0, 2.5, 3.0]
TP1_VALUES = [0.3, 0.35, 0.4, 0.45, 0.5]

DOW_EXCL = {4}

BASE_SESSION = SessionConfig(
    name="NY", orb_start="09:30", orb_end="09:50", entry_start="09:50",
...
BASE_STRATEGY = StrategyConfig(
    ...
    rr=3.5, tp1_ratio=0.4, atr_length=12, impulse_close_filter=False,
    name="NQ NY Long Grid R3",
)
...
def save_checkpoint(results, done, skipped_ticks, skipped_tp1):
    data = {
        "results": results,
        "done": done,
        "skipped_ticks": skipped_ticks,
        "skipped_tp1": skipped_tp1,
    }
...
def load_checkpoint():
    if CHECKPOINT_FILE.exists():
        ...
        return data["results"], data["done"], data["skipped_ticks"], data["skipped_tp1"]
    return [], 0, 0, 0
...
def main():
    ...
    results, start_done, skipped_ticks, skipped_tp1 = load_checkpoint()
    if start_done > 0:
        print(f"Resuming from combo {start_done}/{total_combos} ({len(results)} results so far)", flush=True)

    print(f"NQ NY ORB — Grid Sweep R3 (Longs-Only)", flush=True)
    print(f"=" * 100, flush=True)
    print(f"Grid: stop={STOP_VALUES} × rr={RR_VALUES} × gap={GAP_VALUES} × tp1={TP1_VALUES}", flush=True)
    print(f"Total combos: {total_combos}", flush=True)
    print(f"Structural: 20m ORB, entry<=12:00, flat=15:30, ATR=12, ICF=OFF, excl Fri, long-only", flush=True)
    print(f"Anchor: stop={ANCHOR_STOP}, rr={ANCHOR_RR}, gap={ANCHOR_GAP}, tp1={ANCHOR_TP1}", flush=True)
    ...
    print(f"  5m: {len(df_5m):,} | 1m: {len(df_1m) if df_1m is not None else 0:,} | "
          f"1s: {len(df_1s) if df_1s is not None else 0:,} [{time.time()-t0:.1f}s]", flush=True)
    ...
    for stop, rr, gap, tp1 in combos:
        ...
        results.append({
            ...
            "med_ticks": med_ticks,
            "r_by_year": {str(k): v for k, v in m.get("r_by_year", {}).items()},
        })

        if done % 25 == 0:
            save_checkpoint(results, done, skipped_ticks, skipped_tp1)
            ...
            print(f"  [{done}/{total_combos}] {elapsed:.0f}s elapsed, "
                  ...
                  f"results={len(results)}, skip_ticks={skipped_ticks}, skip_tp1={skipped_tp1}", flush=True)

    # Final save
    save_checkpoint(results, total_combos, skipped_ticks, skipped_tp1)

    elapsed = time.time() - t0
    print(f"\n{'='*100}", flush=True)
    print(f"  GRID COMPLETE — {len(results)} valid results from {total_combos} combos "
          f"({skipped_ticks} skipped <10 ticks, {skipped_tp1} skipped tp1<{MIN_TP1_RATIO})", flush=True)
    print(f"  Runtime: {elapsed:.0f}s ({elapsed/60:.1f}m)", flush=True)
    print(f"{'='*100}", flush=True)

    results.sort(key=lambda x: x["calmar"], reverse=True)

    # -- Top 20 by Calmar (all combos)
    print(f"\n  TOP 20 BY CALMAR (all combos):", flush=True)
    print(f"  {'Rank':>4} {'Stop':>5} {'RR':>5} {'Gap':>5} {'TP1':>5} "
          f"{'Trades':>6} {'WR':>5} {'PF':>5} {'Sharpe':>6} {'Net R':>7} "
          f"{'R/yr':>6} {'MaxDD':>6} {'Calmar':>7} {'Neg':>4} {'Neg Yrs'}", flush=True)
    ...
    for i, r in enumerate(results[:20], 1):
        ...
        print(f"  {i:>4} {r['stop']:>5.1f} {r['rr']:>5.2f} {r['gap']:>5.1f} {r['tp1']:>5.2f} "
              ...
              f"{r['n_neg']:>4} {neg_str}{marker}", flush=True)

    # -- Top 20 by Calmar (0 neg years only)
    zero_neg = [r for r in results if r["n_neg"] == 0]
    print(f"\n  TOP 20 BY CALMAR (0 negative full years only — {len(zero_neg)}/{len(results)} combos):", flush=True)
    print(f"  {'Rank':>4} {'Stop':>5} {'RR':>5} {'Gap':>5} {'TP1':>5} "
          f"{'Trades':>6} {'WR':>5} {'PF':>5} {'Sharpe':>6} {'Net R':>7} "
          f"{'R/yr':>6} {'MaxDD':>6} {'Calmar':>7}", flush=True)
    print(f"  {'----'*20}", flush=True)
    for i, r in enumerate(zero_neg[:20], 1):
        ...
        print(f"  {i:>4} {r['stop']:>5.1f} {r['rr']:>5.2f} {r['gap']:>5.1f} {r['tp1']:>5.2f} "
              f"{r['trades']:>6} {r['wr']:>5.1%} {r['pf']:>5.2f} {r['sharpe']:>6.2f} "
              f"{r['net_r']:>7.1f} {r['r_yr']:>6.1f} {r['max_dd']:>6.1f} {r['calmar']:>7.2f}{marker}", flush=True)

    # -- Top 5 year-by-year breakdown
    print(f"\n  TOP 5 (0 neg years) — Year-by-Year Breakdown:", flush=True)
    for i, r in enumerate(zero_neg[:5], 1):
        ...
        print(f"  #{i} stop={r['stop']}, rr={r['rr']}, gap={r['gap']}, tp1={r['tp1']} "
              f"| Calmar={r['calmar']:.2f}, Net R={r['net_r']:.1f}, DD={r['max_dd']:.1f}", flush=True)
        print(f"     {yr_str}", flush=True)
    ...
    print(f"\n  ANCHOR RANK:", flush=True)
    print(f"    Overall: #{anchor_rank_all}/{len(results)} (stop={ANCHOR_STOP}, rr={ANCHOR_RR}, "
          f"gap={ANCHOR_GAP}, tp1={ANCHOR_TP1})", flush=True)
    print(f"    0-neg-years: #{anchor_rank_zero}/{len(zero_neg)}", flush=True)

    # -- Grid summary
    n_profitable = sum(1 for r in results if r["net_r"] > 0)
    print(f"\n  GRID SUMMARY:", flush=True)
    print(f"    Total combos tested: {len(results)}", flush=True)
    print(f"    Skipped (<10 tick stop): {skipped_ticks}", flush=True)
    print(f"    Skipped (tp1 < {MIN_TP1_RATIO}): {skipped_tp1}", flush=True)
    print(f"    With 0 neg years: {len(zero_neg)} ({100*len(zero_neg)/len(results):.0f}%)", flush=True)
    print(f"    Profitable: {n_profitable} ({100*n_profitable/len(results):.0f}%)", flush=True)
    ...
    if zero_neg:
        ...
        if anchor_entry:
            ...
            print(f"    Grid winner (0-neg): stop={grid_winner['stop']}, rr={grid_winner['rr']}, "
                  f"gap={grid_winner['gap']}, tp1={grid_winner['tp1']} | Calmar={grid_winner['calmar']:.2f}", flush=True)
            print(f"    Anchor: Calmar={anchor_entry['calmar']:.2f}", flush=True)
            print(f"    Delta: {delta:+.2f}", flush=True)
            if delta > 0.5:
                print(f"    => ADOPT grid winner (delta > 0.5) — return to variable sweeps", flush=True)
            else:
                print(f"    => KEEP anchor (delta <= 0.5) — proceed to robust pipeline", flush=True)

    # Cleanup checkpoint
    if CHECKPOINT_FILE.exists():
        CHECKPOINT_FILE.unlink()
        print("\n  Checkpoint file cleaned up.", flush=True)
