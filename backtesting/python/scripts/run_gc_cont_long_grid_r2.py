#!/usr/bin/env python3
"""GC NY Continuation Longs — Grid Sweep R2 (clean data, 1s magnifier).

Anchor (converged after R4 variable sweeps):
  stop=4.0%, rr=4.0, min_gap=3.5%, tp1=0.5
...
Structural params fixed: ATR 10, 10m ORB, entry→11:00, max_gap_atr=30%, FOMC excl.
"""
...
BASE = StrategyConfig(
    rr=4.0, tp1_ratio=0.5, risk_usd=5000.0, atr_length=10,
    min_qty=1.0, qty_step=1.0,
    sessions=(make_session(4.0, 3.5),),
    instrument=GC,
...
if __name__ == "__main__":
    print()
    print("=" * 70)
    print("  GC NY CONT LONGS — GRID SWEEP R2 (clean data, 1s magnifier)")
    print("  Fixed: ATR 10 | 10m ORB | entry→11:00 | max_gap_atr=30% | FOMC excl")
    ...
    print("  " + "-" * 100)
    for r in scored[:20]:
        anchor = "  <- anchor" if (r["stop"] == 4.0 and r["rr"] == 4.0 and r["min_gap"] == 3.5 and r["tp1"] == 0.5) else ""
        print(
    ...
    # Check if anchor is in top 3
    anchor_rank = next(
        (i for i, r in enumerate(scored)
         if r["stop"] == 4.0 and r["rr"] == 4.0 and r["min_gap"] == 3.5 and r["tp1"] == 0.5),
        -1,
    )
