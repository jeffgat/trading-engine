#!/usr/bin/env python3
"""YM ORB Discovery — 3-session sweep with 1m magnifier.

Starting thesis: ORB continuation on YM across NY, Asia, LDN sessions.
Baseline: RR=2.0, TP1=0.5, stop=10% ATR, min_gap=1% ATR.
Sweeps ATR stops, ORB-range stops, both directions, multiple ORB windows.

Pre-holdout only (<2025-01). Uses 1m magnifier (no 1s data for YM).

Previous YM learnings tested only continuation (4 directional combos) + LSI.
This sweep re-examines continuation from scratch across all 3 sessions with
a broader parameter grid (same starting point as recent GC discovery flow).
"""
...
OUTPUT_DIR = ROOT / "data" / "results" / "ym_orb_discovery_3session"
    ...
    for orb_min in ORB_MINUTES:
        for stop_mode in ["atr", "orb"]:
            ...
            base = StrategyConfig(
                ...
                tp1_ratio=0.5,
                atr_length=14,
                name=f"YM {session_name} ORB{orb_min}m {stop_mode}-stop",
            )
...
def main():
    print("YM ORB Discovery — 3-Session Sweep (1m magnifier)")
    print("=" * 70)
    ...
    print(f"Bar magnifier: ON (1m) — no 1s data for YM")
