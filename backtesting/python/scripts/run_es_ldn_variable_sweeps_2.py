#!/usr/bin/env python3
"""ES LDN Continuation Both — Variable Sweeps Round 2 (Full Optimization Step 2).

Updated anchor from R1 adoptions:
  stop=10.0% (KEPT — 3.0% violated 10-tick minimum floor), ATR=3 (was 14), gap=1.5% (was 1.0)
  Everything else unchanged: rr=2.5, tp1=0.5, ORB 15m, flat 08:20, both, ICF off.
...
# Anchor — updated from R1 adoptions
ANCHOR_SESSION = SessionConfig(
    ...
    flat_start="08:20",
    flat_end="08:25",
    stop_atr_pct=10.0,      # KEPT — 3.0% violated 10-tick floor
    min_gap_atr_pct=1.5,    # R1: 1.0 → 1.5
)
...
def sweep_stop(df, df_1m, df_1s):
    values = [5.0, 6.0, 7.5, 10.0, 12.0, 15.0]  # floor 5% = 10 ticks at ATR ~50
    results = []
    for v in values:
...
if __name__ == "__main__":
    print("=" * 90)
    print("  ES LDN CONTINUATION BOTH — VARIABLE SWEEPS ROUND 2")
    print("=" * 90)
    ...
    print(f"  R1 adoptions: ATR 14→3, gap 1.0→1.5% (stop 3% rejected — 10-tick floor)")
    print(f"  Stop sweep floor: 5.0% (≥10 ticks at typical ATR ~50)")
    print()
    ...
    # Summary
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY — Variable Sweeps Round 2")
    ...
    if adoptions:
        print(f"\n  >>> {len(adoptions)} adoption(s) — update anchor and re-sweep (round 3)")
    else:
