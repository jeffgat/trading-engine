#!/usr/bin/env python3
"""GC NY Continuation Longs — R3 Round 3 Variable Sweeps (post fill-bar fix).

R2 adopted: direction=both (Calmar +3.46), max_gap_atr=15% (Calmar +1.44).
...
Anchor:
  ...
  max_gap_points=25.0, max_gap_atr=15%
  Both directions, 1s magnifier, FOMC dates excluded

Evolution: fill-bar fix baseline → R1 (5m ORB, ATR 5) → R2 (both dirs, gap_atr 15%)
...
# ── Anchor config (R2 adopted: both dirs + max_gap_atr=15%) ─────────────────

GC_NY_ANCHOR = SessionConfig(
    name="NY",
    orb_start="09:30",
    orb_end="09:35",          # 5m ORB (adopted R1)
    entry_start="09:35",
    entry_end="11:00",
    flat_start="15:50",
    flat_end="16:00",
    ...
    min_gap_atr_pct=3.5,
    max_gap_points=25.0,
    max_gap_atr_pct=15.0,     # Adopted R2 (was 30%)
)
...
ANCHOR = StrategyConfig(
    ...
    direction_filter="both",   # Adopted R2 (was long)
...
def make_session(**overrides):
    defaults = dict(
        ...
        max_gap_points=25.0, max_gap_atr_pct=15.0,
    )
...
# ── Sweep 8: max_gap_atr_pct (fine-grained around 15%) ──────────────────────

def sweep_max_gap_atr_pct(anchor_calmar):
    section("SWEEP 8: MAX GAP ATR % (fine-grained around 15%)")
    header()
    values = [0.0, 10.0, 12.0, 15.0, 18.0, 20.0, 25.0, 30.0]
    results = []
    for v in values:
        label = (f"max_gap_atr={v:.0f}%" + (" [anchor]" if v == 15.0 else "")) if v > 0 else "off"
...
def print_anchor_metrics(m):
    section("ANCHOR METRICS (R2 adopted: both dirs + max_gap_atr=15%)")
    print(f"  Config: stop=3.0% | rr=4.5 | min_gap=3.5% | tp1=0.5 | ATR 5")
    print(f"          5m ORB | entry→11:00 | both dirs | FOMC excl | max_gap_atr=15% | 1s")
    print(f"  Evolution: baseline → R1 (5m ORB, ATR 5) → R2 (both, gap_atr 15%)")
    print()
...
if __name__ == "__main__":
    print()
    print("=" * 70)
    print("  GC NY CONT LONGS — R3 VARIABLE SWEEPS Round 3 (post fill-bar fix)")
    print("  Anchor: stop=3.0% | rr=4.5 | min_gap=3.5% | tp1=0.5 | ATR 5")
    print("          5m ORB | entry→11:00 | FOMC excl | both dirs | max_gap_atr=15%")
    print("  Evolution: baseline → R1 (5m ORB, ATR 5) → R2 (both, gap_atr 15%)")
    print("=" * 70)
    ...
    print("  If anchor changes, re-run sweeps on new anchor (R3 sweeps_4).")
    print("=" * 70)
    print()
