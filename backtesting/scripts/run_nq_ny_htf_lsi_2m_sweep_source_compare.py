#!/usr/bin/env python3
"""Compare HTF-LSI sweep-source variants on the frozen NQ NY 2m anchor."""
...
OUTPUT_DIR = RESULTS_ROOT / "nq_ny_htf_lsi_2m_sweep_source_compare"
REPORT_PATH = ROOT / "learnings" / "reports" / "NQ_NY_HTF_LSI_2M_SWEEP_SOURCE_COMPARE.md"
    ...
    return [
        build_config(
            **base_kwargs,
            ...
            name="NQ NY HTF_LSI 2m anchor sweep-source htf_only",
        ),
        build_config(
            **base_kwargs,
            htf_lsi_include_htf_levels=False,
            htf_lsi_reference_levels=REFERENCE_LEVELS,
            name="NQ NY HTF_LSI 2m anchor sweep-source reference_only",
        ),
        build_config(
            **base_kwargs,
            htf_lsi_include_htf_levels=True,
            htf_lsi_reference_levels=REFERENCE_LEVELS,
            name="NQ NY HTF_LSI 2m anchor sweep-source htf_plus_reference",
        ),
    ]
...
def _source_breakdown(trades) -> dict:
    ...
    return {
        ...
        "pre_holdout_reference_levels": dict(reference_counts),
        "validation_reference_levels": dict(validation_reference_counts),
    }
...
def _write_report(rows: list[dict]) -> None:
    lines = [
        ...
        f"- Reference/session basket: `{', '.join(REFERENCE_LEVELS)}`.",
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"{src['pre_holdout_reference_trades']} | {src['validation_filled_trades']} | "
            f"{src['validation_htf_trades']} | {src['validation_reference_trades']} |"
        )

    lines.extend(["", "## Reference-Level Breakdown", ""])
    for row in rows:
        lines.append(f"### {row['variant']}")
        lines.append("")
        pre_counts = row["source_breakdown"]["pre_holdout_reference_levels"]
        val_counts = row["source_breakdown"]["validation_reference_levels"]
        if not pre_counts and not val_counts:
            lines.append("No reference-driven filled trades.")
            lines.append("")
            continue
...
def main() -> int:
    ...
    print("Running NQ NY HTF-LSI 2m sweep-source comparison...", flush=True)
    results = run_sweep(
        df_base,
        configs,
