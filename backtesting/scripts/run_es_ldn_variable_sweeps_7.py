#!/usr/bin/env python3
"""ES LDN Continuation Both — Variable Sweeps Round 7 (Full Optimization Step 2).

R6 adoption: gap 1.5→1.0% (Calmar 3.93, 0 neg years).
Anchor: stop=6%, ORB 10m, rr=2.5, tp1=0.5, ATR=3, gap=1.0%,
  flat 08:20, entry_end 08:25, both, DOW excl Mon, ICF off.
"""
...
ANCHOR = StrategyConfig(
    rr=2.5, tp1_ratio=0.5, risk_usd=5000.0, atr_length=3,
...
def sweep_orb(df, df_1m, df_1s):
    results = []
    for label, orb_s, orb_e, es in [("5m","03:00","03:05","03:05"),("10m","03:00","03:10","03:10"),
        ("15m","03:00","03:15","03:15"),("20m","03:00","03:20","03:20"),("30m","03:00","03:30","03:30"),("45m","03:00","03:45","03:45")]:
        sess = replace(ANCHOR_SESSION, orb_start=orb_s, orb_end=orb_e, entry_start=es)
        _, m = run_and_measure(df, replace(ANCHOR, sessions=(sess,)), df_1m, df_1s)
...
def sweep_atr(df, df_1m, df_1s):
    ...
    for v in [3, 5, 7, 10, 14, 20, 30, 50]:
        ...
        results.append((v, m))
    print_sweep_table(results, "ATR Length", 3); return results

def sweep_entry_end(df, df_1m, df_1s):
    results = []
    for v in ["05:00","06:00","06:30","07:00","07:30","08:00","08:25"]:
...
def sweep_rr(df, df_1m, df_1s):
    ...
    for v in [1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0]:
        ...
        results.append((v, m))
    print_sweep_table(results, "R:R", 2.5); return results
...
def sweep_max_gap(df, df_1m, df_1s):
    values = [("OFF",0,0),("20pt",20,0),("50pt",50,0),("75pt",75,0),("100pt",100,0),
              ("20%ATR",0,20),("50%ATR",0,50),("100%ATR",0,100),("150%ATR",0,150)]
    results = []
...
if __name__ == "__main__":
    print("=" * 90)
    print("  ES LDN CONTINUATION BOTH — VARIABLE SWEEPS ROUND 7")
    print("=" * 90)
    print(f"  Anchor: stop=6%, rr=2.5, gap=1.0%, tp1=0.5, ATR=3, ORB 10m, flat 08:20")
    print(f"  Both dirs, DOW excl Mon, ICF off, 1s magnifier")
    print(f"  R6 adoption: gap 1.5→1.0%\n")
    ...
    sweeps = [
        ...
        ("ATR Length", 3, sweep_atr), ("Entry End", "08:25", sweep_entry_end),
        ("Flat Start", "08:20", sweep_flat), ("Direction", "both", sweep_dir),
        ("R:R", 2.5, sweep_rr), ("TP1 Ratio", 0.5, sweep_tp1),
    ...
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY — Variable Sweeps Round 7 ({total_time:.0f}s)")
    print(f"{'='*90}")
    ...
    for dim_name, info in all_sweeps.items():
        ...
        # Block ATR (unstable history)
        if dim_name == "ATR Length" and str(info["best_val"]) != str(info["anchor_val"]) and adopt:
            adopt = False; adopt_str = "OSCL"
        else:
    ...
    if not adoptions:
        print(f"\n  >>> CONVERGED after 7 rounds. Ready for grid sweep.")
        print(f"\n  Final converged anchor:")
        print(f"    stop=6%, rr=2.5, gap=1.0%, tp1=0.5, ATR=3")
        print(f"    ORB 10m, entry_end 08:25, flat 08:20")
        print(f"    Both dirs, DOW excl Mon, ICF off, 1s magnifier")
    else:
        print(f"\n  >>> {len(adoptions)} adoption(s)")
