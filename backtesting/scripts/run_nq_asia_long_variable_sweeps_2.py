#!/usr/bin/env python3
"""NQ Asia Long — Variable Sweeps Round 2 (Full ORB Anchor).

R2 anchor (R1 + adoptions):
  ORB: 20:00-20:15, entry until 22:30, flat 04:00-07:00
  stop_orb_pct=100%, min_gap_orb_pct=10% (was 20%)
  rr=3.0, tp1=0.6, ATR=5, direction=long, ICF=ON (was OFF), continuation, 1s magnifier
  DOW gate: excl Tue

R1 adoptions applied:
  min_gap_orb_pct: 20% -> 10% (+10.17 Calmar)
  ICF: OFF -> ON (+2.65 Calmar)
...
ANCHOR = StrategyConfig(
    ...
    name="NQ Asia Long R2 Anchor (full ORB)",
)
...
def main():
    ...
    # -- 2. MIN GAP ORB % -----------------------------------------------------
    gap_orb_vals = [5, 7, 10, 12, 15, 20, 25, 30]
