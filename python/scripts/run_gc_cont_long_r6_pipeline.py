#!/usr/bin/env python3
"""Robust pipeline: GC NY Continuation Longs — R6 config (1s magnifier).

Stabilized config after 5 rounds of variable sweeps + FOMC diagnostic + grid sweep:
...
GC_NY = SessionConfig(
    ...
    entry_start="09:40",
    entry_end="11:00",      # Adopted Round 3
    flat_start="15:50",
    flat_end="16:00",
    stop_atr_pct=4.0,
    min_gap_atr_pct=2.5,
...
MIN_TRADES        = 400
MIN_PF            = 1.1
MIN_WF_EFFICIENCY = 0.3   # 0.36 is marginal (not overfit); 0.4 was too conservative
MIN_STABILITY     = 0.4
MIN_FOLDS         = 4
...
def phase1(df, df_1m, df_1s):
    ...
    print(f"  Config: stop=4.0% | rr=4.5 | min_gap=2.5% | tp1=0.5 | ATR 16 | 10m ORB | entry→11:00 | FOMC excl")
...
def phase4(df, df_1m, df_1s, stability):
    banner(4, "HOLD-OUT OOS TEST (2025-01-01 → 2026-02-15, 1s magnifier)")
...
def main():
    print()
    print("=" * 70)
    print("  ROBUST PIPELINE: GC NY Cont Longs R6 (1s magnifier)")
    print("  stop=4.0% | rr=4.5 | min_gap=2.5% | tp1=0.5 | ATR 16 | 10m ORB")
    print("  entry→11:00 | FOMC excluded")
    print(f"  WF sweep: {GRID_SIZE} combos/fold (4×2×5×4)")
    print("=" * 70)
    print()
    ...
    # Note: WF efficiency 0.36 is marginal (0.3-0.5 band), not overfit. Continue to phases 3-5.
