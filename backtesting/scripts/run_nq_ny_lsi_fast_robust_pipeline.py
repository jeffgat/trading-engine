#!/usr/bin/env python3
"""NQ NY LSI FAST — 5-Phase Robust Pipeline (Bailey-aware).

FAST execution config NQ_NY_LSI leg:
  ORB: 09:30-09:45, entry 09:35-15:30, flat 15:50-16:00
  min_gap=5.0%, rr=3.0, tp1=0.3
...
# ── Candidate config (FAST execution NQ_NY_LSI) ────────────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ NY LSI FAST Robust Pipeline",
)

# ── Walk-forward param ranges (100 combos/fold) ────────────────────────────
PARAM_RANGES = {
    "rr": [2.0, 2.5, 3.0, 3.5, 4.0],
    "tp1_ratio": [0.2, 0.25, 0.3, 0.35],
    "ny_min_gap_atr_pct": [3.5, 4.0, 4.5, 5.0, 5.5],
}
...
def phase_4(df, df_1m, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST NQ_NY_LSI robust pipeline",
    )
    if entry.warning:
...
def main():
    ...
    print(f"  Structural: LSI strategy, entry 09:35-15:30, flat 15:50, ATR={CANDIDATE.atr_length}, "
