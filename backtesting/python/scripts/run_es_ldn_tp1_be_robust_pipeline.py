#!/usr/bin/env python3
"""ES London ORB Continuation (Both) — Robust Pipeline, BE offset=0.

Instrument: ES (E-mini S&P 500), continuation strategy (both directions), bar magnifier.
Session: London (03:00-03:15 ET ORB, entries until 08:25, flat by 08:25).
BE offset: 0 ticks (fixed).

Candidate params (best by Sharpe from prior sweeps, be=0):
  rr=3.0, stop_atr_pct=1.5, min_gap_atr_pct=2.0, tp1_ratio=0.5

Phases:
  1. Structural validation (full history, candidate params)
...
PROP_CONSTRAINTS = PropFirmConstraints(
    max_drawdown_r=999.0,   # DD is NOT a hard filter — removed as pass/fail criterion
    min_annual_r=24.0,
    max_monthly_loss_r=5.0,
    min_positive_expectancy=True,
)

# Walk-forward sweep: centered around best candidate (be=0 fixed throughout)
PARAM_RANGES = {
    "rr": [2.0, 2.5, 3.0, 3.5],
    "ldn_stop_atr_pct": [1.5, 2.0, 2.5],
    "ldn_min_gap_atr_pct": [1.0, 1.25, 1.5, 2.0],
    "tp1_ratio": [0.20, 0.30, 0.40, 0.50],
}
...
def main():
    # ── Load data ──────────────────────────────────────────────────────
    separator("Loading ES data (5m + 1m for magnifier)")
    ...
    # Base config: ES LDN continuation, both directions, magnifier, be=0
    base_config = StrategyConfig(
        ...
        rr=3.0,
        tp1_ratio=0.5,
        name="ES LDN Continuation Both Robust Pipeline",
    )
    base_config = with_overrides(base_config, ldn_stop_atr_pct=1.5, ldn_min_gap_atr_pct=2.0)

    # ==================================================================
    # PHASE 1: Structural Validation
    # ==================================================================
    separator("PHASE 1: Structural Validation")
    print("Running full-history backtest with candidate params + magnifier...")
    print(f"Params: rr=3.0, stop=1.5%, gap=2.0%, tp1=0.5, be=0, both directions, risk=$5000")
    ...
    print("-" * 66)
    print(f"{'Max drawdown (info only)':<28} {cr.max_drawdown_r:>12.1f}R {'n/a':>12}  {'INFO':>8}")
    ...
    MC_RUIN_THRESHOLD = 25.0  # informational ruin level for MC — not a hard pass/fail filter
    ...
    p5_survival_ok = mc_surv["survival_rate"] >= 0.70  # survival = monthly loss + annual R (no DD gate)
