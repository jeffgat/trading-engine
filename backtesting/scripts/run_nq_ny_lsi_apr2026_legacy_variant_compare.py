#!/usr/bin/env python3
"""Compare corrected LSI sweep semantics vs legacy/live-matched variants.

Study design:
1. Reproduce the old backtest bug explicitly (`rth`-gated, non-consumptive).
2. Test the live-matched bug variant (`entry`-gated, non-consumptive).
3. Compare both against the corrected `08:30-14:30 ET` sweep-window logic.
4. Re-sweep `lsi_n_left` on the live-matched legacy branch.
5. Re-apply DOW filtering post hoc on the best live-matched branch.
"""
...
def main() -> None:
    ...
    corrected_frozen = replace(
        BASE_CONFIG,
        sessions=(CORRECTED_SESSION,),
        excluded_days=(2, 3),
        name="NQ NY 2016-2025 ALPHA_LSI corrected frozen",
    )
    corrected_all_days = replace(
        BASE_CONFIG,
        sessions=(CORRECTED_SESSION,),
        excluded_days=(),
        name="NQ NY 2016-2025 ALPHA_LSI corrected allDays",
    )
    legacy_backtest_frozen = replace(
        BASE_CONFIG,
        excluded_days=(2, 3),
        lsi_sweep_gate="rth",
        lsi_stale_breach_consumes_pivot=False,
        name="NQ NY 2016-2025 ALPHA_LSI legacyBacktest frozen",
    )
    legacy_backtest_all_days = replace(
        BASE_CONFIG,
        excluded_days=(),
        lsi_sweep_gate="rth",
        lsi_stale_breach_consumes_pivot=False,
        name="NQ NY 2016-2025 ALPHA_LSI legacyBacktest allDays",
    )
    legacy_live_frozen = replace(
        BASE_CONFIG,
        excluded_days=(2, 3),
        lsi_sweep_gate="entry",
        lsi_stale_breach_consumes_pivot=False,
        name="NQ NY 2016-2025 ALPHA_LSI legacyLive frozen",
    )
    legacy_live_all_days = replace(
        BASE_CONFIG,
        excluded_days=(),
        lsi_sweep_gate="entry",
        lsi_stale_breach_consumes_pivot=False,
        name="NQ NY 2016-2025 ALPHA_LSI legacyLive allDays",
    )

    sweep_configs = [
        replace(
            legacy_live_all_days,
            lsi_n_left=value,
            name=f"NQ NY 2016-2025 ALPHA_LSI legacyLive allDays nleft{value}",
        )
    ...
    all_configs = [
        corrected_frozen,
        corrected_all_days,
        legacy_backtest_frozen,
        legacy_backtest_all_days,
        legacy_live_frozen,
        legacy_live_all_days,
        *sweep_configs,
    ]
    ...
    signal_cache = build_signal_cache(df_5m, all_configs)

    control_results: dict[str, tuple[dict, list]] = {}
    controls = [
        ("corrected_frozen", corrected_frozen),
        ("corrected_all_days", corrected_all_days),
        ("legacy_backtest_frozen", legacy_backtest_frozen),
        ("legacy_backtest_all_days", legacy_backtest_all_days),
        ("legacy_live_frozen", legacy_live_frozen),
        ("legacy_live_all_days", legacy_live_all_days),
    ]
    ...
    print("\nSweeping lsi_n_left on the live-matched legacy branch...", flush=True)
    ...
    for cfg in sweep_configs:
        ...
        row = summarize(f"legacyLive n_left={cfg.lsi_n_left}", cfg, trades)
    ...
    if best_initial["lsi_n_left"] == max(LEFT_VALUES):
        ...
        signal_cache = build_signal_cache(df_5m, [*all_configs, *extended_configs])
    ...
    print("\nTop legacy live n_left rows:", flush=True)
    for row in sweep_rows[:5]:
        print_summary(row)

    print("\nPost-hoc DOW combo analysis on the best live-matched legacy variant...", flush=True)
    ...
    payload = {
        ...
        "legacy_live_n_left_rows": sweep_rows,
        "best_legacy_live_variant": best_row,
