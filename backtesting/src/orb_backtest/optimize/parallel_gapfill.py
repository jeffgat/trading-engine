"""Parallel execution of Gap Fill grid sweeps using multiprocessing."""
...
from ..gapfill_config import GapFillStrategyConfig
...
def _run_single_gapfill(args: tuple) -> tuple[dict, list[TradeResult]]:
    """Worker function for Gap Fill multiprocessing."""
...
def _gapfill_signal_cache_path(df: pd.DataFrame, configs: list[GapFillStrategyConfig]) -> Path:
    """Stable disk cache key for Gap Fill signal cache."""
    ...
    return cache_dir / f"gapfill_sigcache_{safe_data_key}_{param_key}.pkl"


def _load_or_build_gapfill_signal_cache(
    df: pd.DataFrame,
    configs: list[GapFillStrategyConfig],
) -> object:
    """Load Gap Fill signal cache from disk if available, otherwise build and save it."""
    cache_path = _gapfill_signal_cache_path(df, configs)
    if cache_path.exists():
        print(f"[cache] Gap Fill signal cache loaded from disk: {cache_path.name}")
        with open(cache_path, "rb") as f:
            return pickle.load(f)
    print("[cache] Building Gap Fill signal cache...")
    t0 = time.time()
    signal_cache = build_gapfill_signal_cache(df, configs)
    print(f"[cache] Gap Fill signal cache built in {time.time()-t0:.1f}s — saving to disk")
...
def run_gapfill_sweep(
...
) -> list[tuple[GapFillStrategyConfig, list[TradeResult]]]:
    """Run Gap Fill backtests for all configs, optionally in parallel.

    Same interface as run_sweep() but for Gap Fill configs.
    """
    if n_workers is None:
        n_workers = min(cpu_count(), len(configs))

    maps = _prebuilt_maps if _prebuilt_maps is not None else build_maps(df, df_1m)
    signal_cache = _prebuilt_signal_cache if _prebuilt_signal_cache is not None else _load_or_build_gapfill_signal_cache(df, configs)
    ...
    results = []
    try:
        for i, result in enumerate(pool.imap_unordered(_run_single_gapfill, args_list, chunksize=chunksize)):
