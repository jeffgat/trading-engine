#!/usr/bin/env python3
"""SI ORB Discovery — 3-session sweep with 1m magnifier.

Starting thesis: ORB continuation on SI (Silver) across NY, Asia, LDN sessions.
Baseline: RR=2.0, TP1=0.5, stop=10% ATR, min_gap=1% ATR.
Sweeps ATR stops, ORB-range stops, both directions, multiple ORB windows.

Pre-holdout only (<2025-01). Uses 1m magnifier (no 1s data for SI).

SI is a commodity future ($5,000/pt, 0.005 tick, .v.0 volume roll).
No prior learnings exist — completely fresh instrument.
"""
...
OUTPUT_DIR = ROOT / "data" / "results" / "si_orb_discovery_3session"
    ...
    for orb_min in ORB_MINUTES:
        for stop_mode in ["atr", "orb"]:
            ...
            base = StrategyConfig(
                ...
                tp1_ratio=0.5,
                atr_length=14,
                name=f"SI {session_name} ORB{orb_min}m {stop_mode}-stop",
            )
...
def main():
    print("SI ORB Discovery — 3-Session Sweep (1m magnifier)")
    print("=" * 70)
    ...
    print(f"Bar magnifier: ON (1m) — no 1s data for SI")
