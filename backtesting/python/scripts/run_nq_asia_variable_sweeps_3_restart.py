#!/usr/bin/env python3
"""NQ Asia Continuation — Variable Sweeps Round 3 (RESTART).

R3 anchor (grid sweep winner #5, 1 neg year only):
  ORB: 20:00-20:15, entry until 23:15, flat 23:00-07:00
  stop=4.0%, min_gap_atr=0.9%, max_gap_pts=50.0, max_gap_atr=0.0
  rr=3.0, tp1=0.6, ATR=14, direction=both, ICF=OFF, continuation, 1s magnifier
  DOW gate: none

Grid sweep: anchor Calmar 4.20 ranked #110/625. Winner #1 (Calmar 7.03) had 2 neg years.
  Adopted #5 (stop=4.0, rr=3.0, gap=0.9, tp1=0.6) — Calmar 6.18, 1 neg year (2022 only).
  rr=3.0 was at grid boundary — RR sweep expanded to include 1.5, 2.0, 2.5.
  0/625 combos had 0 negative years — 2022 is structural.
...
ANCHOR = StrategyConfig(
    ...
    name="NQ Asia R3 Anchor (restart)",
)
...
def main():
    ...
    # -- 7. REWARD:RISK --------------------------------------------------------
    rr_vals = [1.0, 1.5, 1.75, 2.0, 2.25, 2.5, 3.0, 3.5, 4.0, 5.0]
