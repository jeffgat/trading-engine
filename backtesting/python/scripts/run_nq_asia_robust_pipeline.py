#!/usr/bin/env python3
"""NQ Asia Continuation Long — 5-Phase Robust Pipeline (R9 restart anchor).

Anchor from R9 variable sweep convergence + Grid Sweep R2 confirmation:
  stop=4.0%, rr=3.0, gap=0.9%, tp1=0.6, ATR=5, max_gap=75pts
  ORB 15m (20:00-20:15), entry until 22:30, flat 04:00-07:00
  Long only, DOW excl Tue, ICF ON, 1s bar magnifier

Grid sweep R2: Anchor ranked #2/500 (Calmar 15.64), delta <0.5 — confirmed.
In-sample: Calmar 15.64, DD -11.3R, 0 neg years, 770 trades.
...
PARAM_RANGES = {
    "asia_stop_atr_pct": [3.5, 4.0, 5.0],
    "rr": [2.5, 3.0, 3.5],
    "asia_min_gap_atr_pct": [0.75, 0.9, 1.0],
...
def phase_1(df, df_1m, df_1s):
    ...
    print("  DOW filter: exclude Tuesday (applied post-backtest)", flush=True)
...
def phase_2(df, df_1m, df_1s):
    ...
    # NOTE: WF uses 1m magnifier only (not 1s). The 1s maps are ~101M entries
    ...
    print(f"  DOW filter: exclude Tuesday (applied post-backtest in WF)", flush=True)
...
def phase_4(df, df_1m, df_1s):
    ...
    print(f"  DOW filter: exclude Tuesday", flush=True)
...
if __name__ == "__main__":
    print(flush=True)
    print("=" * 70, flush=True)
    print("  NQ ASIA CONTINUATION LONG — 5-PHASE ROBUST PIPELINE (R9 restart)", flush=True)
    print("  Anchor: stop=4% | rr=3.0 | gap=0.9% | tp1=0.6 | ATR=5", flush=True)
    print("  Structural: ORB 15m | entry<22:30 | flat 04:00 | max_gap=75pts", flush=True)
    print("  Long only | DOW excl Tue | ICF ON | 1s mag", flush=True)
    print("=" * 70, flush=True)
    ...
    print(f"  Anchor: stop=4% | rr=3.0 | gap=0.9% | tp1=0.6 | ATR=5", flush=True)
    print(f"  Structural: ORB 15m | entry<22:30 | flat 04:00 | max_gap=75pts", flush=True)
    print(f"  Long only | DOW excl Tue | ICF ON | 1s mag", flush=True)
    print(flush=True)
