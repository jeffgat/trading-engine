#!/usr/bin/env python3
"""NQ Asia Continuation Short — 5-Phase Robust Pipeline (R6 converged anchor).

Anchor from variable sweeps R4-R6 convergence + Grid Sweep R2 confirmation:
  stop=3.5%, rr=3.75, gap=1.0%, tp1=0.6, ATR=30
  ORB 10m (20:00-20:10), entry until 01:00, flat 23:00-07:00
  Short only, DOW excl Thu, ICF ON, 1s bar magnifier

Grid sweep R2: Anchor ranked #1/500 (Calmar 6.30), delta=0 — confirmed.
In-sample: Calmar 6.30, DD -17.1R, 2 neg years (2022, 2023), 874 trades.
...
PARAM_RANGES = {
    "asia_stop_atr_pct": [3.0, 3.5, 4.0],
    "rr": [3.25, 3.75, 4.0],
    "asia_min_gap_atr_pct": [0.75, 1.0, 1.25],
...
def phase_1(df, df_1m, df_1s):
    ...
    print("  DOW filter: exclude Thursday (applied post-backtest)", flush=True)
...
def phase_2(df, df_1m, df_1s):
    ...
    print(f"  DOW filter: exclude Thursday (applied post-backtest in WF)", flush=True)
...
def phase_4(df, df_1m, df_1s):
    ...
    print(f"  DOW filter: exclude Thursday", flush=True)
...
if __name__ == "__main__":
    print(flush=True)
    print("=" * 70, flush=True)
    print("  NQ ASIA CONTINUATION SHORT — 5-PHASE ROBUST PIPELINE (R6)", flush=True)
    print("  Anchor: stop=3.5% | rr=3.75 | gap=1.0% | tp1=0.6 | ATR=30", flush=True)
    print("  Structural: ORB 10m | entry<01:00 | flat 23:00", flush=True)
    print("  Short only | DOW excl Thu | ICF ON | 1s mag", flush=True)
    print("=" * 70, flush=True)
    ...
    print(f"  Anchor: stop=3.5% | rr=3.75 | gap=1.0% | tp1=0.6 | ATR=30", flush=True)
    print(f"  Structural: ORB 10m | entry<01:00 | flat 23:00", flush=True)
    print(f"  Short only | DOW excl Thu | ICF ON | 1s mag", flush=True)
    print(flush=True)
