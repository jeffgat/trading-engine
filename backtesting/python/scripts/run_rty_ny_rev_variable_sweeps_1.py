#!/usr/bin/env python3
"""RTY NY Reversal — Variable Sweeps Round 1 (exploratory).

Starting from sensible defaults for a reversal strategy on RTY NY.
Reversal = bearish FVG → long (buy the dip), bullish FVG → short (fade the rally).

Default anchor:
  stop=3.0%, rr=2.0, gap=1.0%, tp1=0.5, entry≤12:00
  ATR=14, 15m ORB, flat=15:50
...
def main():
    ...
    # ── Anchor ──
    print("=" * 100)
    print("  ANCHOR (R1: reversal, both dirs, stop=3.0%, rr=2.0, gap=1.0%, tp1=0.5, entry≤12:00)")
    print("=" * 100)
    ...
    # ── 1. Direction Filter ──
    print("=" * 100)
    print("  SWEEP 1: Direction Filter")
    print("=" * 100)
    print(HDR)
    ...
    # ── 2. Stop ATR % ──
    print("=" * 100)
    print("  SWEEP 2: Stop ATR %")
    print("=" * 100)
    print(HDR)
    for i, s in enumerate([3.0, 4.0, 5.0, 6.0, 7.5, 10.0, 12.5, 15.0], 1):
    ...
    print("=" * 100)
    print(HDR)
    for i, rr in enumerate([1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0, 6.0], 1):
    ...
    # ── 6. Entry End Time ──
    print("=" * 100)
    print("  SWEEP 6: Entry End Time")
    print("=" * 100)
    print(HDR)
    for i, ee in enumerate(["10:30", "11:00", "12:00", "13:00", "14:00", "15:00", "15:30"], 1):
    ...
    print("=" * 100)
    print(HDR)
    for i, atr in enumerate([5, 10, 14, 20, 30, 50], 1):
    ...
    # ── 11. Impulse Close Filter ──
    print("=" * 100)
    print("  SWEEP 11: Impulse Close Filter")
    print("=" * 100)
    print(HDR)
    for i, icf in enumerate([False, True], 1):
        cfg = with_overrides(ANCHOR, impulse_close_filter=icf)
        _, m = run_and_metric(df, df_1m, df_1s, cfg)
        print(fmt(i, f"ICF={'ON' if icf else 'OFF'}", m))
    print()

    print("=" * 100)
    print("  R1 COMPLETE — Review results and adopt winning values.")
    print("=" * 100)
