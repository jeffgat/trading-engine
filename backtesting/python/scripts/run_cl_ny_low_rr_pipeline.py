#!/usr/bin/env python3
"""CL NY Continuation Both — 5-Phase Robust Pipeline (LOW R:R VARIANT).

Testing lower R:R for smoother equity and better prop firm survival.
The high-RR pipeline (rr=6.0, stop=1.0%) had NO-GO verdict: Phase 3 worst month
-12.8R and Phase 5 MC survival 43.3%. This variant tests moderate R:R to reduce
monthly drawdown variance.

Anchor (from R2 grid region — moderate RR center):
  stop=2.0%, rr=4.0, gap=1.0%, tp1=0.50
...
ANCHOR = StrategyConfig(
    rr=4.0,                  # Lower RR for smoother equity
    tp1_ratio=0.50,
    risk_usd=5000.0,
    atr_length=10,
    min_qty=1.0,
...
# -- Walk-forward param ranges (centered on moderate RR) ----------------------

PARAM_RANGES = {
    "ny_stop_atr_pct": [1.0, 1.5, 2.0, 2.5, 3.0],
    "rr": [3.0, 3.5, 4.0, 4.5, 5.0],
    "ny_min_gap_atr_pct": [0.75, 1.0, 1.25],
    "tp1_ratio": [0.4, 0.5, 0.6],
}
# 5 x 5 x 3 x 3 = 225 combos per fold
...
if __name__ == "__main__":
    print(flush=True)
    print("=" * 70, flush=True)
    print("  CL NY CONTINUATION BOTH — LOW R:R PIPELINE", flush=True)
    print("  Anchor: stop=2.0% | rr=4.0 | gap=1.0% | tp1=0.5", flush=True)
    ...
    print(f"  Anchor: stop=2.0% | rr=4.0 | gap=1.0% | tp1=0.5", flush=True)
