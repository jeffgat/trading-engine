#!/usr/bin/env python3
"""Phase-One Robust Pipeline — ES NY HTF-LSI frozen shortlist.

Evaluates the frozen ES HTF-LSI shortlist for first-payout economics:
  0. Model freeze
...
SHORTLIST_SOURCE = ROOT / "data" / "results" / "es_ny_htf_lsi_stitched_followup" / "summary.json"
STITCHED_REPORT_PATH = ROOT / "learnings" / "reports" / "ES_NY_HTF_LSI_STITCHED_FOLLOWUP.md"
OUTPUT_DIR = ROOT / "data" / "results" / "es_ny_htf_lsi_phase_one"
REPORT_PATH = ROOT / "learnings" / "reports" / "ES_NY_HTF_LSI_PHASE_ONE.md"
...
def config_from_summary_row(row: dict):
    ...
    return build_config(
        ...
        lsi_fvg_window_left=int(row["config"]["left_minutes"] / 3),
        lsi_fvg_window_right=int(row["config"]["right_minutes"] / 3),
...
def main() -> None:
    print("Phase-One Robust Pipeline — ES NY HTF-LSI", flush=True)
    print("=" * 72, flush=True)
    ...
    print("\nLoading ES 3m HTF-LSI data (3m + 1m + 1s)...", flush=True)
    df_base, df_1m, df_1s, signal_df_1m = load_timeframe_data("ES", "3m")
    ...
    payload["results"].sort(
        key=lambda row: (
            ...
            float(row.get("oos_prop_scorecard", {}).get("first_payout_rate", 0.0)),
            float(row.get("holdout_prop_scorecard", {}).get("first_payout_rate", 0.0)),
