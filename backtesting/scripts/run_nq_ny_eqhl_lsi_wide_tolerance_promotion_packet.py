#!/usr/bin/env python3
"""Local promotion sweep around the wide-tolerance NQ NY EQHL-LSI winners.

This packet keeps the widened EQHL source semantics fixed and only varies the
local execution knobs around the new winning branches:

1. 1m entry -> 60m EQHL source -> 10pt
2. 1m entry -> 60m EQHL source -> 15pt
3. 2m entry -> 15m EQHL source -> 5pt
4. 3m entry -> 15m EQHL source -> 15pt
5. 5m entry -> 5m EQHL source -> 5pt

Holdout remains closed. We only tune:
- rr
- tp1_ratio
- lsi_fvg_window_left
...
OUTPUT_DIR = ROOT / "data" / "results" / "nq_ny_eqhl_lsi_wide_tolerance_promotion_packet"
REPORT_PATH = ROOT / "learnings" / "reports" / "NQ_NY_EQHL_LSI_WIDE_TOLERANCE_PROMOTION_PACKET.md"

BRANCH_SPECS = (
    {
        "slug": "1m_eqhl60m_tol10p",
        "timeframe": "1m",
        "eqhl_level_tf_minutes": 60,
        "eqhl_tolerance_ticks": 40,
        "eqhl_tolerance_points": 10.0,
        "eqhl_min_touches": 2,
        ...
        "left_values": (80, 100, 120),
        "right_values": (8, 10, 12),
        "rr_values": (2.75, 3.0, 3.25, 3.5),
        "tp1_values": (0.4, 0.5, 0.6),
        "min_discovery_trades": 200,
        "min_validation_trades": 50,
    },
    {
        "slug": "1m_eqhl60m_tol15p",
        "timeframe": "1m",
        "eqhl_level_tf_minutes": 60,
        "eqhl_tolerance_ticks": 60,
        "eqhl_tolerance_points": 15.0,
        "eqhl_min_touches": 2,
        "direction_filter": "long",
        "entry_mode": "fvg_limit",
        "entry_end": "15:00",
...
def make_row(config, trades, *, branch: dict) -> dict:
    ...
    return {
        ...
        "eqhl_tolerance_points": float(branch["eqhl_tolerance_points"]),
...
def _build_configs(branch: dict) -> list:
    configs = []
    tf_min = timeframe_minutes(branch["timeframe"])
    for left, right, rr, tp1 in product(
        ...
        config = build_config(
            ...
            tolerance_label=f"{branch['eqhl_tolerance_points']:g}p",
            ...
            left_minutes=left * tf_min,
            right_minutes=right * tf_min,
            min_stop_points=0.0,
            min_tp1_points=0.0,
        )
        configs.append(
    ...
    for branch in payload["branches"]:
        lines.extend(
            [
                ...
                f"- Frozen sweep semantics: `tol={branch['eqhl_tolerance_points']} points`, `touches={branch['eqhl_min_touches']}`, `{branch['direction_filter']}`, `{branch['entry_mode']}`, `entry_end={branch['entry_end']}`",
