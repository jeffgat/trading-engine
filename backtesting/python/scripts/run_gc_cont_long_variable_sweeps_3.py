#!/usr/bin/env python3
"""GC NY Continuation Longs — Round 3 Variable Sweeps (clean data, 1s magnifier).

R2 adopted ATR 10 (Calmar 11.80 vs 10.49, Δ=+1.31).
Re-sweep all dimensions on updated anchor.

Anchor:
  stop=4.0% ATR, min_gap=3.5%, rr=4.0, tp1=0.5, entry_end=11:00
  10m ORB (09:30-09:40), long-only, ATR 10, flat_start=15:50
  FOMC dates excluded
...
# ── Anchor config (R2 winner: ATR 10 adopted) ───────────────────────────────
...
ANCHOR = StrategyConfig(
    ...
    tp1_ratio=0.5,
    risk_usd=5000.0,
    atr_length=10,             # changed from 16 (R2 sweep winner)
    min_qty=1.0,
    qty_step=1.0,
...
def print_anchor_metrics(m):
    section("ANCHOR METRICS (R2 Winner: ATR 10)")
    print(f"  Config: stop=4.0% | rr=4.0 | min_gap=3.5% | tp1=0.5 | ATR 10")
    print(f"          10m ORB | entry→11:00 | long-only | FOMC excl | 1s magnifier")
    print(f"  Changed from R2: ATR 16→10")
    print()
...
if __name__ == "__main__":
    print()
    print("=" * 70)
    print("  GC NY CONT LONGS — R3 VARIABLE SWEEPS (clean data, 1s magnifier)")
    print("  Anchor: stop=4.0% | rr=4.0 | min_gap=3.5% | tp1=0.5 | ATR 10")
    print("          10m ORB | entry→11:00 | FOMC excl | long-only")
    print("  Changed from R2: ATR 16→10")
    print("=" * 70)
    ...
    print("  If anchor changes, re-run sweeps on new anchor (R4).")
    print("=" * 70)
    print()
