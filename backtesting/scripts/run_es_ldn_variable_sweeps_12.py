#!/usr/bin/env python3
"""ES LDN Continuation Both — Variable Sweeps Round 12.

New anchor: stop=6.0%, rr=4.0, gap=1.0%, tp1=0.75, ATR=14, max_gap=20%ATR
Structural: ORB 10m, entry 08:25, flat 08:00, both, DOW excl Mon, ICF off, 1s mag.
...
ANCHOR_SESSION = SessionConfig(
    ...
    entry_end="08:25", flat_start="08:00", flat_end="08:25",
    stop_atr_pct=6.0, min_gap_atr_pct=1.0, max_gap_points=0.0,
)
ANCHOR = StrategyConfig(
    rr=4.0, tp1_ratio=0.75, risk_usd=5000.0, atr_length=14,
...
def sweep_stop(df, df_1m, df_1s):
    results = []
    for v in [4.0, 5.0, 5.5, 6.0, 6.5, 7.0, 7.5, 10.0]:
...
def sweep_flat(df, df_1m, df_1s):
    ...
    for v in ["06:00","06:30","07:00","07:30","08:00","08:20"]:
        ...
        _, m = run_and_measure(df, replace(ANCHOR, sessions=(sess,)), df_1m, df_1s)
        results.append((v, m))
    print_sweep_table(results, "Flat Start", "08:00"); return results
...
def sweep_tp1(df, df_1m, df_1s):
    results = []
    for v in [0.4, 0.5, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85]:
        _, m = run_and_measure(df, replace(ANCHOR, tp1_ratio=v), df_1m, df_1s)
        results.append((v, m))
    print_sweep_table(results, "TP1 Ratio", 0.75); return results
...
def sweep_max_gap(df, df_1m, df_1s):
    values = [("OFF",0,0),("10%ATR",0,10),("15%ATR",0,15),("20%ATR",0,20),("25%ATR",0,25),("30%ATR",0,30),("50%ATR",0,50)]
    results = []
    for label, pts, atr_pct in values:
        sess = replace(ANCHOR_SESSION, max_gap_points=float(pts), max_gap_atr_pct=float(atr_pct))
        _, m = run_and_measure(df, replace(ANCHOR, sessions=(sess,)), df_1m, df_1s)
        results.append((label, m))
    print_sweep_table(results, "Max Gap Filter", "20%ATR"); return results
...
if __name__ == "__main__":
    print("=" * 90)
    print("  ES LDN CONTINUATION BOTH — VARIABLE SWEEPS ROUND 12")
    print("=" * 90)
    print(f"  Anchor: stop=6%, rr=4.0, gap=1.0%, tp1=0.75, ATR=14, max_gap=20%ATR, ORB 10m")
    print(f"  Flat 08:00, entry 08:25, both, DOW excl Mon, ICF off, 1s mag")
    print(f"  Changes from R11: tp1 0.5→0.75, flat 08:20→08:00\n")
    ...
    sweeps = [
        ...
        ("Flat Start", "08:00", sweep_flat), ("Direction", "both", sweep_dir),
        ("R:R", 4.0, sweep_rr), ("TP1 Ratio", 0.75, sweep_tp1),
        ("Min Gap ATR%", 1.0, sweep_gap), ("DOW Exclusion", "Mon", sweep_dow),
        ("Max Gap", "20%ATR", sweep_max_gap), ("ICF", False, sweep_icf),
    ]
    ...
    print(f"\n\n{'='*90}")
    print(f"  SUMMARY — Variable Sweeps Round 12 ({total_time:.0f}s)")
    print(f"{'='*90}")
    ...
    if not adoptions:
        ...
        print(f"\n  Final anchor: stop=6%, rr=4.0, gap=1.0%, tp1=0.75, ATR=14, max_gap=20%ATR")
        print(f"    ORB 10m, entry 08:25, flat 08:00, both, DOW excl Mon, 1s mag")
    else:
