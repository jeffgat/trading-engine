#!/usr/bin/env python3
"""Robust pipeline: GC NY Inversion Longs v9 Regime-Sized SPY.

Base strategy: v9 (v8 + 10% qualifying sweep)
Sizing overlay: 2x when VIX<18 AND SPY>SMA20 (prior day close), 1x otherwise.

Regime sizing is applied as a post-hoc overlay at each phase:
  Phase 1: Structural — apply sizing to full-history trades
  Phase 2: Walk-forward — apply sizing to OOS trades from each fold
  Phase 3: Prop constraints — evaluate regime-sized WF OOS trades
  Phase 4: Hold-out — apply sizing to hold-out trades
  Phase 5: Monte Carlo — bootstrap regime-sized R-multiples
"""
...
def apply_regime_sizing(trades, env):
    """Return new trade list with 2x R in favorable regime, 1x otherwise."""
    sized = []
    n_doubled = 0
    for t in trades:
...
def phase1(df, df_1m, env):
    banner(1, "STRUCTURAL VALIDATION (regime-sized)")
    print(f"  Config: GC NY Inversion Longs v9 + 2x VIX<18+SPY>SMA20")
    print(f"  Range:  {WF_START} to {HOLDOUT_END}")
    print()

    t0 = time.time()
    trades = run_backtest_qm(df, BASE_CONFIG, start_date=WF_START, df_1m=df_1m)
    sized_trades, n_doubled = apply_regime_sizing(trades, env)
    print(f"  Completed in {time.time() - t0:.1f}s")
    print(f"  Regime sizing: {n_doubled} trades doubled, "
          f"{len([t for t in trades if t.exit_type != EXIT_NO_FILL]) - n_doubled} at 1x")

    m = compute_metrics(sized_trades)
    ...
    return all_pass, m, sized_trades


# ── Phase 2 ──────────────────────────────────────────────────────────────────

def phase2(df, df_1m, env):
    banner(2, "WALK-FORWARD + PARAMETER STABILITY (regime-sized OOS)")
    print(f"  IS: 36m | OOS: 12m | Step: 12m | Objective: sharpe")
    print(f"  Range: {WF_START} to {WF_END}")
    print(f"  Note: WF optimizes BASE params; regime sizing applied to OOS trades")
    combos = 1
    ...
    # Print fold details (base metrics)
    ...
    # Apply regime sizing to combined OOS trades
    base_oos_trades = wf_result.combined_oos_trades
    sized_oos_trades, n_doubled = apply_regime_sizing(base_oos_trades, env)

    # Replace the combined OOS trades and metrics in wf_result for downstream phases
    wf_result.combined_oos_trades = sized_oos_trades
    wf_result.combined_oos_metrics = compute_metrics(sized_oos_trades)

    print()
    m = wf_result.combined_oos_metrics
    n_folds = len(wf_result.folds)
    print(f"  Combined OOS ({n_folds} folds) — REGIME-SIZED:")
    print(f"    Regime: {n_doubled} trades doubled, "
          f"{len([t for t in base_oos_trades if t.exit_type != EXIT_NO_FILL]) - n_doubled} at 1x")
    ...
    # Stability (based on base param selection, unaffected by sizing)
...
def phase3(wf_result):
    banner(3, "PROP FIRM CONSTRAINT FILTER (regime-sized)")
    print(f"  Evaluating on regime-sized WF OOS trades ({len(wf_result.combined_oos_trades)} trades)")
    print()
...
def phase4(df, df_1m, stability, env):
    banner(4, "HOLD-OUT OOS TEST (regime-sized)")
    ...
    sized_trades, n_doubled = apply_regime_sizing(trades, env)
    filled = len([t for t in trades if t.exit_type != EXIT_NO_FILL])
    print(f"  Completed in {time.time() - t0:.1f}s")
    print(f"  Regime sizing: {n_doubled} doubled, {filled - n_doubled} at 1x")
...
def phase5(wf_trades):
    banner(5, "MONTE CARLO SURVIVAL (regime-sized)")
    ...
    print(f"  Note: R-multiples include regime sizing (some trades at 2x)")
    print()
...
def main():
    print()
    print("=" * 70)
    print("  ROBUST PIPELINE: GC NY Inversion Longs v9 Regime-Sized SPY")
    print("  Base: v9 (v8 + 10% QM)")
    print("  Sizing: 2x when VIX<18 + SPY>SMA20, 1x otherwise")
    print("=" * 70)
    print()
    ...
    print("\nLoading regime data (VIX + SPY)...")
    env = build_env_lookup()
    print(f"  {len(env)} dates in lookup")

    results = {}

    p1_pass, _, _ = phase1(df, df_1m, env)
    results["phase1"] = p1_pass
    if not p1_pass:
        _verdict(results)
        return

    p2_pass, wf_result, stability = phase2(df, df_1m, env)
    ...
    p4_pass, _, _ = phase4(df, df_1m, stability, env)
