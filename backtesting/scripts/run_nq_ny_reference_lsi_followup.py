#!/usr/bin/env python3
"""Focused second-branch discovery for NQ NY reference_lsi.

This script intentionally stays narrow after the first discovery pass:
- direction fixed to both
- entry_end fixed to 11:00
- gap edge fixed to near
- structural search centered on the two winning neighborhoods:
  - gap6 / inv18
  - gap12 / inv12
- reward search centered on RR ~ 3.0 and TP1 ~ 0.7-0.8
...
OUTPUT_DIR = ROOT / "data" / "results" / "nq_ny_reference_lsi_followup"
REPORT_PATH = ROOT / "learnings" / "reports" / "NQ_NY_REFERENCE_LSI_FOLLOWUP.md"
SPEC_PATH = ROOT / "learnings" / "reports" / "NQ_NY_REFERENCE_LSI_FOLLOWUP_SPEC.md"
...
STRUCTURES = [
    {"gap_lookback": 4, "inversion_max": 15},
    {"gap_lookback": 6, "inversion_max": 15},
    {"gap_lookback": 6, "inversion_max": 18},
    {"gap_lookback": 8, "inversion_max": 18},
    {"gap_lookback": 8, "inversion_max": 21},
    {"gap_lookback": 10, "inversion_max": 12},
    {"gap_lookback": 12, "inversion_max": 9},
    {"gap_lookback": 12, "inversion_max": 12},
    {"gap_lookback": 12, "inversion_max": 15},
    {"gap_lookback": 14, "inversion_max": 12},
]
RR_VALUES = [2.75, 3.0, 3.25]
TP1_VALUES = [0.7, 0.8, 0.9]
...
def write_report(payload: dict) -> None:
    ...
    for row in payload["top_rows"][:10]:
        lines.append(
...
def main() -> None:
    ...
    print(f"Running focused follow-up sweep ({len(configs)} configs)...", flush=True)
    ...
    write_json(OUTPUT_DIR / "followup_results.json", payload)
    write_report(payload)
    print(f"Follow-up complete in {time.time() - t0:.1f}s", flush=True)
