#!/usr/bin/env python3
"""NQ NY FAST_V2 — 5-Phase Robust Pipeline (Bailey-aware).

FAST_V2 execution config NQ_NY leg:
  ORB: 09:30-09:45, entry until 13:00, flat 15:50-16:00
  stop=8.0% ATR-14, min_gap=2.25%, rr=2.5, tp1=0.3
  Both directions, Friday excluded
...
# ── Candidate config (FAST_V2 execution NQ_NY) ─────────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ NY FAST_V2 Robust Pipeline",
)

# ── Walk-forward param ranges (centered on candidate) ────────────────────
PARAM_RANGES = {
    "ny_stop_atr_pct": [5.0, 6.0, 7.0, 8.0, 9.0, 10.0],
...
# Count filtered grid size
_rr_tp1_valid = sum(1 for r in PARAM_RANGES["rr"] for t in PARAM_RANGES["tp1_ratio"] if t * r >= 1.0)
GRID_SIZE = _rr_tp1_valid * len(PARAM_RANGES["ny_stop_atr_pct"]) * len(PARAM_RANGES["ny_min_gap_atr_pct"])
...
def phase_4(df, df_1m, df_1s, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST_V2 NQ_NY robust pipeline",
    )
    if entry.warning:
