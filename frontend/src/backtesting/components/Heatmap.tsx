...
function formatMetricValue(key: MetricKey, value: number, riskUsd: number): string {
  switch (key) {
    case "total_pnl_usd":
    case "max_drawdown_usd":
      return `${(value / riskUsd).toFixed(1)}R`;
    case "win_rate":
...
const ZOOM_LEVELS = [
  { cellMin: 32, cellH: 18, font: 8, label: 8, labelCol: 24 },
  { cellMin: 42, cellH: 24, font: 10, label: 10, labelCol: 32 },
  { cellMin: 56, cellH: 32, font: 11, label: 10, labelCol: 36 },
  ...
  return (
    <div className="rounded-lg border border-border bg-bg-card p-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center rounded-md border border-border">
            <button
              ...
              disabled={zoom === 0}
              className="px-2 py-1 text-xs text-text-muted transition-colors hover:text-text-primary disabled:opacity-30"
            >
              &minus;
            </button>
            <span className="border-x border-border px-2 py-1 text-[10px] text-text-muted">
      ...
      <ScrollArea className="w-full">
        {/* Y-axis label */}
        {!is1D && (
          <div style={{ fontSize: zl.label }} className="mb-1 font-medium text-text-muted">
            {paramKeys[1]}
          </div>
        )}
        ...
        <div className="inline-block">
          ...
          <div
            ...
            style={{
              gridTemplateColumns: is1D
                ? `${zl.labelCol}px repeat(${xValues.length}, minmax(${zl.cellMin}px, 1fr))`
                : `${zl.labelCol}px repeat(${xValues.length}, minmax(${zl.cellMin}px, 1fr))`,
              gridTemplateRows: is1D
                ? "auto"
                : `repeat(${yValues.length}, minmax(${zl.cellH}px, 1fr)) auto`,
            }}
          >
            {/* Rows */}
            {yValues.map((yVal, yi) => (
              <Fragment key={`row-${yi}`}>
                {/* Y label */}
                <div
                  className="flex items-center justify-end pr-1 font-mono text-text-muted"
                  style={{ fontSize: zl.label }}
                >
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        minHeight: is1D ? zl.cellH + 4 : zl.cellH,
                      }}
                      onMouseEnter={(e) => {
                        if (!entry) return;
                    ...
                    >
                      <span className="font-mono font-medium text-white/90" style={{ fontSize: zl.font }}>
                        {entry ? formatMetricValue(metric, val, riskUsd) : "—"}
                      </span>
                    </div>
                  );
                })}
            ...
            {xValues.map((xVal, xi) => (
              <div
                key={`xlabel-${xi}`}
                className="flex items-start justify-center pt-1 font-mono text-text-muted"
                style={{ fontSize: zl.label }}
              >
          ...
          {/* X-axis label */}
          <div className="mt-1 text-center font-medium text-text-muted" style={{ fontSize: zl.label }}>
            {paramKeys[0]}
          </div>
        </div>
      </ScrollArea>
      ...
      {tooltip && (
        ...
        >
          <div className="space-y-0.5 text-xs">
            ...
            <div className="text-text-muted">
              Net R: <span className="font-mono text-text-primary">{(tooltip.summary.total_pnl_usd / riskUsd).toFixed(2)}R</span>
            </div>
            <div className="text-text-muted">
