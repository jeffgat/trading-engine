#!/usr/bin/env python3
"""RTY Asia Continuation — Variable Sweeps Round 2.

R1 adopted: direction=long (Calmar -0.52 → 2.55, +3.08 Δ).
Short side was dragging everything down (-59.1R net).

R2 anchor:
  ORB: 20:00-20:15 (15m), entry until 23:15, flat 06:45-07:00
  stop=5.25%, min_gap_atr=0.9%, max_gap_pts=50, max_gap_atr=0 (off)
  rr=2.0, tp1=0.5, ATR=14, direction=LONG, ICF=OFF, continuation, 1s magnifier
  DOW gate: none
...
ANCHOR = StrategyConfig(
    ...
    risk_usd=5000.0,
    direction_filter="long",   # ← R1 adoption
    rr=2.0,
    tp1_ratio=0.5,
    atr_length=14,
    impulse_close_filter=False,
    name="RTY Asia R2 Anchor",
)
...
def main():
    print("RTY Asia ORB — Round 2: long-only adopted from R1")
    print("=" * 90)
    ...
    # ── 3. ATR LENGTH ────────────────────────────────────────────────
    atr_values = [3, 5, 7, 10, 12, 14, 20, 30, 50]
    ...
    # ── 5. FLAT START ────────────────────────────────────────────────
    flat_starts = ["02:00", "03:00", "04:00", "05:00", "06:00", "06:30", "06:45"]
    ...
    print_header("7. TP1 RATIO (anchor=0.5)")
    ...
    # ── 8. MIN GAP ATR % ─────────────────────────────────────────────
    gap_values = [0.25, 0.5, 0.75, 0.9, 1.0, 1.5, 2.0, 2.5, 3.0]
    ...
    print_header("9. DOW EXCLUSION (anchor=none)")
    ...
    # ── 10. MAX GAP POINTS ────────────────────────────────────────────
    maxgap_values = [0, 5, 10, 20, 30, 50, 75]
    print_header("10. MAX GAP POINTS (anchor=50, 0=no limit)")
    ...
    # ── 11. MAX GAP ATR % ────────────────────────────────────────────
    maxgap_atr_values = [0, 10, 15, 20, 25, 30, 50]
    print_header("11. MAX GAP ATR % (anchor=0/off)")
    ...
    dimension_order = [
        ...
        "max_gap_points", "max_gap_atr_pct", "icf",
    ]
