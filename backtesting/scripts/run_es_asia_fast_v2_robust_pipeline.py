#!/usr/bin/env python3
"""ES Asia FAST_V2 — 5-Phase Robust Pipeline (Bailey-aware).

FAST_V2 execution config ES_Asia leg:
  ORB: 20:00-20:10, entry until 03:00, flat 06:45-06:55
  stop=2.5% ATR-5, min_gap=1.0% ATR, rr=1.75, tp1=0.3
  Both directions, no DOW exclusion
...
# ── Candidate config (FAST_V2 execution ES_Asia) ───────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="ES Asia FAST_V2 Robust Pipeline",
)

# ── Walk-forward param ranges (centered on candidate) ────────────────────
PARAM_RANGES = {
    "asia_stop_atr_pct": [5.0, 6.0, 7.0, 8.0, 9.0],
...
GRID_SIZE = _rr_tp1_valid * len(PARAM_RANGES["asia_stop_atr_pct"]) * len(PARAM_RANGES["asia_min_gap_atr_pct"])
...
def phase_4(df, df_1m, df_1s, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST_V2 ES_Asia robust pipeline",
    )
    if entry.warning:
...
def main():
    ...
    print(f"  Structural: ORB 10m, entry<03:00, flat 06:45, ATR={CANDIDATE.atr_length}, "
