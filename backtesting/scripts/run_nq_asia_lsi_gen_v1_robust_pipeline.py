#!/usr/bin/env python3
"""NQ Asia LSI general_v1 — 5-Phase Robust Pipeline (Bailey-aware).

general_v1 execution config NQ_Asia_LSI leg:
  ORB: 20:00-20:05, entry 20:40-23:00, flat 04:00-07:00
  min_gap=1.75%, rr=2.0, tp1=0.7
  Long only, no DOW exclusion
...
# ── Candidate config (general_v1 execution NQ_Asia_LSI) ──────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ Asia LSI general_v1 Robust Pipeline",
)

# ── Walk-forward param ranges ─────────────────────────────────────────────
PARAM_RANGES = {
    "rr": [1.5, 1.75, 2.0, 2.25],
...
def phase_4(df, df_1m, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="general_v1 NQ_Asia_LSI robust pipeline",
    )
    if entry.warning:
...
def main():
    ...
    print(f"  Structural: LSI strategy, entry 20:40-23:00, flat 04:00, ATR={CANDIDATE.atr_length}, "
          f"dir={CANDIDATE.direction_filter}, no DOW exclusion")
