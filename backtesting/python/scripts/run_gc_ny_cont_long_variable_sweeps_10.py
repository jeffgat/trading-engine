#!/usr/bin/env python3
"""Step 2 — Variable Sweeps Round 10: GC NY Continuation Longs.

Anchor (from R9 + Friday exclusion adoption):
  stop=4.5%, rr=9.0, tp1=0.4, ATR 7, min_gap=3.0%, max_gap_atr=30%
  ICF=True, 8m ORB (09:30-09:38), entry→12:00, flat 14:30, long-only, FOMC excluded
  **Friday excluded** (post-backtest filter)

R9 anchor without Fri: Calmar 9.38, 798 trades, 2 neg years
R9 with Fri excluded: Calmar 10.32, 622 trades, 0 neg years
"""
...
def dow_filter(trades):
    """Remove Friday trades (post-backtest)."""
...
def apply_dow_filter_custom(trades, exclude_days):
    """For DOW sweep only — test different exclusion sets."""
...
def sweep_entry_end():
    R = []
    for v in ["10:30", "11:00", "11:30", "12:00", "13:00", "14:00"]:
...
def sweep_flat():
    R = []
    for v in ["13:00", "13:30", "14:00", "14:30", "15:00", "15:30", "15:50"]:
        m = run_and_measure(replace(ANCHOR, sessions=(replace(GC_NY, flat_start=v),)))
        m["value"] = v; m["is_anchor"] = (v == "14:30"); R.append(m)
...
def sweep_rr():
    ...
    for v in [5.0, 6.0, 7.0, 8.0, 9.0, 10.0, 11.0, 12.0, 14.0]:
        ...
        m["value"] = v; m["is_anchor"] = (v == 9.0); R.append(m)
...
def sweep_tp1():
    ...
    for v in [0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.6, 0.7]:
        ...
        m["value"] = v; m["is_anchor"] = (v == 0.4); R.append(m)
...
def sweep_dow():
    """DOW sweep with Friday as anchor. Test adding/removing other days."""
    trades_all = run_backtest(df_5m, ANCHOR, start_date=START_DATE, df_1m=df_1m, df_1s=df_1s)
    R = []
    for label, excl_set in [("Fri only",{4}), ("none",set()), ("Mon+Fri",{0,4}), ("Tue+Fri",{1,4}),
                             ("Wed+Fri",{2,4}), ("Thu+Fri",{3,4}), ("Mon+Thu+Fri",{0,3,4})]:
    ...
    print_table(R, "DOW Exclusion (Fri = anchor)"); return R
...
if __name__ == "__main__":
    print(f"\n{'='*90}")
    print("  GC NY CONT LONGS — VARIABLE SWEEPS ROUND 10")
    print("  Anchor: stop=4.5%, rr=9.0, tp1=0.4, ATR 7, gap=3.0%, max_gap_atr=30%")
    print("          ICF=True, 8m ORB, entry→12:00, flat 14:30, long-only, FOMC excl")
    print("          **FRIDAY EXCLUDED** (post-backtest filter)")
    print(f"{'='*90}")

    t_start = time.time()

    print("\nRunning anchor (with Friday filter)...")
    ...
    print(f"\n{'='*90}")
    print(f"  ROUND 10 SUMMARY — Anchor Calmar: {anchor_calmar:.2f} | Neg years: {anchor_neg}")
    print(f"{'='*90}")
