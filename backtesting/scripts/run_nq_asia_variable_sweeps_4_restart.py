#!/usr/bin/env python3
"""NQ Asia Continuation — Variable Sweeps Round 4 (RESTART).

R4 anchor (R3 + dir=long, ICF=ON):
  ORB: 20:00-20:15, entry until 23:15, flat 23:00-07:00
  stop=4.0%, min_gap_atr=0.9%, max_gap_pts=50.0, max_gap_atr=0.0
  rr=3.0, tp1=0.6, ATR=14, direction=long, ICF=ON, continuation, 1s magnifier
  DOW gate: none

R3 adoptions applied: dir: both -> long (+0.84), ICF: OFF -> ON (+2.72)
  dir=long eliminated 2022 neg year (longs +19 vs shorts -26 in 2022)
  ICF=ON massive impact (+2.72 Calmar, from 6.18 to 8.90)
...
ANCHOR = StrategyConfig(
    ...
    name="NQ Asia R4 Anchor (restart)",
)
