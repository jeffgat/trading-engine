#!/usr/bin/env python3
"""NQ ORB Discovery — 3-session parallel sweep with regime gate.
...
OUTPUT_DIR = ROOT / "data" / "results" / "nq_orb_discovery_3session"
    ...
    for orb_min in ORB_MINUTES:
        for stop_mode in ["atr", "orb"]:
            ...
            base = StrategyConfig(
                ...
                tp1_ratio=0.5,
                atr_length=14,
                name=f"NQ {session_name} ORB{orb_min}m {stop_mode}-stop",
            )
...
def main():
    print("NQ ORB Discovery — 3-Session Parallel Sweep")
    print("=" * 70)
    ...
    print("\nLoading NQ data (5m only — no bar magnifier for discovery)...", flush=True)
