#!/usr/bin/env python3
"""NQ Asia Short Grid Sweep R2 -- 4D grid centered on converged R6 anchor.

Anchor (from variable sweeps R4-R6):
  ORB: 20:00-20:10, entry until 01:00, flat 23:00-07:00
  stop=3.5%, rr=3.75, gap=1.0%, tp1=0.6
...
ANCHOR = StrategyConfig(
    ...
    name="NQ Asia Short Grid Sweep R2",
)

# Grid dimensions -- narrow range centered on anchor
STOPS = [2.5, 3.0, 3.5, 4.0, 5.0]
RRS   = [3.0, 3.25, 3.5, 3.75, 4.0]
...
def main():
    print(f"{INSTRUMENT_NAME} {SESSION_NAME} SHORT -- Grid Sweep R2 ({len(GRID)} combos)")
    print("=" * 110)
