#!/usr/bin/env python3
"""Step 3 — Grid Sweep R2: GC NY Continuation Longs.

Anchor (converged from R9 variable sweeps):
  ...
  Calmar 9.38, 798 trades, DD -19.6, 2 neg years

Grid: stop x rr x gap x tp1 = 5x5x5x5 = 625 combos
Narrow range centered on converged anchor.
If winner is within 0.5 Calmar of anchor -> confirmed, proceed to robust pipeline.
"""
...
# Grid — narrow range centered on converged anchor
STOPS = [3.5, 4.0, 4.5, 5.0, 5.5]
RRS   = [7.0, 8.0, 9.0, 10.0, 11.0]
GAPS  = [2.0, 2.5, 3.0, 3.5, 4.0]
TP1S  = [0.30, 0.35, 0.40, 0.45, 0.50]

GRID = list(product(STOPS, RRS, GAPS, TP1S))
ANCHOR_STOP, ANCHOR_RR, ANCHOR_GAP, ANCHOR_TP1 = 4.5, 9.0, 3.0, 0.4
...
def main():
    print(f"\n{'='*110}")
    print(f"  GC NY CONT LONGS — GRID SWEEP R2 ({len(GRID)} combos)")
    print(f"  Anchor: stop=4.5%, rr=9.0, tp1=0.4, ATR 7, gap=3.0%, max_gap_atr=30%")
    print(f"          ICF=True, 8m ORB, entry→12:00, flat 14:30, long-only, FOMC excl")
