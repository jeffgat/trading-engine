#!/usr/bin/env python3
"""ES LDN Continuation Both — Fine-tune grid v2 (post fill-bar fix, 1s magnifier).

Previous fine-tune winner: stop=1.0%, rr=4.5, gap=3.0%, tp1=0.7 (pre-3% stop floor)
Robust pipeline anchor: stop=5.2%, rr=2.0, gap=1.25%, tp1=0.40

New grid centered on robust pipeline anchor:
  stop_atr_pct:    [3.0, 3.5, 4.0, 4.5, 5.0, 5.2, 5.5, 6.0]  = 8
  rr:              [1.5, 1.75, 2.0, 2.25, 2.5, 3.0]            = 6
  min_gap_atr_pct: [0.75, 1.0, 1.25, 1.5, 2.0, 2.5]           = 6
  tp1_ratio:       [0.30, 0.35, 0.40, 0.45, 0.50, 0.55]        = 6
  Total: 1,728 combos

Structural config from sweep_3 (update if sweep_3 changes anything):
  ORB 10m (03:00-03:10), flat 08:00, ATR 50, both dir, 1s magnifier
  3.0% stop floor enforced (per ES minimum stop rule)
"""
...
# -- Structural config (locked from sweep_3 — update if sweep_3 changes) ------
...
# -- Fine-tune grid values (centered on robust pipeline anchor) ----------------

STOP_VALUES = [3.0, 3.5, 4.0, 4.5, 5.0, 5.2, 5.5, 6.0]
RR_VALUES = [1.5, 1.75, 2.0, 2.25, 2.5, 3.0]
GAP_VALUES = [0.75, 1.0, 1.25, 1.5, 2.0, 2.5]
TP1_VALUES = [0.30, 0.35, 0.40, 0.45, 0.50, 0.55]
...
def main():
    print()
    print("=" * 70)
    print("  ES LDN CONTINUATION — FINE-TUNE GRID v2 (post fill-bar fix, 1s)")
    print(f"  Anchor: stop=5.2% | rr=2.0 | gap=1.25% | tp1=0.4")
    print(f"  Structural: ORB 10m | flat 08:00 | ATR 50 | both | 1s")
    print(f"  Grid: {len(STOP_VALUES)}s x {len(RR_VALUES)}rr x "
          f"{len(GAP_VALUES)}g x {len(TP1_VALUES)}tp1 = {TOTAL_COMBOS} combos")
    print("=" * 70)
    ...
    for stop in STOP_VALUES:
        ...
        print(f"  --- stop={stop:.2f}% done ({done}/{TOTAL_COMBOS}, "
    ...
    for i, r in enumerate(ranked[:20]):
        print(f"  {i+1:>3} {r['stop']:>5.2f} {r['rr']:>5.2f} {r['gap']:>5.2f} {r['tp1']:>5.2f} | "
    ...
    if clean:
        print()
        print("=" * 70)
        print(f"  TOP 20 BY CALMAR (0 negative full years) — {len(clean)}/{len(results)} clean")
        print("=" * 70)
        print(f"  {'#':>3} {'Stop':>5} {'RR':>5} {'Gap':>5} {'TP1':>5} | "
              f"{'Trades':>6} {'WR':>6} {'PF':>5} {'Net R':>8} {'R/yr':>7} {'Max DD':>8} "
              f"{'Calmar':>7} {'Sharpe':>7}")
    ...
    for label, src in [("TOP OVERALL", ranked[0]),
        ...
        print()
        print("=" * 70)
        print(f"  {label}: stop={src['stop']:.2f}% rr={src['rr']:.2f} "
              f"gap={src['gap']:.2f}% tp1={src['tp1']:.2f}")
    ...
    print()
    print("=" * 70)
    print("  DONE — Next: if anchor changed, re-sweep variables (sweep_4).")
    print("  If converged, proceed to robust pipeline.")
    print("=" * 70)
    print()
