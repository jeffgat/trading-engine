#!/usr/bin/env python3
"""NQ Asia Long — Variable Sweeps Round 6 (Full ORB Anchor).

R6 anchor (R5 + adoptions):
  ORB: 20:00-20:15, entry until 00:00, flat 04:00-07:00
  stop_orb_pct=100%, min_gap_orb_pct=5%
  rr=8.0, tp1=0.3, ATR=5, direction=long, ICF=OFF, continuation, 1s magnifier
  DOW gate: excl Tue

R5 adoptions applied:
  flat_start: 00:00 -> 04:00 (+1.25)
  rr: 6.0 -> 8.0 (+0.48)
  tp1_ratio: 0.7 -> 0.3 (+0.91)

Oscillation detected in rr/tp1/flat_start — running R6 to confirm.
If oscillation continues, lock structural params and grid sweep rr x tp1 x flat.
...
ANCHOR = StrategyConfig(
    ...
    name="NQ Asia Long R6 Anchor (full ORB)",
)
...
def main():
    ...
    # -- 5. FLAT START TIME ----------------------------------------------------
    flat_starts = ["22:00", "23:00", "00:00", "01:00", "02:00", "03:00", "04:00", "05:00", "06:00"]
