#!/usr/bin/env python3
"""Step 2 — Variable Sweeps Round 8: GC NY Continuation Longs.

Anchor (from Grid Sweep R1 winner):
  stop=4.5%, rr=9.0, tp1=0.4, ATR 7, min_gap=3.0%, max_gap_atr=25%
  ICF=True, 8m ORB (09:30-09:38), entry→12:00, flat 15:50, long-only, FOMC excluded

Note: Grid winner was at boundary on rr=9.0 and tp1=0.4.
Variable sweeps will explore beyond these boundaries.
"""
...
def sweep_flat():
    ...
    for v in ["13:00", "14:00", "14:30", "15:00", "15:30", "15:50"]:
        ...
        m["value"] = v; m["is_anchor"] = (v == "15:50"); R.append(m)
...
def sweep_rr():
    """Extended range — grid winner hit rr=9.0 boundary."""
    R = []
    ...
    print_table(R, "R:R Ratio (extended)"); return R


def sweep_tp1():
    """Extended range — grid winner hit tp1=0.4 boundary."""
    R = []
    ...
    print_table(R, "TP1 Ratio (extended)"); return R
...
def sweep_dow():
    ...
    R = []
    for label, excl_set in [("none",set()), ("Mon",{0}), ("Tue",{1}), ("Wed",{2}),
                             ("Thu",{3}), ("Fri",{4}), ("Mon+Fri",{0,4}), ("Thu+Fri",{3,4})]:
        filtered = apply_dow_filter(trades_all, excl_set) if excl_set else trades_all
        m = compute_metrics(filtered)
        m["neg_full_years"] = sum(1 for y, r in m.get("r_by_year", {}).items() if r < 0 and y != CPY)
        m["r_per_yr"] = m["total_r"] / DATA_YEARS
        m["value"] = label; m["is_anchor"] = (label == "none"); R.append(m)
...
def sweep_max_gap_atr():
    ...
    for label, v in [("OFF",0.0), ("15%",15.0), ("20%",20.0), ("25%",25.0),
        ...
        m["value"] = label; m["is_anchor"] = (label == "25%"); R.append(m)
...
if __name__ == "__main__":
    print(f"\n{'='*90}")
    print("  GC NY CONT LONGS — VARIABLE SWEEPS ROUND 8")
    print("  Anchor (Grid R1 winner): stop=4.5%, rr=9.0, tp1=0.4, ATR 7, gap=3.0%")
    print("          max_gap_atr=25%, ICF=True, 8m ORB, entry→12:00, flat 15:50, long-only")
    print("  Note: rr and tp1 hit grid boundaries — sweeps explore beyond")
    print(f"{'='*90}")
    ...
    print(f"\n{'='*90}")
    print(f"  ROUND 8 SUMMARY — Anchor Calmar: {anchor_calmar:.2f} | Neg years: {anchor_neg}")
    print(f"{'='*90}")
