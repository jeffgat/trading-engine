#!/usr/bin/env python3
"""NQ NY Continuation — Variable Sweeps Round 4 (Longs-Only Fresh Start).

R4 anchor (R3 + 5 adoptions — mostly reverting destructive R2 changes):
  ORB: 09:30-09:50 (20m), entry until 12:00, flat 14:30-16:00
  stop=9.0%, min_gap_atr=2.5%, max_gap_pts=100.0, max_gap_atr=0.0
  rr=3.0, tp1=0.5, ATR=7, direction=long, ICF=OFF, continuation, 1s magnifier
  DOW gate: excl Fri

R3 adoptions applied (5 — mostly reverting R2 oscillation):
  atr_length: 20 -> 7 (+5.67)      [was 12->20 in R2, oscillation detected]
  entry_end: 14:00 -> 12:00 (+7.22) [was 12:00->14:00 in R2, reverted]
  flat_start: 15:00 -> 14:30 (+0.94)
  icf: ON -> OFF (+1.38)            [was OFF->ON in R2, reverted]
  stop_atr_pct: 7.0 -> 9.0 (+2.48) [was 9->7 in R2, reverted]
...
def main():
    ...
    print(f"\nPost-oscillation recovery — R3 reverted most R2 changes, ATR shifted 12->7, flat 15:50->14:30")
