#!/usr/bin/env python3
"""Prop firm phase 1 sweep: NQ ASIA LSI Long Final

Anchor from saved config: bt-nq-asia-lsi-long-2016-2026-final-478590
...
ANCHOR = StrategyConfig(
    ...
    # LSI defaults from saved config
    lsi_n_left=3,
    lsi_n_right=3,
...
PARAM_GRID = {
    ...
    "asia_min_gap_atr_pct": [1.0, 1.5, 1.75, 2.0, 2.5],
}
...
def config_label(cfg: StrategyConfig) -> str:
    return f"rr={cfg.rr} tp1={cfg.tp1_ratio} gap={cfg.sessions[0].min_gap_atr_pct}"
...
def main():
    ...
    # ── Generate grid ──────────────────────────────────────────────────────
    all_configs = generate_param_grid(ANCHOR, PARAM_GRID)
    ...
    # Filter: >= 20 trades (2yr window is smaller)
    ...
    print(f"\n{'=' * 150}")
    print("TOP 25 BY CALMAR — NQ ASIA LSI Long")
    print(f"{'=' * 150}")
    ...
    print(f"\n{'=' * 150}")
    print("TOP 25 BY PROP FIRM SUCCESS — NQ ASIA LSI Long")
    print(f"{'=' * 150}")
    ...
    output_path = Path(__file__).parent.parent / "data" / "results" / "nq_asia_lsi_propfirm_sweep.json"

    save_data = {
        "sweep_info": {
            "anchor_name": "NQ ASIA LSI Long Final",
