#!/usr/bin/env python3
"""NQ ASIA general_v1 — 5-Phase Robust Pipeline (Bailey-aware).

general_v1 execution config NQ_Asia leg:
  ORB: 20:00-20:15, entry until 22:30, flat 04:00-07:00
  stop=100% ORB, min_gap=10% ORB, rr=6.0, tp1=0.3
  Long only, Tuesday excluded
...
from orb_backtest.analysis.gates import TUE, apply_dow_filter
...
# ── Candidate config (general_v1 execution NQ_Asia) ─────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ Asia general_v1 Robust Pipeline",
)

# ── Walk-forward param ranges (centered on candidate) ────────────────────
PARAM_RANGES = {
    "asia_stop_orb_pct": [75.0, 100.0, 125.0, 150.0, 175.0],
    "rr": [4.0, 5.0, 5.5, 6.0, 7.0, 8.0],
    "asia_min_gap_orb_pct": [5.0, 7.5, 10.0, 12.5, 15.0],
...
def phase_1(df, df_1m, df_1s):
    ...
    print(f"  DOW filter: exclude Tuesday")
...
def phase_4(df, df_1m, df_1s, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="general_v1 NQ_Asia robust pipeline",
    )
    if entry.warning:
...
def main():
    ...
    print(f"\n  Candidate: stop={CANDIDATE_SESSION.stop_orb_pct}% rr={CANDIDATE.rr} "
          f"gap={CANDIDATE_SESSION.min_gap_orb_pct}% tp1={CANDIDATE.tp1_ratio}")
    print(f"  Structural: ORB 15m, entry<22:30, flat 04:00, ATR={CANDIDATE.atr_length}, "
          f"dir={CANDIDATE.direction_filter}, DOW excl Tue")
    ...
    print(f"  Candidate: stop={CANDIDATE_SESSION.stop_orb_pct}% rr={CANDIDATE.rr} "
          f"gap={CANDIDATE_SESSION.min_gap_orb_pct}% tp1={CANDIDATE.tp1_ratio}")
