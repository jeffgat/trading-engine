#!/usr/bin/env python3
"""NQ Asia Grid Sweep R2 (RESTART) -- 4D grid centered on converged R9 anchor.

Anchor (from variable sweeps R9 restart):
  ORB: 20:00-20:15, entry until 22:30, flat 04:00-07:00
  stop=4.0%, rr=3.0, gap=0.9%, tp1=0.6
  ATR=5, direction=long, ICF=ON, continuation, 1s magnifier
  DOW gate: excl Tue
  max_gap_points=75.0
...
ANCHOR = StrategyConfig(
    ...
    name="NQ Asia Grid Sweep R2 (restart)",
)

# Grid dimensions -- narrow range centered on anchor
STOPS = [3.0, 3.5, 4.0, 5.0, 6.0]
RRS   = [2.5, 2.75, 3.0, 3.5, 4.0]
GAPS  = [0.5, 0.75, 0.9, 1.0, 1.25]
...
def main():
    print(f"{INSTRUMENT_NAME} {SESSION_NAME} -- Grid Sweep R2 ({len(GRID)} combos)")
    print("=" * 110)
