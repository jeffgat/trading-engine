#!/usr/bin/env python3
"""NQ NY Continuation — Grid Sweep R3 (Longs-Only Fresh Start).

Narrow 4D grid around converged R11 anchor:
  stop=[6,6.5,7,7.5,8], rr=[3.0,3.25,3.5,3.75,4.0], gap=[2.0,2.5,3.0], tp1=[0.3,0.35,0.4,0.45,0.5]
  Total: 5 × 5 × 3 × 5 = 375 combos

Structural params held fixed:
  ORB: 09:30-09:50 (20m), entry until 12:00, flat 15:30-16:00
  ATR=12, ICF=OFF, direction=long, continuation, 1s magnifier
  DOW gate: excl Fri
"""
...
# Grid dimensions — narrow around R11 anchor
...
def main():
    ...
    print(f"NQ NY ORB — Grid Sweep R3 (Longs-Only)")
    print(f"=" * 100)
    print(f"Grid: stop={STOP_VALUES} × rr={RR_VALUES} × gap={GAP_VALUES} × tp1={TP1_VALUES}")
    print(f"Total combos: {total_combos}")
    print(f"Structural: 20m ORB, entry<=12:00, flat=15:30, ATR=12, ICF=OFF, excl Fri, long-only")
