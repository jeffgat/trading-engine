#!/usr/bin/env python3
"""NQ Asia ORB — Variable sweeps round 2: re-sweep after R1 anchor change.

R1 adopted (Δ > +0.3, 0 new neg years):
  ORB window:  10m → 15m  (Δ+2.95, Calmar 19.68)
  entry_end:   23:00 → 01:00  (Δ+7.13, Calmar 23.85)

R1 NOT adopted:
  flat_start=23:00 (Δ+1.40) — conflicts with entry_end=01:00.
  Keeping flat_start=00:00 (from R1 anchor). Will re-sweep.

R2 anchor:
  ORB: 20:00-20:15 (15m), entry until 01:00, flat 00:00
...
# ── R2 anchor (adopted from R1: ORB=15m, entry_end=01:00) ─────────────
ANCHOR_SESSION = SessionConfig(
    name="Asia",
    orb_start="20:00",
    orb_end="20:15",         # ← R1 adopted (was 20:10)
    entry_start="20:15",     # ← R1 adopted (was 20:10)
    entry_end="01:00",       # ← R1 adopted (was 23:00)
    flat_start="00:00",
    flat_end="07:00",
    stop_atr_pct=3.7,
...
ANCHOR = StrategyConfig(
    ...
    tp1_ratio=0.35,
    atr_length=5,
    name="NQ Asia R2 Anchor",
)

ANCHOR_DOW_EXCL = {3}  # no-Thursday
...
def main():
    print("NQ Asia ORB — Round 2: re-sweep after R1 changes (ORB=15m, entry_end=01:00)")
    print("=" * 90)
    print(f"Anchor: rr={ANCHOR.rr}, tp1={ANCHOR.tp1_ratio}, "
          f"stop={ANCHOR_SESSION.stop_atr_pct}%, gap={ANCHOR_SESSION.min_gap_atr_pct}%")
    print(f"ORB=15m (20:00-20:15), entry≤01:00, flat=00:00, ATR={ANCHOR.atr_length}, "
    ...
    # ── 5. FLAT START ────────────────────────────────────────────────
    # With entry_end=01:00, sweep a wider range including post-entry values
    flat_starts = ["23:00", "23:30", "00:00", "00:30", "01:00", "01:30", "02:00", "03:00"]
    ...
    if not any_adopted:
        ...
        print(f"\n  ** NOT CONVERGED — Update anchor and re-sweep as R3 **")
