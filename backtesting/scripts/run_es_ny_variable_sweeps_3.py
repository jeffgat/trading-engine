#!/usr/bin/env python3
"""ES NY ORB Continuation — Variable Sweeps Round 3.

R2 adopted 7 dimensions. ORB reverted from 45m→15m (interaction effect).
Entry_end massively expanded from 11:00→14:00 (Calmar 17.51).

R3 anchor:
  ORB: 09:30-09:45 (15m), entry until 14:00, flat 15:50-16:00
  stop=3.0%, min_gap_atr=0.5%, max_gap_pts=100, max_gap_atr=0 (off)
  rr=5.0, tp1=0.4, ATR=10, direction=long, ICF=ON, continuation, 1s magnifier
...
ANCHOR_SESSION = SessionConfig(
    name="NY",
    orb_start="09:30",
    orb_end="09:45",         # 15m ORB (reverted from 45m in R2)
    entry_start="09:45",
    entry_end="14:00",       # adopted R2 (massive winner)
    flat_start="15:50",      # default
    flat_end="16:00",
    stop_atr_pct=3.0,        # adopted R1 (ES minimum floor)
    min_gap_atr_pct=0.5,     # adopted R1
...
ANCHOR = StrategyConfig(
    ...
    rr=5.0,                   # adopted R2
    tp1_ratio=0.4,            # adopted R2
    atr_length=10,            # adopted R2
    impulse_close_filter=True,  # adopted R2
    name="ES NY R3 Anchor",
)

ANCHOR_DOW_EXCL = {3, 4}  # excl Thu+Fri (adopted R2)
...
def main():
    print("ES NY ORB Continuation — Round 3: post-R2 anchor")
    print("=" * 90)
    print(f"Anchor: rr={ANCHOR.rr}, tp1={ANCHOR.tp1_ratio}, "
          f"stop={ANCHOR_SESSION.stop_atr_pct}%, gap={ANCHOR_SESSION.min_gap_atr_pct}%")
    print(f"ORB=15m (09:30-09:45), entry≤14:00, flat=15:50, ATR={ANCHOR.atr_length}, "
    ...
    # ── 3. ATR LENGTH ────────────────────────────────────────────────
    atr_values = [3, 5, 7, 10, 12, 14, 20]
    print_header("3. ATR LENGTH (anchor=10)")
    for i, atr in enumerate(atr_values, 1):
        config = replace(ANCHOR, atr_length=atr)
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"atr={atr}", m, is_base=(atr == 10))
        all_results.append(("atr_length", atr, m))

    # ── 4. ENTRY END TIME ────────────────────────────────────────────
    entry_ends = ["11:00", "12:00", "13:00", "14:00", "15:00", "15:30"]
    print_header("4. ENTRY END TIME (anchor=14:00)")
    for i, ee in enumerate(entry_ends, 1):
        sess = replace(ANCHOR_SESSION, entry_end=ee)
        config = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"end={ee}", m, is_base=(ee == "14:00"))
    ...
    # ── 7. REWARD:RISK ───────────────────────────────────────────────
    rr_values = [2.0, 3.0, 3.5, 4.0, 4.5, 5.0, 6.0, 7.0]
    ...
    print_header("8. TP1 RATIO (anchor=0.4)")
    ...
    # ── 9. MIN GAP ATR % ─────────────────────────────────────────────
    gap_values = [0.25, 0.5, 0.75, 1.0, 1.5, 2.0, 2.5, 3.0]
    print_header("9. MIN GAP ATR % (anchor=0.5%)")
    for i, g in enumerate(gap_values, 1):
        sess = replace(ANCHOR_SESSION, min_gap_atr_pct=g)
        config = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"gap={g}%", m, is_base=(abs(g - 0.5) < 0.01))
