#!/usr/bin/env python3
"""Robust pipeline: GC NY Continuation Shorts (clean data, 1s magnifier).

Converged anchor after R1-R4 variable sweeps + R1-R2 grid sweeps:
  stop=2.5% ATR, min_gap=5.5%, rr=7.0, tp1=0.6
  ATR 10, 15m ORB (09:30-09:45), entry→15:00, flat_start=15:50
  max_gap_atr=25%, short-only, FOMC dates excluded

Anchor evolution:
  R1: diagnostic → compound → sweeps (rr=5.0, gap=5.0, entry→15:00, ORB 15m)
  R2: re-sweep → all confirmed
  Grid R1: 450 combos → winner stop=3.0, rr=6.0, gap=5.5, tp1=0.6 (Calmar 10.94)
  R3: post-grid sweep → rr/tp1 beyond grid range
  Grid R2: 378 combos → winner stop=2.5, rr=7.0, gap=5.5, tp1=0.6 (Calmar 14.52)
  R4: post-grid R2 sweep → all 10 dimensions confirmed. Converged.
...
# ── Base config (R4 converged anchor) ────────────────────────────────────────
...
# ── WF sweep params (centred on R4 converged anchor) ────────────────────────

PARAM_RANGES = {
    "rr":                  [5.5, 6.0, 6.5, 7.0, 7.5],
    "tp1_ratio":           [0.5, 0.6],
    "ny_stop_atr_pct":     [2.0, 2.5, 3.0],
...
def phase1(df, df_1m, df_1s):
    ...
    print(f"  Config: stop=2.5% | rr=7.0 | min_gap=5.5% | tp1=0.6 | ATR 10 | max_gap_atr=25%")
...
def main():
    print()
    print("=" * 70)
    print("  ROBUST PIPELINE: GC NY Cont Shorts (clean data, 1s magnifier)")
    print("  stop=2.5% | rr=7.0 | min_gap=5.5% | tp1=0.6 | ATR 10")
    print("  max_gap_atr=25% | 15m ORB | entry→15:00 | FOMC excluded")
