...
export function useCoverage(): UseCoverageReturn {
  ...
  const refreshCoverage = useCallback(async () => {
    ...
    try {
      const [covRes, planRes] = await Promise.all([
        fetch("/bt-api/coverage"),
        fetch("/bt-api/testing-plan"),
      ]);
      if (covRes.ok) {
  ...
  const loadParamCoverage = useCallback(async (instrument: string) => {
    try {
      const res = await fetch(`/bt-api/coverage/${instrument}/params`);
      if (res.ok) {
  ...
  const createPlanItem = useCallback(
    async (
      ...
      try {
        const res = await fetch("/bt-api/testing-plan", {
          method: "POST",
  ...
  const updatePlanItem = useCallback(
    async (
      ...
      try {
        const res = await fetch(`/bt-api/testing-plan/${id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const deletePlanItem = useCallback(async (id: number) => {
    try {
      await fetch(`/bt-api/testing-plan/${id}`, { method: "DELETE" });
