#!/usr/bin/env python3
"""Robust pipeline: GC NY Continuation Longs — Round 5 config (1s magnifier).

Grid sweep winner (Round 5, ATR 14 + 10m ORB, 450 combos):
  stop=4.0% ATR, min_gap=2.5%, rr=4.5, tp1=0.5
  ATR length=14, ORB 09:30-09:40 (10 min), entry→12:00, flat 15:50
  Full IS: 810 trades, 37.4% WR, 178.2R, Calmar 10.71, DD -16.6R

WF sweep focuses on the winning region found in grid exploration.
...
# ── Base config: Round 5 grid winner ──────────────────────────────────────────

GC_NY_R5 = SessionConfig(
    name="NY",
    orb_start="09:30",
    orb_end="09:40",        # 10-min ORB (fine-tune winner)
    entry_start="09:40",
    entry_end="12:00",
    flat_start="15:50",
    flat_end="16:00",
...
BASE_CONFIG = StrategyConfig(
    rr=4.5,                 # grid winner
    tp1_ratio=0.5,          # grid winner
    risk_usd=5000.0,
    atr_length=14,          # ATR fine-tune winner
    min_qty=1.0,
    qty_step=1.0,
    sessions=(GC_NY_R5,),
    instrument=GC,
...
# ── Pipeline thresholds ───────────────────────────────────────────────────────
# Win rate threshold relaxed: rr=4.5 strategy naturally produces ~35-38% WR.
# High rr = fewer wins needed; Calmar is the real gate.
...
# ── WF sweep: focused on winning region from Round 5 grid ─────────────────────

PARAM_RANGES = {
    "rr":                 [3.5, 4.0, 4.5, 5.0],
    "tp1_ratio":          [0.4, 0.5],
    "ny_stop_atr_pct":    [3.0, 3.5, 4.0, 4.5, 5.0],
...
WF_WORKERS = 1  # 1s data: pass maps by reference, no IPC overhead
...
def phase1(df, df_1m, df_1s):
    ...
    print(f"  Config: ATR 14 | 10m ORB | stop=4.0% | min_gap=2.5% | rr=4.5 | tp1=0.5 | entry→12:00")
...
def phase5(wf_trades):
    ...
    print(f"  Ruin threshold: -{MC_RUIN_THRESHOLD_R}R  (standalone)")
    print()
...
def main():
    print()
    print("=" * 70)
    print("  ROBUST PIPELINE: GC NY Continuation Longs — Round 5 (1s magnifier)")
    print("  Config: ATR 14 | 10m ORB | stop=4.0% | min_gap=2.5% | rr=4.5 | tp1=0.5")
    print(f"  WF sweep: {GRID_SIZE} combos/fold | 36m IS / 12m OOS")
    print("=" * 70)
    print()
