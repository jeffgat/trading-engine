#!/usr/bin/env python3
"""Compare session-extreme data-sweep variants on the frozen NQ NY 2m anchor."""
...
OUTPUT_DIR = RESULTS_ROOT / "nq_ny_htf_lsi_2m_data_session_extreme_compare"
REPORT_PATH = ROOT / "learnings" / "reports" / "NQ_NY_HTF_LSI_2M_DATA_SESSION_EXTREME_COMPARE.md"
    ...
    return [
        build_config(
            **base_kwargs,
            ...
            name="NQ NY HTF_LSI 2m anchor data-session-extreme htf_only",
        ),
        build_config(
            **base_kwargs,
            htf_lsi_include_htf_levels=False,
            htf_lsi_reference_levels=DATA_LEVELS,
            name="NQ NY HTF_LSI 2m anchor data-session-extreme data_only",
        ),
        build_config(
            **base_kwargs,
            htf_lsi_include_htf_levels=True,
            htf_lsi_reference_levels=DATA_LEVELS,
            name="NQ NY HTF_LSI 2m anchor data-session-extreme htf_plus_data",
        ),
    ]
...
def _variant_key(config) -> str:
    if config.htf_lsi_include_htf_levels and config.htf_lsi_reference_levels:
        return "htf_plus_data_session_extreme"
    if config.htf_lsi_include_htf_levels:
        return "htf_only"
    return "data_only_session_extreme"
...
def _write_report(rows: list[dict]) -> None:
    lines = [
        ...
        f"- Data-level basket: `{', '.join(DATA_LEVELS)}` at `data_sweep_min_daily_atr_pct={DATA_SWEEP_MIN_DAILY_ATR_PCT}` with `data_sweep_require_session_extreme=True`.",
...
def main() -> int:
    ...
    print("Running NQ NY HTF-LSI 2m data session-extreme comparison...", flush=True)
    results = run_sweep(
        df_base,
        configs,
