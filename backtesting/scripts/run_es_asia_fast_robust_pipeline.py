#!/usr/bin/env python3
"""ES Asia FAST — 5-Phase Robust Pipeline (Bailey-aware).

FAST execution config ES_Asia leg:
  ORB: 20:00-20:15, entry until 03:00, flat 07:00
  stop=125% ORB range, min_gap=0.5% ATR, rr=1.5, tp1=0.7
  Long only, hybrid (ORB-based stop + ATR-based gap filter)
...
# ── Candidate config (FAST execution ES_Asia) ──────────────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="ES Asia FAST Robust Pipeline",
)

# ── Walk-forward param ranges (centered on candidate) ────────────────────
PARAM_RANGES = {
    "asia_stop_orb_pct": [75.0, 100.0, 125.0, 150.0, 175.0],
    "rr": [1.0, 1.25, 1.5, 1.75, 2.0],
    "asia_min_gap_atr_pct": [0.25, 0.5, 0.75, 1.0],
    "tp1_ratio": [0.5, 0.6, 0.7, 0.8, 0.9],
}
...
def phase_4(df, df_1m, df_1s, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST ES_Asia robust pipeline",
    )
    if entry.warning:
...
def main():
    ...
    print(f"\n  Candidate: stop_orb={CANDIDATE_SESSION.stop_orb_pct}% rr={CANDIDATE.rr} "
          f"gap_atr={CANDIDATE_SESSION.min_gap_atr_pct}% tp1={CANDIDATE.tp1_ratio}")
    print(f"  Structural: ORB 15m, entry<03:00, flat 07:00, ATR={CANDIDATE.atr_length}, "
          f"dir={CANDIDATE.direction_filter}, min_stop={CANDIDATE_SESSION.min_stop_points}pt, "
          f"min_tp1={CANDIDATE_SESSION.min_tp1_points}pt")
    ...
    print(f"  Candidate: stop_orb={CANDIDATE_SESSION.stop_orb_pct}% rr={CANDIDATE.rr} "
          f"gap_atr={CANDIDATE_SESSION.min_gap_atr_pct}% tp1={CANDIDATE.tp1_ratio}")
