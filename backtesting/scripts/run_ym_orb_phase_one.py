#!/usr/bin/env python3
"""Phase-One Robust Pipeline — YM ORB Continuation promoted candidates.

3 promoted from discovery pipeline (all LDN longs, 30m ORB):
  LDN-3: ATR 15%, RR=2.0, TP1=0.6, long  (WF Cal 4.98, DD -11.4R)
  LDN-2: ATR 15%, RR=2.5, TP1=0.5, long  (WF Cal 4.38, DD -13.9R)
  LDN-1: ORB 25%, RR=3.5, TP1=0.6, long  (WF Cal 4.37, DD -18.7R)
...
OUTPUT_DIR = ROOT / "data" / "results" / "ym_orb_phase_one"
HOLDOUT_START = "2025-01-01"
HOLDOUT_END = "2026-03-01"
N_DISCOVERY_CONFIGS = 1296  # per-session sweep count for DSR
...
# ---------------------------------------------------------------------------
# 3 promoted candidates (all LDN longs, 30m ORB)
# ---------------------------------------------------------------------------
...
def main():
    print("Phase-One Robust Pipeline — YM LDN ORB Continuation (3 candidates)")
    print("=" * 70)
    ...
    # Build trading date lists from the 5m data
    dates_all = pd.Series(df_5m.index.date).drop_duplicates().astype(str).tolist()
    ho_start = pd.Timestamp(HOLDOUT_START)
    ...
    for cand_name, config in CANDIDATES.items():
        ...
        # Phase 2: Walk-forward (reuse discovery pipeline results)
        print(f"\n  [Phase 2] Walk-forward — see discovery pipeline results (pipeline_results.json)")
    ...
    # Summary
    print(f"\n{'=' * 70}")
    print("PHASE-ONE SUMMARY — YM LDN ORB Continuation")
    print(f"{'=' * 70}")
