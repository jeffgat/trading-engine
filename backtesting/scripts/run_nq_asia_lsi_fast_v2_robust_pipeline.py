#!/usr/bin/env python3
"""NQ Asia LSI FAST_V2 — 5-Phase Robust Pipeline (Bailey-aware).

FAST_V2 execution config NQ_Asia_LSI leg:
  ORB: 20:00-20:05, entry 20:45-22:00, flat 00:00-00:10
  min_gap=1.75%, rr=1.75, tp1=0.7
...
# ── Candidate config (FAST_V2 execution NQ_Asia_LSI) ─────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ Asia LSI FAST_V2 Robust Pipeline",
)
...
def phase_4(df, df_1m, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST_V2 NQ_Asia_LSI robust pipeline",
    )
    if entry.warning:
...
def main():
    ...
    print(f"  Structural: LSI strategy, entry 20:45-22:00, flat 00:00, ATR={CANDIDATE.atr_length}, "
