#!/usr/bin/env python3
"""RTY NY Continuation — Variable Sweeps Round 2.

R2 anchor (adopted from R1):
  direction=long (shorts destructive -19.3R)
  rr=4.0 (Calmar Δ +0.32 > +0.3)
  Everything else: defaults (stop=7.5%, gap=2.25%, tp1=0.5, ATR=14, 15m ORB, entry≤12:00)
...
def main():
    ...
    # ── Anchor ──
    print("=" * 100)
    print("  ANCHOR (R2: long-only, rr=4.0)")
    print("=" * 100)
    ...
    # ── 4. Stop ATR % ──
    print("=" * 100)
    print("  SWEEP 4: Stop ATR %")
    print("=" * 100)
    print(HDR)
    for i, s in enumerate([3.0, 4.0, 5.0, 6.0, 7.5, 9.0, 10.0, 12.0, 15.0], 1):
        cfg = with_overrides(ANCHOR, ny_stop_atr_pct=s)
        _, m = run_and_metric(df, df_1m, cfg)
        print(fmt(i, f"stop={s}%", m))
    print()

    # ── 5. R:R (fine-tune around 4.0) ──
    print("=" * 100)
    print("  SWEEP 5: Risk-Reward Ratio (fine-tune)")
    print("=" * 100)
    print(HDR)
