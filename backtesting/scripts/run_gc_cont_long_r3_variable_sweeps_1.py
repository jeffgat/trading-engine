#!/usr/bin/env python3
"""GC NY Continuation Longs — R3 Round 1 Variable Sweeps (post fill-bar fix).

Fresh optimization starting from the fill-bar fix baseline:
  DB run #10796: "GC NY Cont Longs R2 Final (WF Mode) [fill-bar fix]"
  440 trades, 42.0% WR, 139.9R, Calmar 15.75, DD -8.88R

Anchor (fill-bar fix WF mode params):
  rr=4.5, tp1=0.5, stop=3.0%, min_gap=3.5%, ATR 10
  10m ORB (09:30-09:40), entry→11:00, flat_start=15:50
...
Sweeps each dimension independently:
  ...
  2. ATR length         — 5, 8, 10*, 12, 14, 16, 18, 20, 25, 30
...
END_DATE = "2026-02-19"
FULL_YEARS = [str(y) for y in range(2017, 2026)]  # GC data starts ~2017

# ── Anchor config (fill-bar fix baseline, WF mode params) ────────────────────
...
GC_NY_ANCHOR = SessionConfig(
    ...
    flat_start="15:50",
    flat_end="16:00",
    stop_atr_pct=3.0,         # WF mode (was 4.0% structural)
    min_gap_atr_pct=3.5,
)

ANCHOR = StrategyConfig(
    rr=4.5,                    # WF mode (was 4.0 structural)
    tp1_ratio=0.5,
    risk_usd=5000.0,
    atr_length=10,
    min_qty=1.0,
...
def run(cfg, excluded_dow=None):
    """Run backtest with 1s magnifier, optionally filter by DOW."""
...
def make_session(**overrides):
    """Create a SessionConfig from anchor with overrides."""
    defaults = dict(
...
def sweep_atr_length(anchor_calmar):
    ...
    lengths = [5, 8, 10, 12, 14, 16, 18, 20, 25, 30]
    results = []
    for atr in lengths:
...
def print_anchor_metrics(m):
    section("ANCHOR METRICS (fill-bar fix baseline)")
    print(f"  Config: stop=3.0% | rr=4.5 | min_gap=3.5% | tp1=0.5 | ATR 10")
    print(f"          10m ORB | entry→11:00 | long-only | FOMC excl | max_gap_atr=30% | 1s")
    print(f"  Source: DB #10796 'GC NY Cont Longs R2 Final (WF Mode) [fill-bar fix]'")
    print()
...
if __name__ == "__main__":
    print()
    print("=" * 70)
    print("  GC NY CONT LONGS — R3 VARIABLE SWEEPS Round 1 (post fill-bar fix)")
    print("  Anchor: stop=3.0% | rr=4.5 | min_gap=3.5% | tp1=0.5 | ATR 10")
    print("          10m ORB | entry→11:00 | FOMC excl | long-only | max_gap_atr=30%")
    print("  Baseline: DB #10796 (Calmar 15.75, DD -8.88R, 440 trades)")
    print("=" * 70)
    ...
    print("  If anchor changes, re-run sweeps on new anchor (R3 sweeps_2).")
    print("=" * 70)
    print()
