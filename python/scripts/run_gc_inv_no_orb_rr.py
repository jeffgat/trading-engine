#!/usr/bin/env python3
"""No-ORB GC — sweep R:R ratio.

Fixes QM=100%, stop=12%, entry_end=20:00, longs only.
Tests rr: 2.0, 2.5, 3.0, 3.5, 4.0, 4.5, 5.0
"""
...
MIN_GAP  = 1.0
MAX_GAP  = 25.0
RR_VALS  = [2.0, 2.5, 3.0, 3.5, 4.0, 4.5, 5.0]
...
def main():
    ...
    print("=" * 100)
    print("NO-ORB GC — R:R SWEEP (QM=100%, stop=12%, entry_end=20:00, longs)")
    print("=" * 100)
    ...
    print(f"\nBest RR: {best['rr']:.1f} — {best['trades']} trades, "
          f"{best['net_r']}R, {best['max_dd']}R DD, Sharpe {best['sharpe']:.3f}")

    print(f"\n{'='*60}")
    print("YEARLY — best RR config")
    print(f"{'='*60}")
    if "yearly" in best:
