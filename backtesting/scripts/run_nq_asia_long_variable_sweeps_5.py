#!/usr/bin/env python3
"""NQ Asia Long — Variable Sweeps Round 5 (Full ORB Anchor).

R5 anchor (R4 + adoptions):
  ORB: 20:00-20:15, entry until 00:00, flat 00:00-07:00
  stop_orb_pct=100%, min_gap_orb_pct=5%
  rr=6.0, tp1=0.7, ATR=5, direction=long, ICF=OFF, continuation, 1s magnifier
  DOW gate: excl Tue

R4 adoptions applied:
  min_gap_orb_pct: 7% -> 5% (+0.46)
  rr: 3.0 -> 6.0 (+4.53)
  tp1_ratio: 0.3 -> 0.7 (+4.41)

NOTE: rr/tp1 oscillation observed (R3: 5.0/0.6, R4: 3.0/0.3, R5: 6.0/0.7).
These dimensions interact strongly. If oscillation continues, lock them and
proceed to grid sweep to explore the surface jointly.
...
ANCHOR = StrategyConfig(
    ...
    name="NQ Asia Long R5 Anchor (full ORB)",
)
...
def main():
    ...
    # -- 7. REWARD:RISK --------------------------------------------------------
    rr_vals = [2.0, 2.5, 3.0, 3.5, 4.0, 5.0, 6.0, 7.0, 8.0]
