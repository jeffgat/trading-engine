#!/usr/bin/env python3
"""NQ NY LSI FAST_V2 — 5-Phase Robust Pipeline (Bailey-aware).

FAST_V2 execution config NQ_NY_LSI leg:
  ORB: 09:30-09:45, entry 10:10-14:30, flat 15:50-16:00
  min_gap=3.75%, rr=2.5, tp1=0.2
  Long only, Wednesday + Thursday excluded
...
from orb_backtest.analysis.gates import WED, THU, apply_dow_filter
...
# ── Candidate config (FAST_V2 execution NQ_NY_LSI) ───────────────────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ NY LSI FAST_V2 Robust Pipeline",
)

# ── Walk-forward param ranges (100 combos/fold) ──────────────────────────
PARAM_RANGES = {
    "rr": [2.0, 2.5, 3.0, 3.5, 4.0],
    "tp1_ratio": [0.3, 0.35, 0.4, 0.5],
    "ny_min_gap_atr_pct": [3.0, 3.5, 3.75, 4.0, 4.5],
}
...
GRID_SIZE = _rr_tp1_valid * len(PARAM_RANGES["ny_min_gap_atr_pct"])
...
def phase_1(df, df_1m):
    section("PHASE 1: STRUCTURAL VALIDATION")
    print(f"  Running full pre-holdout backtest with 1m magnifier...")
    print(f"  DOW filter: exclude Wednesday + Thursday")
...
def phase_4(df, df_1m, stability):
    ...
    entry = log_holdout_test(
        ...
        config=mode_params,
        experiment_name="FAST_V2 NQ_NY_LSI robust pipeline",
    )
    if entry.warning:
...
def main():
    ...
    print(f"\n  Candidate: rr={CANDIDATE.rr} "
          f"gap={CANDIDATE_SESSION.min_gap_atr_pct}% tp1={CANDIDATE.tp1_ratio}")
    print(f"  Structural: LSI strategy, entry 10:10-14:30, flat 15:50, ATR={CANDIDATE.atr_length}, "
          f"dir={CANDIDATE.direction_filter}, DOW excl Wed+Thu")

    results = {}

    # Phase 1
    p1_pass, p1_trades, p1_m = phase_1(df_pre, df_pre_1m)
    ...
    # Phase 4
    p4_pass, p4_trades, p4_m = phase_4(df_5m, df_1m, stability)
    ...
    print(f"  Candidate: rr={CANDIDATE.rr} "
          f"gap={CANDIDATE_SESSION.min_gap_atr_pct}% tp1={CANDIDATE.tp1_ratio}")
