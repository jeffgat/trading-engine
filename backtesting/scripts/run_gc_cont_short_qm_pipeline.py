#!/usr/bin/env python3
"""Robust pipeline: GC NY Inversion Shorts with qualifying move gate.
...
Uses the qualifying-move engine: price must first extend upward to
orb_high + (qualifying_move_atr_pct / 100) * ATR before accepting a
short inversion entry. Mean-reversion concept — fade only proven moves.
"""
...
PARAM_RANGES = {
    ...
    "ny_min_gap_atr_pct": [0.75, 1.0, 1.5],
    "ny_qualifying_move_atr_pct": [25, 50, 75, 100],
}
...
def phase1(df, df_1m):
    ...
    print(f"  Config: GC NY Inversion Shorts + QM gate (qm_atr={GC_NY_INVERSION.qualifying_move_atr_pct}%)")
...
def main():
    print()
    print("=" * 70)
    print("  ROBUST PIPELINE: GC NY Inversion Shorts + Qualifying Move Gate")
    print("=" * 70)
    print()
