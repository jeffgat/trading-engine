#!/usr/bin/env python3
"""ES NY Micro-Tune Grid — 0.1 resolution on stop × rr × gap.

Anchor updated from R7: max_gap_points=20 adopted.
Centered on fine-tune grid winner: stop=2.5%, rr=5.5, gap=1.5%

Grid: stop(7) × rr(11) × gap(7) = 539 combos
1s bar magnifier, tp1=0.3, ATR=3 locked.

Structural: 25m ORB | entry<=13:00 | long-only | ICF=ON | flat=15:50 | excl Thu+Fri
"""
...
ANCHOR_SESSION = SessionConfig(
    ...
    stop_atr_pct=2.5,
    min_gap_atr_pct=1.5,
    max_gap_points=20.0,       # adopted from R7
    max_gap_atr_pct=0.0,
)
...
ANCHOR = StrategyConfig(
    ...
    name="ES NY Micro Tune",
)

# ── Micro-tune grid: 0.1 step ────────────────────────────────────────────────

STOPS = [2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7]                              # 7
RRS   = [5.0, 5.1, 5.2, 5.3, 5.4, 5.5, 5.6, 5.7, 5.8, 5.9, 6.0]        # 11
GAPS  = [1.1, 1.2, 1.3, 1.4, 1.5, 1.6, 1.7]                              # 7

GRID = list(product(STOPS, RRS, GAPS))
TOTAL = len(GRID)
...
def main():
    print()
    print("=" * 110)
    print(f"  ES NY MICRO-TUNE GRID: {TOTAL:,} combos "
          f"({len(STOPS)}x{len(RRS)}x{len(GAPS)})")
    print(f"  Anchor: stop=2.5% | rr=5.5 | gap=1.5% | tp1=0.3 | ATR=3 | maxgap=20")
    ...
    for i, (stop, rr, gap) in enumerate(GRID):
        sess = replace(ANCHOR_SESSION, stop_atr_pct=stop, min_gap_atr_pct=gap)
        cfg = replace(ANCHOR, sessions=(sess,), rr=rr)
    ...
    # ── Table header ──
    HDR = (f"  {'Rank':>4} {'stop':>5} {'rr':>5} {'gap':>5} "
    ...
    for rank, r in enumerate(results[:30], 1):
        is_anchor = (abs(r["stop"] - 2.5) < 0.01 and abs(r["rr"] - 5.5) < 0.01
                     and abs(r["gap"] - 1.5) < 0.01)
    ...
    for rank, r in enumerate(results, 1):
        if (abs(r["stop"] - 2.5) < 0.01 and abs(r["rr"] - 5.5) < 0.01
                and abs(r["gap"] - 1.5) < 0.01):
    ...
    for dim_name, dim_values, dim_key in [
        ("stop", STOPS, "stop"), ("rr", RRS, "rr"), ("gap", GAPS, "gap"),
    ]:
