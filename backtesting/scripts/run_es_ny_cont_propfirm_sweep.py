#!/usr/bin/env python3
"""Prop firm phase 1 sweep: ES NY Long ORB Final (continuation)

Anchor from saved config: bt-es-ny-cont-long-2016-2026-final-650260
Notes: min_stop=3.0, min_tp1=3.0, DOW excl Thu.
Optimizes for Calmar + prop firm success rate (-4R/+5R biweekly accounts).
...
PARAM_GRID = {
    ...
    "tp1_ratio": [0.15, 0.2, 0.25, 0.3, 0.4],
    "ny_stop_atr_pct": [3.0, 4.0, 5.0, 6.0, 7.0],
    "ny_min_gap_atr_pct": [0.0, 0.25, 0.5, 1.0],
}

# DOW variants
DOW_VARIANTS = {
    "thu_excl": (3,),       # anchor — Thu excluded
    "no_excl": (),           # all days
}
...
def config_label(cfg: StrategyConfig) -> str:
    ...
    dow = "noThu" if excl == (3,) else "ALL" if not excl else str(excl)
    return f"rr={cfg.rr} tp1={cfg.tp1_ratio} stop={cfg.sessions[0].stop_atr_pct} gap={cfg.sessions[0].min_gap_atr_pct} {dow}"
...
def main():
    ...
    for dow_name, excl_days in DOW_VARIANTS.items():
    ...
    print(f"  Model: new account every {CYCLE_DAYS} days, +{PAYOUT_TARGET}R payout / {BREACH_LIMIT}R breach")
    ...
    for cfg, trades in results:
        ...
        rows.append({
            ...
            "stop_atr": cfg.sessions[0].stop_atr_pct,
            "gap": cfg.sessions[0].min_gap_atr_pct,
            "excluded_days": cfg.excluded_days,
            "dow": "noThu" if cfg.excluded_days == (3,) else "ALL" if not cfg.excluded_days else str(cfg.excluded_days),
            # Standard metrics
    ...
    # Filter: >= 20 trades (2yr window)
    rows = [r for r in rows if r["trades"] >= 20]
    ...
    print(f"\n{'=' * 160}")
    print("TOP 25 BY CALMAR — ES NY Long ORB Continuation")
    print(f"{'=' * 160}")
    print(f"{'#':>3} {'Config':<45} {'Tr':>4} {'WR%':>6} {'NetR':>6} {'MaxDD':>6} {'Calm':>5} {'Shrp':>5} "
    ...
    for i, r in enumerate(rows_by_calmar[:25]):
        ...
        print(
            f"{i+1:>3} {r['label']:<45} {r['trades']:>4} {r['win_rate']:>5.1%} {r['net_r']:>6.1f} "
    ...
    print(f"\n{'=' * 160}")
    print("TOP 25 BY PROP FIRM SUCCESS — ES NY Long ORB Continuation")
    print(f"{'=' * 160}")
    print(f"{'#':>3} {'Config':<45} {'Tr':>4} {'WR%':>6} {'NetR':>6} {'Calm':>5} "
    ...
    for i, r in enumerate(rows_by_propfirm[:25]):
        ...
        print(
            f"{i+1:>3} {r['label']:<45} {r['trades']:>4} {r['win_rate']:>5.1%} {r['net_r']:>6.1f} {r['calmar']:>5.2f} "
    ...
    output_path = Path(__file__).parent.parent / "data" / "results" / "es_ny_cont_propfirm_sweep.json"

    save_data = {
        "sweep_info": {
            "anchor_name": "ES NY Long ORB Final",
            ...
            "dow_variants": list(DOW_VARIANTS.keys()),
