#!/usr/bin/env python3
"""ES NY ORB Continuation — Variable Sweeps Round 2.

R1 adopted 9 dimensions. New anchor:
  ORB: 09:30-10:15 (45m), entry until 11:00, flat 15:50-16:00
  stop=3.0%, min_gap_atr=0.5%, max_gap_pts=100, max_gap_atr=0 (off)
  rr=3.5, tp1=0.2, ATR=5, direction=long, ICF=OFF, continuation, 1s magnifier
  DOW gate: excl Thu
...
ANCHOR_SESSION = SessionConfig(
    name="NY",
    orb_start="09:30",
    orb_end="10:15",         # 45m ORB (adopted R1)
    entry_start="10:15",
    entry_end="11:00",       # adopted R1 (early cutoff)
    flat_start="15:50",      # default
    flat_end="16:00",
    stop_atr_pct=3.0,        # adopted R1 (ES minimum floor)
    min_gap_atr_pct=0.5,     # adopted R1
    max_gap_points=100.0,    # default (insensitive)
...
ANCHOR = StrategyConfig(
    ...
    rr=3.5,                   # adopted R1
    tp1_ratio=0.2,            # adopted R1
    atr_length=5,             # adopted R1
    impulse_close_filter=False,
    name="ES NY R2 Anchor",
)

ANCHOR_DOW_EXCL = {3}  # excl Thu (adopted R1)
...
def main():
    print("ES NY ORB Continuation — Round 2: post-R1 anchor")
    print("=" * 90)
    print(f"Anchor: rr={ANCHOR.rr}, tp1={ANCHOR.tp1_ratio}, "
          f"stop={ANCHOR_SESSION.stop_atr_pct}%, gap={ANCHOR_SESSION.min_gap_atr_pct}%")
    print(f"ORB=45m (09:30-10:15), entry≤11:00, flat=15:50, ATR={ANCHOR.atr_length}, "
          f"dir={ANCHOR.direction_filter}, excl Thu, ICF=OFF")
    ...
    # ── 1. STOP ATR % ────────────────────────────────────────────────
    stop_values = [3.0, 4.0, 5.0, 6.0, 7.0, 8.0, 10.0, 12.0]
    ...
    print_header("2. ORB WINDOW (anchor=45m)")
    for i, (label, orb_s, orb_e, entry_s) in enumerate(orb_windows, 1):
        ...
        config = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"orb={label}", m, is_base=(label == "45m"))
    ...
    # ── 4. ENTRY END TIME ────────────────────────────────────────────
    entry_ends = ["10:30", "11:00", "11:30", "12:00", "13:00", "14:00", "15:30"]
    print_header("4. ENTRY END TIME (anchor=11:00)")
    for i, ee in enumerate(entry_ends, 1):
        sess = replace(ANCHOR_SESSION, entry_end=ee)
        config = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"end={ee}", m, is_base=(ee == "11:00"))
        all_results.append(("entry_end", ee, m))

    # ── 5. FLAT START ────────────────────────────────────────────────
    flat_starts = ["13:00", "14:00", "14:30", "15:00", "15:30", "15:50"]
    ...
    print_header("10. DOW EXCLUSION (anchor=excl Thu)")
