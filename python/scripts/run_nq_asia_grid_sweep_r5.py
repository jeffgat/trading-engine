#!/usr/bin/env python3
"""NQ Asia ORB — Grid sweep R5 (post same-candle SL/TP bug fix).

Converged structural anchor from R1-R5 variable sweeps:
  ORB: 20:00-20:10 (10m), entry until 00:00, flat 01:00
  ATR=5, direction=both, continuation, 1s magnifier, no-Thursday, ICF=OFF

Current continuous params: stop=3.7% rr=1.75 gap=0.90% tp1=0.10

Grid (broad, centered on R5 anchor):
  stop:  [2.5, 3.0, 3.5, 3.7, 4.0, 4.5, 5.0, 6.0]
  rr:    [1.25, 1.5, 1.75, 2.0, 2.25, 2.5, 3.0]
  gap:   [0.50, 0.75, 0.90, 1.10, 1.25, 1.50]
  tp1:   [0.05, 0.08, 0.10, 0.12, 0.15, 0.20]
  Total: 8 x 7 x 6 x 6 = 2,016 combos
"""
...
# Structural params (frozen from R5 variable sweeps)
...
BASE_CONFIG = StrategyConfig(
    ...
    tp1_ratio=0.10,          # placeholder — overridden
    atr_length=5,
    name="NQ Asia Grid R5",
)

# Grid dimensions (centered on anchor, finer resolution around tp1=0.10)
STOPS = [2.5, 3.0, 3.5, 3.7, 4.0, 4.5, 5.0, 6.0]
RRS = [1.25, 1.5, 1.75, 2.0, 2.25, 2.5, 3.0]
GAPS = [0.50, 0.75, 0.90, 1.10, 1.25, 1.50]
TP1S = [0.05, 0.08, 0.10, 0.12, 0.15, 0.20]
...
def main():
    ...
    print(f"NQ Asia ORB — Grid Sweep R5: {total} combos")
    print(f"  stop: {STOPS}")
    print(f"  rr:   {RRS}")
    print(f"  gap:  {GAPS}")
    print(f"  tp1:  {TP1S}")
    ...
    for i, row in enumerate(scored[:30], 1):
        ...
        marker = " <--" if (abs(row["stop"] - 3.7) < 0.01 and abs(row["rr"] - 1.75) < 0.01 and
                            abs(row["gap"] - 0.90) < 0.01 and abs(row["tp1"] - 0.10) < 0.01) else ""
    ...
    ref = [s for s in scored if abs(s["stop"] - 3.7) < 0.01 and abs(s["rr"] - 1.75) < 0.01 and
           abs(s["gap"] - 0.90) < 0.01 and abs(s["tp1"] - 0.10) < 0.01]
    if ref:
        m = ref[0]["metrics"]
        ...
        rank_zero = next((i for i, s in enumerate(zero_neg, 1)
                          if abs(s["stop"] - 3.7) < 0.01 and abs(s["rr"] - 1.75) < 0.01 and
                          abs(s["gap"] - 0.90) < 0.01 and abs(s["tp1"] - 0.10) < 0.01), None)
        print(f"\n  R5 anchor (stop=3.7 rr=1.75 gap=0.90 tp1=0.10): "
    ...
    anchor_changed = not (abs(winner["stop"] - 3.7) < 0.01 and abs(winner["rr"] - 1.75) < 0.01 and
                          abs(winner["gap"] - 0.90) < 0.01 and abs(winner["tp1"] - 0.10) < 0.01)
    if anchor_changed:
        print(f"\n  ** ANCHOR MOVED — need to re-sweep variable sweeps on new winner **")
    else:
        print(f"\n  ** ANCHOR STABLE — ready for walk-forward **")
