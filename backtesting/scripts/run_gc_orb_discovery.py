#!/usr/bin/env python3
"""GC ORB Discovery — 3-session sweep with regime gate + 1s magnifier.

Starting thesis: ORB continuation on GC across NY, Asia, LDN sessions.
Baseline: RR=2.0, TP1=0.5, stop=10% ATR, min_gap=1% ATR.
Also sweeps ORB range % stops, both directions, multiple ORB windows.

Pre-holdout only (<2024-03). Uses 1s magnifier (required for GC accuracy).
Note: GC learnings say Asia/LDN are thin — this sweep tests that assumption.
"""
...
OUTPUT_DIR = ROOT / "data" / "results" / "gc_orb_discovery_3session"
HOLDOUT_START = REGIME_RESEARCH_HOLDOUT_START
AVOID_BUCKETS = {"bull_medium_vol", "sideways_medium_vol"}
N_WORKERS = 1  # Sequential — 1s magnifier maps too large for multiprocessing
    ...
    for orb_min in ORB_MINUTES:
        for stop_mode in ["atr", "orb"]:
            ...
            base = StrategyConfig(
                ...
                tp1_ratio=0.5,
                atr_length=14,
                name=f"GC {session_name} ORB{orb_min}m {stop_mode}-stop",
            )
...
def main():
    print("GC ORB Discovery — 3-Session Sweep (1s magnifier)")
    print("=" * 70)
    ...
    print(f"Bar magnifier: ON (1s) — required for GC accuracy")
