#!/usr/bin/env python3
"""Step 2 — Variable Sweeps Round 2: NQ LDN Continuation Longs.

Anchor (from R1 adoptions):
  stop=2.0% ATR, rr=5.0, tp1=0.4, ATR 14, min_gap=1.0% ATR
  ICF=False, 45m ORB (03:00-03:45), entry->08:25, flat 08:20, long-only
...
NQ_LDN = SessionConfig(
    name="LDN",
    orb_start="03:00",
    orb_end="03:45",       # 45m ORB (adopted R1)
    entry_start="03:45",
    entry_end="08:25",
    flat_start="08:20",
    flat_end="08:25",
    stop_atr_pct=2.0,      # adopted R1
    min_gap_atr_pct=1.0,
)

ANCHOR = StrategyConfig(
    rr=5.0,                 # adopted R1
    tp1_ratio=0.4,          # adopted R1
    risk_usd=5000.0,
    atr_length=14,
    sessions=(NQ_LDN,),
    instrument=NQ,
...
def sweep_stop_atr():
    values = [1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0, 7.5, 10.0]
    results = []
    for v in values:
...
def sweep_rr():
    values = [2.0, 3.0, 3.5, 4.0, 4.5, 5.0, 6.0, 7.0, 8.0]
    results = []
    for v in values:
...
if __name__ == "__main__":
    print("\n" + "=" * 90)
    print("  NQ LDN CONTINUATION LONGS — VARIABLE SWEEPS ROUND 2")
    print("  Anchor: stop=2.0% ATR, rr=5.0, tp1=0.4, ATR 14, gap=1.0% ATR")
    print("          ICF=False, 45m ORB (03:00-03:45), entry->08:25, flat 08:20, long-only")
    print("=" * 90)
    ...
    if adoptions:
        ...
        print(f"\n  -> Anchor changed. Must re-sweep all 13 dimensions (Round 3).")
    else:
