#!/usr/bin/env python3
"""NQ Asia Long — Variable Sweeps Round 3 (Full ORB Anchor).

R3 anchor (R2 + adoptions):
  ORB: 20:00-20:15, entry until 22:00, flat 04:00-07:00
  stop_orb_pct=100%, min_gap_orb_pct=15%
  rr=5.0, tp1=0.6, ATR=5, direction=long, ICF=OFF, continuation, 1s magnifier
  DOW gate: excl Tue

R2 adoptions applied:
  min_gap_orb_pct: 10% -> 15% (+2.97)
  entry_end: 22:30 -> 22:00 (+1.33)
  rr: 3.0 -> 5.0 (+2.89)
  ICF: ON -> OFF (+6.04)
...
ANCHOR = StrategyConfig(
    ...
    name="NQ Asia Long R3 Anchor (full ORB)",
)
...
def main():
    ...
    # -- 2. MIN GAP ORB % -----------------------------------------------------
    gap_orb_vals = [5, 7, 10, 12, 15, 18, 20, 25]
