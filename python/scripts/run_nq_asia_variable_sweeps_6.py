#!/usr/bin/env python3
"""NQ Asia ORB — Variable sweeps round 6 (R2): post bug fix, updated anchor.

R1 (sweeps_5) adopted 2 changes from R4 Final baseline:
  - entry_end: 01:00 -> 00:00 (Δ+0.97)
  - tp1_ratio: 0.35 -> 0.20 (Δ+17.26)

R2 anchor:
  ORB: 20:00-20:10 (10m), entry until 00:00, flat 00:00
  stop=3.7%, min_gap_atr=0.90%, max_gap_atr=5.0%
  rr=1.75, tp1=0.20, ATR=5, direction=both, ICF=OFF, continuation, 1s magnifier
...
ANCHOR_SESSION = SessionConfig(
    ...
    orb_end="20:10",
    entry_start="20:10",
    entry_end="00:00",       # R1 adopted (was 01:00)
    flat_start="00:00",
    flat_end="07:00",
    stop_atr_pct=3.7,
...
ANCHOR = StrategyConfig(
    ...
    rr=1.75,
    tp1_ratio=0.20,          # R1 adopted (was 0.35)
    atr_length=5,
    impulse_close_filter=False,
    name="NQ Asia R5-R2 Anchor",
)
...
def main():
    print("NQ Asia ORB — Round 6 (R2): entry_end=00:00, tp1=0.20")
    print("=" * 90)
    print(f"Anchor: rr={ANCHOR.rr}, tp1={ANCHOR.tp1_ratio}, "
          f"stop={ANCHOR_SESSION.stop_atr_pct}%, gap={ANCHOR_SESSION.min_gap_atr_pct}%")
    print(f"ORB=10m (20:00-20:10), entry<=00:00, flat=00:00, ATR={ANCHOR.atr_length}, "
    ...
    # ── 4. ENTRY END TIME ────────────────────────────────────────────
    entry_ends = ["22:00", "23:00", "23:30", "00:00", "00:30", "01:00", "02:00"]
    print_header("4. ENTRY END TIME (anchor=00:00)")
    for i, ee in enumerate(entry_ends, 1):
        sess = replace(ANCHOR_SESSION, entry_end=ee)
        config = replace(ANCHOR, sessions=(sess,))
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"end={ee}", m, is_base=(ee == "00:00"))
    ...
    # ── 10. TP1 RATIO ────────────────────────────────────────────────
    tp1_values = [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40]
    print_header("10. TP1 RATIO (anchor=0.20)")
    for i, tp1 in enumerate(tp1_values, 1):
        config = replace(ANCHOR, tp1_ratio=tp1)
        _, m = run_and_metric(df_5m, df_1m, df_1s, config)
        print_row(i, f"tp1={tp1}", m, is_base=(abs(tp1 - 0.20) < 0.01))
    ...
    if not any_adopted:
        ...
        print(f"  Update anchor and re-sweep as R3 (run_nq_asia_variable_sweeps_7.py)")
