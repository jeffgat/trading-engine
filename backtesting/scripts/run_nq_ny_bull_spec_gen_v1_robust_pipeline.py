#!/usr/bin/env python3
"""NQ NY BULL SPECIALIST general_v1 — 5-Phase Robust Pipeline (Bailey-aware).

general_v1 execution config NQ_NY_BULL_SPECIALIST leg:
  ORB: 09:30-09:50, entry until 12:00, flat 15:30-16:00
  stop=6.0% ATR-12, min_gap=2.5%, rr=3.0, tp1=0.6
  Long only, Friday excluded
  Regime filter: bull only, no low-confidence, hh_hl_2_vwap gate
...
# ── Candidate config (general_v1 execution NQ_NY_BULL_SPECIALIST) ────────
...
CANDIDATE = StrategyConfig(
    ...
    name="NQ NY Bull Specialist general_v1 Robust Pipeline",
)

# ── Walk-forward param ranges (centered on candidate) ────────────────────
PARAM_RANGES = {
    "ny_stop_atr_pct": [4.0, 5.0, 6.0, 7.0, 8.0],
    "rr": [2.0, 2.5, 3.0, 3.5, 4.0],
    "ny_min_gap_atr_pct": [1.5, 2.0, 2.5, 3.0, 3.5],
    "tp1_ratio": [0.4, 0.5, 0.6, 0.7, 0.8],
}
...
def make_bull_specialist_gate(df):
    """Factory: build regime cal + 15m signals, return trade filter."""
    cal = build_nq_ny_regime_calendar(df)
    session = CANDIDATE.sessions[0]
    signals = build_structure_vwap_signals(df, session, atr_length=12)

    def gate(trades):
        trades = [t for t in trades if t.exit_type != EXIT_NO_FILL]
        trades = filter_trades_by_regime(trades, cal, include={"bull"})
        trades = filter_trades_by_low_confidence(trades, cal, include_low_confidence=False)
        trades = apply_structure_vwap_gate(trades, signals, "hh_hl_2_vwap")
        return trades

    return gate


def make_bull_specialist_gate_with_dow(df):
    """Factory for WF: compose DOW gate + bull specialist gate."""
    inner_gate = make_bull_specialist_gate(df)

    def gate(trades):
        trades = dow_gate(trades)
        return inner_gate(trades)
...
def phase_1(df, df_1m, df_1s):
    section("PHASE 1: STRUCTURAL VALIDATION (NQ NY BULL SPECIALIST)")
    print(f"  Running full pre-holdout backtest with 1s magnifier...")
    print(f"  DOW filter: exclude Friday")
    print(f"  Bull specialist gates: regime=bull, no low-confidence, hh_hl_2_vwap")

    t0 = time.time()
    trades = run_backtest(df, CANDIDATE, start_date=WF_START,
                          df_1m=df_1m, df_1s=df_1s)
    trades = dow_gate(trades)
    bull_gate = make_bull_specialist_gate(df)
    trades = bull_gate(trades)
    m = compute_metrics(trades)
    print(f"  Completed in {time.time() - t0:.1f}s")
    print_metrics(m, "Full pre-holdout metrics (bull specialist filtered)")
...
def phase_2(df, df_1m):
    section("PHASE 2: WALK-FORWARD OPTIMIZATION + STABILITY (NQ NY BULL SPECIALIST)")
    ...
    wf_result = run_walkforward(
        ...
        df_1m=df_wf_1m,
        gate_factory=make_bull_specialist_gate_with_dow,
    )
...
def phase_3(wf_result):
    section("PHASE 3: PROP FIRM CONSTRAINTS (on WF OOS trades — NQ NY BULL SPECIALIST)")
...
def phase_4(df, df_1m, df_1s, stability):
    section("PHASE 4: HOLD-OUT OOS TEST (NQ NY BULL SPECIALIST)")
    ...
    m = compute_metrics(trades)
    print(f"  Completed in {time.time() - t0:.1f}s")
    print_metrics(m, "Hold-out OOS metrics (bull specialist filtered)")

    # Log hold-out usage
    entry = log_holdout_test(
        HOLDOUT_START, HOLDOUT_END,
        config=mode_params,
        experiment_name="general_v1 NQ_NY_BULL_SPECIALIST robust pipeline",
    )
    if entry.warning:
...
def phase_5(wf_result):
    section("PHASE 5: MONTE CARLO SURVIVAL (NQ NY BULL SPECIALIST)")
...
def main():
    ...
    print(f"  Structural: ORB 20m, entry<12:00, flat 15:30, ATR={CANDIDATE.atr_length}, "
          f"dir={CANDIDATE.direction_filter}, DOW excl Fri")
    print(f"  Bull specialist: regime=bull, no low-confidence, hh_hl_2_vwap gate")
    ...
    # ── Final Verdict ─────────────────────────────────────────────────
    section("FINAL VERDICT (NQ NY BULL SPECIALIST)")
