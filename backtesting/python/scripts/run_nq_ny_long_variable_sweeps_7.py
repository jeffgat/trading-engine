#!/usr/bin/env python3
"""NQ NY Continuation — Variable Sweeps Round 7 (Longs-Only Fresh Start).

R7 anchor (R6 + 1 adoption):
  ORB: 09:30-09:50 (20m), entry until 12:00, flat 15:30-16:00
  stop=9.0%, min_gap_atr=2.5%, max_gap_pts=100.0, max_gap_atr=0.0
  rr=3.0, tp1=0.6, ATR=20, direction=long, ICF=ON, continuation, 1s magnifier
  DOW gate: excl Fri

R6 adoption applied:
  icf: OFF -> ON (+0.97)

ICF has oscillated OFF->ON->OFF->OFF->OFF->ON across rounds.
If it flips back to OFF in R7, lock at whichever has higher Calmar.
