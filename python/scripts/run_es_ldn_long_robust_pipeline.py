#!/usr/bin/env python3
"""ES London ORB Long-Only Robust Pipeline — 5-phase prop firm validation.

Instrument: ES (E-mini S&P 500), continuation strategy, LONG ONLY, bar magnifier enabled.
Session: London (03:00-03:15 ET ORB, entries until 08:25, flat by 08:25).
BE offset: 0 ticks.

Candidate params (from direction sweep — best Calmar):
  rr=3.0, stop_atr_pct=1.5, min_gap_atr_pct=1.25, tp1_ratio=0.3, risk=5000, direction=long
...
# Walk-forward sweep ranges (centered around candidate: rr=3.0, stop=1.5, gap=1.25, tp1=0.3)
PARAM_RANGES = {
    "rr": [2.0, 2.5, 2.75, 3.0, 3.5],
    "ldn_stop_atr_pct": [1.5, 2.0, 2.5, 3.0],
    "ldn_min_gap_atr_pct": [0.75, 1.0, 1.25, 1.5, 2.0],
    "tp1_ratio": [0.2, 0.25, 0.3, 0.4, 0.5],
}
...
def main():
    ...
    # Base config: ES LDN continuation, LONG ONLY, magnifier
    base_config = StrategyConfig(
        ...
        rr=3.0,
        tp1_ratio=0.3,
        name="ES LDN Long Robust Pipeline",
    )
    ...
    print(f"Params: rr=3.0, stop=1.5%, gap=1.25%, tp1=0.3, be=0, direction=long, risk=$5000")
